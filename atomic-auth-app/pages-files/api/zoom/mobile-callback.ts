...
export default async function handler(
    ...
    try {
        const body: RequestMinimalCalendarIntegrationType
            | RequestUpdateZoomIntegrationType = req.body

        if (body?.method === 'getMinimalCalendarIntegration') {
            const {
                userId,
                resource,
            } = body?.variables
            const zoomIntegration = await getMinimalCalendarIntegration(userId, resource)

            if (zoomIntegration?.id) {
                return res.status(200).json(zoomIntegration)
            }
        }
