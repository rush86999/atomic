...
export async function searchBox(
  ...
  const endpoint = `${PYTHON_API_SERVICE_BASE_URL}/api/box/search`;
  ...
  try {
    ...
    return handleAxiosError(error as AxiosError, 'searchBox');
  }
}
...
export async function listBoxFiles(
  ...
  const endpoint = `${PYTHON_API_SERVICE_BASE_URL}/api/box/list-files`;
  ...
  try {
    ...
    return handleAxiosError(error as AxiosError, 'listBoxFiles');
  }
}
