...
import { executeGraphQLMutation, executeGraphQLQuery } from '../../../../project/functions/_libs/graphqlClient';
...
async function saveCredential(userId: string, service: string, secret: string) {
    ...
    await executeGraphQLMutation(mutation, variables, 'InsertUserCredential', userId);
}

async function getCredential(userId: string, service: string): Promise<{ isConnected: boolean, value?: string | null }> {
    const query = `
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        if (!service || !secret) {
            return res.status(400).json({ message: 'Service and secret are required' });
        }
        try {
            await saveCredential(userId, service, secret);
            return res.status(200).json({ message: `${service} credentials saved successfully` });
        } catch (error) {
            console.error(`Error saving ${service} credentials:`, error);
            return res.status(500).json({ message: `Failed to save ${service} credentials` });
        }
        ...
        try {
            ...
            return res.status(500).json({ message: `Failed to check ${service} credentials` });
        }
    } else {
