...
describe('/api/meeting_attendance_status/[taskId] API Endpoint', () => {
  ...
  it('should return 405 if method is not GET', async () => {
    ...
    expect(mockedLogger.warn).toHaveBeenCalledWith(
      expect.stringContaining('Method not allowed'),
      expect.anything()
    );
  });
  ...
  it('should return 401 if session is not found', async () => {
    ...
    expect(mockedLogger.warn).toHaveBeenCalledWith(
      expect.stringContaining('Unauthorized attempt: No active session.'),
      expect.anything()
    );
  });
  ...
  it('should return 400 if taskId is missing', async () => {
    ...
    expect(mockedLogger.warn).toHaveBeenCalledWith(
      expect.stringContaining('Invalid or missing taskId.'),
      expect.anything()
    );
  });
  ...
  it('should fetch and return status record successfully', async () => {
    ...
    expect(responseData.current_status_message).toEqual(
      mockStatusRecord.current_status_message
    );
    ...
    expect(mockedLogger.info).toHaveBeenCalledWith(
      expect.stringContaining('Status found for task.'),
      expect.anything()
    );
  });
  ...
  it('should return 404 if task is not found or not authorized', async () => {
    ...
    expect(mockedLogger.info).toHaveBeenCalledWith(
      expect.stringContaining('Task not found or not authorized for user.'),
      expect.anything()
    );
  });
