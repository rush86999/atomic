...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!notionApiKey || !notionDatabaseId || !githubOwner || !githubRepo || !slackChannelId) {
        return res.status(400).json({ message: 'Notion, GitHub, and Slack credentials are required' });
    }

    try {
        // Notion Task Score
        const tasksResponse = await queryNotionTasks(userId, { database_id: notionDatabaseId }, notionApiKey);
        let notionScore = 0;
        ...
        // GitHub Score
        const commitActivity = await getRepoCommitActivity(userId, githubOwner, githubRepo);
        const pullRequests = await getRepoPullRequestActivity(userId, githubOwner, githubRepo);
        let githubScore = 0;
        if (commitActivity && pullRequests) {
            const commitScore = commitActivity.slice(-10).filter((week: any) => week.total > 0).length * 5;
        ...
        const messages = await searchMySlackMessages(userId, `in:${slackChannelId}`, 100);
        let sentimentScore = 50; // Default to neutral
        if (messages.length > 0) {
            const totalScore = messages.reduce((acc, msg) => acc + analyzeSentiment(msg.text), 0);
        ...
        const events = await getMeetingLoad(userId, timeMin.toISOString(), timeMax.toISOString());
        ...
        return res.status(500).json({ message: 'Failed to calculate project health score' });
    }
}
