...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!competitors || !notionDatabaseId) {
        return res.status(400).json({ message: 'Competitors and Notion Database ID are required' });
    }
    ...
    try {
        ...
        return res.status(500).json({ message: 'Failed to run competitor analysis' });
    }
}
