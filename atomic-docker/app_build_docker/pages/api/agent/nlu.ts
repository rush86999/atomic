...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            const nluResponse = await understandMessage(message, undefined, undefined, { service: 'openai', apiKey: process.env.OPENAI_API_KEY || '' });
            return res.status(200).json(nluResponse);
        } catch (error) {
            console.error('Error in NLU service:', error);
            return res.status(500).json({ message: 'Failed to process message with NLU service' });
        }
    } else {
