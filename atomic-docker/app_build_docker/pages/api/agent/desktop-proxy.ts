...
import { handleMessage, HandleMessageResponse } from '../../../../project/functions/atom-agent/handler';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            const atomResponse: HandleMessageResponse = await handleMessage(message, settings);
            return res.status(200).json(atomResponse);
        } catch (error) {
            console.error('Error in desktop proxy:', error);
            return res.status(500).json({ message: 'Failed to process message via desktop proxy' });
        }
    } else {
