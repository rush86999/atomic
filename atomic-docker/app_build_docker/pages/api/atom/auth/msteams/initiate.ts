...
import { PublicClientApplication, Configuration, LogLevel, AuthorizationUrlRequest } from '@azure/msal-node';
...
const MSTEAMS_SCOPES = ["Chat.Read", "ChannelMessage.Read.All", "User.Read", "offline_access"];
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!MSTEAMS_CLIENT_ID || MSTEAMS_CLIENT_ID === "YOUR_MSTEAMS_APP_CLIENT_ID") {
        logger.error("[MSTeamsAuthInitiate] MS Teams Client ID not configured.");
        return res.status(500).json({ message: "MS Teams OAuth configuration error on server." });
    }
    ...
    try {
        ...
        logger.info(`[MSTeamsAuthInitiate] Redirecting user ${userId} to MS Teams auth URL.`);
        res.redirect(authUrl);
    } catch (error) {
        logger.error("[MSTeamsAuthInitiate] Error generating MS Teams auth URL:", error);
        res.status(500).json({ message: "Failed to initiate MS Teams authentication." });
    }
}
