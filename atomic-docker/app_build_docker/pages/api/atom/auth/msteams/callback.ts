...
const MSTEAMS_CLIENT_SECRET =
  process.env.MSTEAMS_CLIENT_SECRET || 'YOUR_MSTEAMS_APP_CLIENT_SECRET';
...
const MSTEAMS_AUTHORITY =
  process.env.MSTEAMS_AUTHORITY || 'https://login.microsoftonline.com/common';
...
async function saveMSTeamsUserTokens(
  userId: string,
  tokens: AuthenticationResult
): Promise<{ ok: boolean; error?: any }> {
  logger.info(
    `[MSTeamsAuthCallback] Saving MS Teams tokens for userId: ${userId}`
  );

  if (!HASURA_GRAPHQL_URL || !HASURA_ADMIN_SECRET) {
    logger.error(
      '[MSTeamsAuthCallback] GraphQL client not configured for saveMSTeamsUserTokens.'
    );
    return {
      ok: false,
      error: {
  ...
  if (!tokens.accessToken || !tokens.account) {
    logger.error(
      '[MSTeamsAuthCallback] Critical token information missing from MSAL response.'
    );
    return {
      ok: false,
      error: {
  ...
  try {
    ...
    if (
      ...
      logger.info(
        `[MSTeamsAuthCallback] MS Teams tokens saved successfully for user ${userId}.`
      );
      return { ok: true };
    } else {
      logger.warn(
        `[MSTeamsAuthCallback] MS Teams token save reported 0 affected_rows for user ${userId}.`,
        response
      );
      return {
        ok: false,
    ...
    logger.error(
      `[MSTeamsAuthCallback] Exception saving MS Teams tokens for user ${userId}:`,
      error
    );
    return {
      ok: false,
...
export default async function handler(
  ...
  if (!sessionUserId || sessionUserId !== receivedState) {
    logger.error(
      `[MSTeamsAuthCallback] Invalid state or session mismatch. Session UserID: ${sessionUserId}, State: ${receivedState}`
    );
    return res
      .status(400)
  ...
  if (req.query.error) {
    logger.error(
      `[MSTeamsAuthCallback] Error from MS Identity platform for user ${userId}: ${req.query.error_description || req.query.error}`
    );
    return res.redirect(
      `/Settings/UserViewSettings?msteams_auth_error=${encodeURIComponent(req.query.error as string)}`
    );
  }

  if (!req.query.code || typeof req.query.code !== 'string') {
    logger.error(
      `[MSTeamsAuthCallback] No authorization code received for user ${userId}.`
    );
    return res.redirect(
      '/Settings/UserViewSettings?msteams_auth_error=no_code'
    );
  }
  ...
  if (
    ...
    logger.error(
      '[MSTeamsAuthCallback] MS Teams Client ID or Secret not configured.'
    );
    return res
      .status(500)
      .json({
  ...
  try {
    ...
    if (!msalResponse || !msalResponse.accessToken) {
      logger.error(
        `[MSTeamsAuthCallback] Failed to acquire MS Teams token for user ${userId}. MSAL response empty or missing accessToken.`,
        msalResponse
      );
    ...
    logger.info(
      `[MSTeamsAuthCallback] MS Teams token acquired successfully for user ${userId}. Account: ${msalResponse.account?.username}`
    );

    const saveResult = await saveMSTeamsUserTokens(userId, msalResponse);
    if (!saveResult.ok) {
      logger.error(
        `[MSTeamsAuthCallback] Failed to save MS Teams tokens for user ${userId}:`,
        saveResult.error
      );
      const errMsg = saveResult.error?.message || 'token_save_failed';
      return res.redirect(
        `/Settings/UserViewSettings?msteams_auth_error=${encodeURIComponent(errMsg)}`
      );
    }

    logger.info(
      `[MSTeamsAuthCallback] MS Teams successfully connected and tokens stored for user ${userId}.`
    );
    // Redirect to a success page or settings page
    return res.redirect('/Settings/UserViewSettings?msteams_auth_success=true');
  } catch (error: any) {
    logger.error(
      `[MSTeamsAuthCallback] Error acquiring/saving MS Teams token for user ${userId}:`,
      error
    );
    const errMsg =
      error.message || error.errorCode || 'msteams_token_acquisition_failed';
    return res.redirect(
      `/Settings/UserViewSettings?msteams_auth_error=${encodeURIComponent(errMsg)}`
    );
  }
