...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!consumerKey) {
        return res.status(500).json({ message: 'Pocket consumer key not configured.' });
    }

    const pocket = new PocketAPI({ consumer_key: consumerKey });

    try {
        const { access_token } = await pocket.getAccessToken({ request_token: code as string });
        ...
        return res.status(500).json({ message: 'Failed to complete Pocket OAuth flow' });
    }
}
