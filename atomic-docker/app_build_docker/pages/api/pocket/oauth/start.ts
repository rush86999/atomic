...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!consumerKey || !redirectUri) {
        return res.status(500).json({ message: 'Pocket environment variables not configured.' });
    }

    const pocket = new PocketAPI({ consumer_key: consumerKey, redirect_uri: redirectUri });
    ...
    try {
        ...
        const authorizationUrl = pocket.getAuthorizationURL({ request_token: code });
        
        res.redirect(authorizationUrl);
    } catch (error) {
        console.error('Error getting Pocket request token:', error);
        return res.status(500).json({ message: 'Failed to start Pocket authentication' });
    }
}
