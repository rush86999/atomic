...
export const createChatSocket = async (token: string) => {
  try {
    const socket = new WebSocket(
      `${chatWSAPIUrl}?${qs.stringify({ Auth: `Bearer ${token}` })}`
    );
    return socket;
  } catch (e) {
...
export const newSession = (
  ...
  setMessageHistory: React.Dispatch<
    React.SetStateAction<[] | SkillChatHistoryType>
  >,
...
export const receiveMessageFromBrain = async (
  ...
  try {
    ...
    if (skillDataForLog.query === 'missing_fields') {
      ...
      skillDataForLog.query === 'completed' ||
      ...
      console.warn(
        'receiveMessageFromBrain: Unhandled skillDataForLog.query state:',
        skillDataForLog.query,
        'Appending to log.'
      );
