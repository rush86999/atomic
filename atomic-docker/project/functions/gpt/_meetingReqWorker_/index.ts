...
import {
  meetingRequest as sendMeetingRequestEmail,
  streamToString,
} from '../_libs/api-helper';
...
const SCHEDULER_API_URL =
  process.env.SCHEDULER_API_URL || 'http://localhost:8080'; // Same as in app_build_docker
...
const callSchedulerApi = async (
  ...
  try {
    ...
    console.log(
      '[MeetingReqWorker] Scheduler API Payload:',
    ...
    if (
      ...
      console.log(
        '[MeetingReqWorker] Scheduler API success response:',
        response.body
      );
      ...
      console.error(
        `[MeetingReqWorker] Scheduler API error response (Status ${response.statusCode}):`,
        errorBody
      );
      return {
        success: false,
    ...
    console.error(
      '[MeetingReqWorker] Failed to call Scheduler API:',
      error.message,
      error.stack
    );
    return {
  ...
  try {
    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        ...
        console.log(
          '[MeetingReqWorker] S3 delete command output:',
        ...
        if (body.windowStartDate) {
          ...
          if (startDateObj.isValid()) {
            ...
            console.warn(
              `[MeetingReqWorker] Invalid windowStartDate format: ${body.windowStartDate}`
            );
          }
        } else {
          console.warn(
            `[MeetingReqWorker] windowStartDate is missing or undefined.`
          );
        }
        ...
        if (body.windowEndDate) {
          ...
          if (endDateObj.isValid()) {
            ...
            console.warn(
              `[MeetingReqWorker] Invalid windowEndDate format: ${body.windowEndDate}`
            );
          }
        } else {
          console.warn(
            `[MeetingReqWorker] windowEndDate is missing or undefined.`
          );
        }
        ...
        if (
          ...
          console.error(
            '[MeetingReqWorker] Could not determine preferredDate, preferredStartTimeFrom, or preferredStartTimeTo from S3 payload. Skipping scheduler call.'
          );
          ...
          const scheduleMeetingPayload: ScheduleMeetingRequestType = {
            participantNames: [body.sender, body.receiver].filter(
              (name) => !!name
            ), // Basic parsing
          ...
          console.log(
            '[MeetingReqWorker] Attempting to call scheduler API with payload:',
            JSON.stringify(scheduleMeetingPayload, null, 2)
          );
          const schedulerResponse = await callSchedulerApi(
            scheduleMeetingPayload
          );

          if (schedulerResponse.success) {
            console.log(
              '[MeetingReqWorker] Successfully called scheduler API. Response:',
              schedulerResponse.data
            );
            // TODO: Potentially use this response to enrich the email content or next steps
          } else {
            console.error(
              '[MeetingReqWorker] Failed to call scheduler API. Error:',
        ...
        console.log(
          '[MeetingReqWorker] Proceeding to generate and send meeting request email.'
        );
