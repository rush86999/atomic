...
class GoogleMeetAgent:
    ...
    async def start_audio_capture(self, meeting_url_to_confirm: str) -> AsyncIterator[bytes]:
        if not SOUNDDEVICE_AVAILABLE:
            err_msg = "GoogleMeetAgent: sounddevice library is not available or failed to initialize. Cannot start audio capture."
        ...
        if self.current_meeting_url != meeting_url_to_confirm:
            msg = f"GoogleMeetAgent: Meeting URL mismatch. Current: '{self.current_meeting_url}', requested: '{meeting_url_to_confirm}'.";
        ...
        try:
            ...
            if not all_devices:
                err_msg = "GoogleMeetAgent: No audio devices found by sounddevice."
            ...
            if self.target_device_specifier:
                logger.info(f"GoogleMeetAgent: Attempting to use specified audio device: '{self.target_device_specifier}'")
                try:
                    ...
                    if 0 <= dev_idx < len(all_devices):
                        ...
                        if candidate_device.get('max_input_channels', 0) > 0:
                            ...
                            logger.warning(f"GoogleMeetAgent: Device at index {dev_idx} ('{candidate_device.get('name')}') is not an input device. Max input channels: {candidate_device.get('max_input_channels', 0)}.")
                    else:
                        logger.warning(f"GoogleMeetAgent: Specified device index {dev_idx} is out of range (0-{len(all_devices)-1}).")
                except ValueError:
                    for dev in all_devices:
                        if self.target_device_specifier.lower() in dev.get('name', '').lower() and dev.get('max_input_channels', 0) > 0:
                            ...
                            logger.info(f"GoogleMeetAgent: Matched specified name to input device: '{dev.get('name')}'")
                            break

                if not self.selected_audio_device_info:
                    err_msg = f"GoogleMeetAgent: Specified audio device '{self.target_device_specifier}' not found as a valid input device."
                ...
                logger.info(f"GoogleMeetAgent (Linux): Attempting auto-detect for browsers: {self.target_linux_process_names}")
                ...
                if monitor_name:
                    ...
                    if found_device_info:
                        ...
                        logger.info(f"GoogleMeetAgent (Linux): Auto-detected and validated monitor source: '{self.selected_audio_device_info['name']}'")
                    else:
                        logger.warning(f"GoogleMeetAgent (Linux): Auto-detected monitor source '{monitor_name}' not found or not a valid input device via sounddevice. Falling back to default device.")
                else:
                    logger.info("GoogleMeetAgent (Linux): Could not auto-detect browser audio monitor source. Falling back to default device.")

            if not self.selected_audio_device_info: # Fallback to default
                logger.warning("*************************************************************************************")
                logger.warning("GoogleMeetAgent: WARNING: Using system default input device. This typically captures MICROPHONE audio.")
                logger.warning("For capturing Google Meet AUDIO OUTPUT, ensure system audio (what you hear) is routed")
                logger.warning("to this default input (e.g., via 'Stereo Mix' on Windows, or Loopback software).")
                logger.warning("*************************************************************************************")
                try:
                    default_input_device = sd.query_devices(kind='input')
                except ValueError as e:
                     logger.warning(f"GoogleMeetAgent: Could not query default input device directly ({e}). Checking all default devices.")
                ...
                if not default_input_device or default_input_device.get('max_input_channels', 0) == 0:
                    err_msg = "GoogleMeetAgent: No suitable default audio input device found or default device is not an input device."
                    detailed_device_list = "\n".join([f"  - Index {i}: {d.get('name')} (Inputs: {d.get('max_input_channels',0)})" for i,d in enumerate(all_devices)])
                    logger.error(f"{err_msg}\nDefault device query result: {default_input_device}\nAvailable devices:\n{detailed_device_list}")
                    raise AudioDeviceSelectionError(err_msg)
                self.selected_audio_device_info = default_input_device
                logger.info(f"GoogleMeetAgent: Successfully selected default input device: '{self.selected_audio_device_info.get('name')}'")

            if not self.selected_audio_device_info:
                err_msg = "GoogleMeetAgent: Audio device selection process failed unexpectedly."
                logger.error(err_msg)
                raise AudioDeviceSelectionError(err_msg)
            ...
            logger.info(f"GoogleMeetAgent: Final selected audio device for capture: '{device_name_for_log}' (Sounddevice Index: {device_id_for_stream}, HostAPI: {host_api_name}, Inputs: {self.selected_audio_device_info.get('max_input_channels')})")
        except AudioDeviceSelectionError: raise
        except SoundDeviceNotAvailableError: raise
        except Exception as e:
            err_msg = f"GoogleMeetAgent: An unexpected error occurred during audio device selection: {e}"
