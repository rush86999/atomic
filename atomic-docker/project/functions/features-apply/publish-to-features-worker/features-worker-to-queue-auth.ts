...
const publisher = async (
  req: Request,
  res: Response<FeaturesApplyResponse<SuccessPayload>>
) => {
  ...
  try {
    ...
    console.log(
      `Message successfully added to queue topic ${kafkaFeaturesApplyTopic} by auth-publisher. Message ID/Offset: ${messageId}`
    );
    ...
    return res.status(202).json({
      ...
      data: {
        message:
          'Successfully published message to features worker queue (auth).',
    ...
    if (transaction) {
      try {
        ...
        console.error(
          'Failed to abort Kafka transaction (auth-publisher):',
          abortError
        );
      }
    }
    ...
    if (producer) {
      try {
        ...
        console.error(
          'Failed to disconnect Kafka producer (auth-publisher):',
          disconnectError
        );
      }
    }
