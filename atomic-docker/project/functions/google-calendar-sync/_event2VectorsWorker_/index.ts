...
import {
  bulkUpsertToLanceDBEvents,
  bulkDeleteFromLanceDBEvents,
} from '../_libs/lancedb_helper';
    ...
    for (const eventObject of body?.events) {
      if (eventObject?.method === 'upsert') {
        ...
        const startDate = dayjs(
          event?.start?.dateTime?.slice(0, 19) ||
            event?.start?.date?.slice(0, 19)
        )
          .tz(event?.start?.timeZone, true)
          .format();
        const endDate = dayjs(
          event?.end?.dateTime?.slice(0, 19) || event?.end?.date?.slice(0, 19)
        )
