...
export const initialGoogleCalendarSync2 = async (
  ...
  try {
    ...
    if (deletedEvents?.[0]?.id) {
      ...
      const alreadyDeletedEvents = await deleteEvents(
        deletedEvents,
        calendarId
      );
    ...
    if (e.code === 410) {
      console.log(
        `Google API returned 410 (GONE) for calendar ${calendarId}. Attempting full resync.`
      );
      // Assuming resetGoogleSyncForCalendar is designed to handle a full resync
      // and returns true on success, false on failure.
      return await resetGoogleSyncForCalendar(
        calendarId,
        userId,
        clientType,
        colorItem
...
export const performCalendarSync = async (
  params: PerformCalendarSyncParams
) => {
  ...
  const syncEnabled = await initialGoogleCalendarSync2(
    calendarId,
    userId,
    clientType,
    colorItem
