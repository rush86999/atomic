...
import {
  ...
  hasuraAdminSecret,
  hasuraGraphUrl,
  kafkaGoogleCalendarSyncGroupId,
  kafkaGoogleCalendarSyncTopic,
  searchIndex,
  selfGoogleCalendarWebhookPublicUrl,
  vectorDimensions,
  zoomBaseTokenUrl,
  zoomBaseUrl,
  zoomClientId,
...
export const insertEventTriggers = async (objects: EventTriggerType[]) => {
  try {
    ...
    const res: { data: { insert_Event_Trigger: { affected_rows: number } } } =
      await got
        .post(hasuraGraphUrl, {
          json: {
            operationName,
...
export const getCalendarWebhookById = async (channelId: string) => {
  try {
    ...
    const res: {
      data: { Calendar_Push_Notification_by_pk: CalendarWebhookType };
    } = await got
      .post(hasuraGraphUrl, {
        headers: {
          "X-Hasura-Admin-Secret": hasuraAdminSecret,
...
export const deleteCalendarWebhookById = async (channelId: string) => {
  try {
    ...
    const res: {
      data: { delete_Calendar_Push_Notification_by_pk: CalendarWebhookType };
    } = await got
      .post(hasuraGraphUrl, {
        headers: {
          "X-Hasura-Admin-Secret": hasuraAdminSecret,
...
export const stopCalendarWatch = async (id: string, resourceId: string) => {
  try {
...
export const getCalendarWebhookByCalendarId = async (calendarId: string) => {
  try {
    ...
    const res: { data: { Calendar_Push_Notification: CalendarWebhookType[] } } =
      await got
        .post(hasuraGraphUrl, {
          headers: {
            "X-Hasura-Admin-Secret": hasuraAdminSecret,
...
export const insertCalendarWebhook = async (webhook: CalendarWebhookType) => {
  try {
    ...
    const response: {
      data: { insert_Calendar_Push_Notification_one: CalendarWebhookType };
    } = await got
      .post(hasuraGraphUrl, {
        headers: {
          "X-Hasura-Admin-Secret": hasuraAdminSecret,
...
export const getGoogleIntegration = async (calendarIntegrationId: string) => {
  try {
    ...
    if (response?.data?.Calendar_Integration_by_pk) {
      const {
        data: {
          Calendar_Integration_by_pk: { token, refreshToken, clientType },
        },
      } = response;
...
export const getGoogleColor = async (token: string) => {
  try {
    const config = {
      headers: {
        Authorization: `Bearer ${token}`,
...
export const getGoogleCalendarInDb = async (calendarId: string) => {
  try {
    ...
    if (response?.data?.Calendar_by_pk) {
      const {
        data: {
          Calendar_by_pk: { pageToken, syncToken, id },
        },
      } = response;
...
export const generateBreakEventsForCalendarSync = async (
  ...
  try {
    ...
    const breakEvents = await generateBreakEventsForDate(
      userPreferences,
      userId,
...
export async function directUpdateGoogleEventAndHasura(
  ...
  try {
    ...
    const patchRequestBody: Partial<GoogleEventPatchAttributes> = {
      ...updates,
    };
    ...
    if (updates.conferenceData === null) {
      ...
      if (
        updates.conferenceData.entryPoints &&
