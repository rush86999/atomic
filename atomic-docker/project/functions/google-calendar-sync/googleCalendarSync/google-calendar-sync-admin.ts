...
const handler = async (req: Request, res: Response) => {
  try {
    ...
    if (!calendarIntegrationId) {
      return res.status(400).json({ message: 'no calendarIntegrationId found', event: bodyObj })
    }
    if (!calendarId) {
      return res.status(400).json({ message: 'no calendarId found', event: bodyObj })
    }
    if (!userId) {
      return res.status(400).json({ message: 'no userId found', event: bodyObj })
    }
    if (!timezone) {
      return res.status(400).json({ message: 'no timezone present', event: bodyObj })
    }
