...
describe('_chat/_libs/api-helper.ts', () => {
    ...
    describe('resilientGotPostHasura (tested via a consumer like upsertConference)', () => {
        ...
        it('should succeed on the second attempt after a 503 error', async () => {
            mockedGot.post
                ...
                .mockResolvedValueOnce({ json: () => Promise.resolve({ data: { insert_Conference_one: { id: 'conf1' } } }) } as any);
        ...
        it('should fail after all retries for persistent 500 errors', async () => {
            mockedGot.post
                .mockRejectedValueOnce({ response: { statusCode: 500 }, message: 'Internal Server Error 1', isGotError: true })
                .mockRejectedValueOnce({ response: { statusCode: 500 }, message: 'Internal Server Error 2', isGotError: true })
                .mockRejectedValueOnce({ response: { statusCode: 500 }, message: 'Internal Server Error 3', isGotError: true });
        ...
        it('should fail immediately on a non-retryable 400 error', async () => {
            mockedGot.post.mockRejectedValueOnce({ response: { statusCode: 400 }, message: 'Bad Request', isGotError: true });
        ...
        it('should handle GraphQL errors in response (and retry by default)', async () => {
             mockedGot.post
                .mockResolvedValueOnce({ json: () => Promise.resolve({ errors: mockGraphQLErrors }) } as any)
                .mockResolvedValueOnce({ json: () => Promise.resolve({ data: { insert_Conference_one: { id: 'conf1' } } }) } as any);

            const result = await upsertConference(mockConferenceInput);
            expect(result?.id).toBe('conf1');
            expect(mockedGot.post).toHaveBeenCalledTimes(2);
            expect(spiedLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Hasura call attempt 1 for UpsertConference failed"), expect.objectContaining({ error: expect.stringContaining("GraphQL error in UpsertConference")}));
        });
    ...
    describe('refreshZoomToken (axios resilience)', () => {
        ...
        const mockZoomSuccess = { access_token: 'new_zoom_token', expires_in: 3600 };
        ...
        it('should fail after all retries for persistent 500 errors', async () => {
            mockedAxios
                .mockRejectedValue({ isAxiosError: true, response: { status: 500 }}); // Fails all 3 times
    ...
    describe('convertEventTitleToOpenAIVector (OpenAI resilience)', () => {
        ...
        it('should succeed on first attempt', async () => {
            mockOpenAICreateEmbedding.mockResolvedValueOnce({ data: [{ embedding: mockEmbedding }] });
        ...
        it('should retry on API error then succeed', async () => {
            mockOpenAICreateEmbedding
                .mockRejectedValueOnce({ response: { status: 500 }, message: "OpenAI server error" })
    ...
    describe('refreshGoogleToken (via resilientGotGoogleAuth)', () => {
        ...
        const mockGoogleSuccess = { access_token: 'new_google_token', expires_in: 3599, scope: 'test_scope', token_type: 'Bearer' };

        it('should succeed on the first attempt', async () => {
            mockedGot.post.mockResolvedValueOnce({ json: () => Promise.resolve(mockGoogleSuccess) } as any);
            ...
            expect(mockedGot.post).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    form: expect.objectContaining({ refresh_token: refreshToken, grant_type: 'refresh_token' }),
                    timeout: { request: 10000 }
                })
            );
        ...
        it('should retry on 503 then succeed', async () => {
            mockedGot.post
                .mockRejectedValueOnce({ response: { statusCode: 503 }, message: 'Server Error', isGotError: true })
                .mockResolvedValueOnce({ json: () => Promise.resolve(mockGoogleSuccess) } as any);
        ...
        it('should fail after all retries for persistent 500 error', async () => {
            mockedGot.post.mockRejectedValue({ response: { statusCode: 500 }, message: 'Internal Error', isGotError: true });
        ...
        it('should fail immediately on 400 error', async () => {
            mockedGot.post.mockRejectedValueOnce({ response: { statusCode: 400, body: { error: 'invalid_grant' } }, message: 'Bad Request', isGotError: true });
    ...
    describe('getGoogleAPIToken', () => {
        ...
        it('should refresh token if expired', async () => {
            ...
            refreshGoogleTokenMock.mockResolvedValueOnce({ access_token: 'new-refreshed-token', expires_in: 3600 });
        ...
        it('should refresh token if token is missing', async () => {
            ...
            refreshGoogleTokenMock.mockResolvedValueOnce({ access_token: 'new-refreshed-token-no-original', expires_in: 3600 });
            updateCalendarIntegrationMock.mockResolvedValueOnce(undefined);
        ...
        it('should throw and attempt to disable integration if getCalendarIntegrationByName returns nothing', async () => {
            ...
            await expect(require('./api-helper').getGoogleAPIToken(userId, name, clientType))
                .rejects
                .toThrow('Calendar integration or essential details not found');
    ...
    describe('getGlobalCalendar', () => {
        ...
        it('should return calendar data on successful fetch', async () => {
            resilientGotPostHasuraMock.mockResolvedValueOnce({ Calendar: [mockCalendarData] });
    ...
    describe('Google Calendar API Event Functions (create, patch, delete)', () => {
        ...
        describe('createGoogleEvent', () => {
            ...
            const eventData = { summary: eventSummary, startDateTime: dayjs().add(1, 'day').toISOString(), endDateTime: dayjs().add(1, 'day').add(1, 'hour').toISOString(), timezone: 'America/New_York' };
            ...
            it('should create event successfully on first attempt', async () => {
                ...
                mockGoogleEventsInsert.mockResolvedValueOnce({ data: { id: mockGCalEventId, summary: eventSummary } });
                ...
                expect(mockGoogleEventsInsert).toHaveBeenCalledWith(expect.objectContaining({ calendarId, requestBody: expect.objectContaining({ summary: eventSummary }) }), { timeout: 20000 });
            ...
            it('should retry on 500 error then succeed', async () => {
                ...
                mockGoogleEventsInsert
                    ...
                    .mockResolvedValueOnce({ data: { id: mockGCalEventId, summary: eventSummary } });
            ...
            it('should fail immediately on 401 error', async () => {
                getGoogleAPITokenMock.mockResolvedValue('fake-google-api-token');
                mockGoogleEventsInsert.mockRejectedValueOnce({ code: 401, message: 'Unauthorized' });

                await expect(require('./api-helper').createGoogleEvent(userId, calendarId, clientType, mockOurEventId, eventData.endDateTime, eventData.startDateTime, 1, undefined, undefined, undefined, undefined, eventSummary, undefined, eventData.timezone))
        ...
        describe('patchGoogleEvent', () => {
            ...
            it('should patch event successfully', async () => {
                getGoogleAPITokenMock.mockResolvedValue('fake-google-api-token');
                mockGoogleEventsPatch.mockResolvedValueOnce({ data: { id: mockGCalEventId, summary: eventSummaryUpdate }});
                ...
                expect(mockGoogleEventsPatch).toHaveBeenCalledWith(expect.objectContaining({ calendarId, eventId: mockGCalEventId, requestBody: expect.objectContaining(patchData) }), { timeout: 20000 });
            ...
            it('should retry on 403 rateLimitExceeded then succeed', async () => {
                 ...
                 mockGoogleEventsPatch
                    .mockRejectedValueOnce({ code: 403, errors: [{ reason: 'rateLimitExceeded' }], message: 'Rate limit' })
                    .mockResolvedValueOnce({ data: { id: mockGCalEventId, summary: eventSummaryUpdate }});
        ...
        describe('extrapolateDateFromJSONData', () => {
            ...
            it('should run and make a debug log call', () => {
                ...
                expect(spiedLogger.debug).toHaveBeenCalledWith(
                    expect.stringContaining('[extrapolateDateFromJSONData] Initial params:'),
                    expect.anything()
                );
            });
        });
