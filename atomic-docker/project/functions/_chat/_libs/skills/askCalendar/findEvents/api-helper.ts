...
export const queryEventsControlCenter = async (
    ...
    try {
        ...
        if (eventsRes?.query === 'completed') {
            const assistantMessage = await generateAssistantMessageFromAPIResponseForUserQuery(openai, eventsRes.data as string, messageHistoryObject)
            ...
            const assistantMessage = await generateAssistantMessageToRequestUserForMissingFields(openai, eventsRes?.data as RequiredFieldsType, messageHistoryObject)
