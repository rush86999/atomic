...
export const processQueryEventPending = async (
  ...
  try {
    ...
    const response2 = await finalStepQueryEvent(
      body,
      windowStartDate,
      windowEndDate,
      timezone,
  ...
  try {
    ...
    switch (query) {
      case 'pending':
        ...
        eventRes = await processQueryEventPending(
          userId,
          timezone,
          jsonBody,
          attributesObject,
      ...
      case 'missing_fields':
        ...
        eventRes = await processQueryEventMissingFieldsReturned(
          userId,
          timezone,
    ...
    if (eventRes?.query === 'completed') {
      ...
      const assistantMessage =
        await generateAssistantMessageToRequestUserForMissingFields(
          openai,
          eventRes?.data as RequiredFieldsType,
          messageHistoryObject
        );
