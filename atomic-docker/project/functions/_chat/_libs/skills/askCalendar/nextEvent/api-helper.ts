...
export const nextEventControlCenter = async (
    ...
    try {
        ...
        if (nextEventRes?.query === 'completed') {
            const assistantMessage = await generateAssistantMessageFromAPIResponseForUserQuery(openai, nextEventRes.data as string, messageHistoryObject)
            ...
            const assistantMessage = await generateAssistantMessageToRequestUserForMissingFields(openai, nextEventRes?.data as RequiredFieldsType, messageHistoryObject)
