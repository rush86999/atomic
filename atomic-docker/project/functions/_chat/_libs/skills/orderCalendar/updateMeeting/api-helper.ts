...
export const processUpdateMeetingPending = async (
  ...
  try {
    ...
    const response2 = await finalStepUpdateMeeting(
      body,
  ...
  try {
    ...
    switch (query) {
      case 'pending':
        ...
        updateMeetingRes = await processUpdateMeetingPending(
          userId,
          timezone,
          jsonBody,
          dateTime,
      ...
      case 'missing_fields':
        ...
        updateMeetingRes = await processUpdateMeetingMissingFieldsReturned(
          userId,
          timezone,
    ...
    if (updateMeetingRes?.query === 'completed') {
      const assistantMessage =
        await generateAssistantMessageFromAPIResponseForUserQuery(
          openai,
          updateMeetingRes.data as string,
          messageHistoryObject
        );
      ...
      const assistantMessage =
        await generateAssistantMessageToRequestUserForMissingFields(
          openai,
          updateMeetingRes?.data as RequiredFieldsType,
          messageHistoryObject
        );
