...
import {
  dateAvailabilityFunctionName,
  summarizeAvailabilityFunctionName,
} from './constants';
import {
...
export const processGenerateMeetingInvitePending = async (
  ...
  try {
    ...
    const response2 = await finalStepGenerateMeetingInvite(
      body,
      hostInfo,
...
export const processGenerateMeetingInviteMissingFieldsReturned = async (
  ...
  try {
    ...
    console.log(
      e,
      ' unable to generate meeting invite missing fields returned'
    );
  }
};
  ...
  try {
    ...
    switch (query) {
      case 'pending':
        ...
        GMIRes = await processGenerateMeetingInvitePending(
          userId,
          timezone,
          jsonBody,
          dateTime,
      ...
      case 'missing_fields':
        ...
        GMIRes = await processGenerateMeetingInviteMissingFieldsReturned(
          userId,
          timezone,
    ...
    if (GMIRes?.query === 'completed') {
      ...
      const assistantMessage =
        await generateAssistantMessageToRequestUserForMissingFields(
          openai,
          GMIRes?.data as RequiredFieldsType,
          messageHistoryObject
        );
