...
import { MeetingUrlQueryParamsType, ReceiverTimezoneFormDataResponseType, GenerateMeetingInviteType } from "./types"
import RequiredFieldsType from "@chat/_libs/types/RequiredFieldsType"
import requiredFields from "./requiredFields"
import { callOpenAI, extrapolateEndDateFromJSONData, extrapolateStartDateFromJSONData, generateAssistantMessageFromAPIResponseForUserQuery, generateAssistantMessageToRequestUserForMissingFields, generateDateTime, generateJSONDataFromUserInput, generateMissingFieldsDateTime, generateMissingFieldsJSONDataFromUserInput, getCalendarIntegrationByName, getContactByNameWithUserId, getGlobalCalendar, getUserGivenId, listUserContactInfosGivenUserId } from "@chat/_libs/api-helper"
...
import { atomicCalendarSubdomainUrl, defaultOpenAIAPIKey, googleCalendarName, noReplyAddress, openAIChatGPT35LongModel, openAIChatGPT35Model, replyToAddress } from "@chat/_libs/constants"
import { DayAvailabilityType, SummarizeDayAvailabilityType } from "./availabilityTypes"
import { dateAvailabilityFunctionName, summarizeAvailabilityFunctionName } from './constants';
...
export const finalStepGenerateMeetingInvite = async (
    body: GenerateMeetingInviteType,
    ...
    try {
        ...
        const uniqDates = _.uniqBy(availableSlots, (curr) => (dayjs(curr?.startDate?.slice(0, 19)).tz(body?.receiverTimezone).format('YYYY-MM-DD')))
        ...
        for (const uniqDate of uniqDates) {
            ...
            const filteredAvailability = availableSlots?.filter(a => (dayjs(a?.startDate).tz(body?.receiverTimezone).format('YYYY-MM-DD') === dayjs(uniqDate?.startDate).tz(body?.receiverTimezone).format('YYYY-MM-DD')))
        ...
        const meetingQueryParams: MeetingUrlQueryParamsType = {
            ...
            duration: body?.duration ? `${body?.duration}` : `${defaultMeetingPreferences?.duration || 30}`,
            ...
            anyoneCanAddSelf: defaultMeetingPreferences?.anyoneCanAddSelf ? 'true' : 'false',
            guestsCanInviteOthers: defaultMeetingPreferences?.guestsCanInviteOthers ? 'true' : 'false',
...
export const processGenerateMeetingInvitePending = async (
    userId: string,
    timezone: string,
    ...
    try {
        ...
        const body: GenerateMeetingInviteType = {
            userId,
            timezone,
...
export const processGenerateMeetingInviteMissingFieldsReturned = async (
    userId: string,
    timezone: string,
    jsonBody: UserInputToJSONType,
    dateJSONBody: DateTimeJSONType,
    ...
    try {
        ...
        let windowEndDate = extrapolateEndDateFromJSONData(
            ...
            minuteEnd,
            endTimeEnd,
            dateJSONBody?.findTimeWindowEnd?.relativeTimeChangeFromNow || messageHistoryObject?.prevDateJsonBody?.findTimeWindowEnd?.relativeTimeChangeFromNow,
            dateJSONBody?.findTimeWindowEnd?.relativeTimeFromNow || messageHistoryObject?.prevDateJsonBody?.findTimeWindowEnd?.relativeTimeFromNow,
        )
        ...
        const newBody: GenerateMeetingInviteType = {
            userId,
            timezone,
        ...
        const prevBody: GenerateMeetingInviteType = {
...
export const generateMeetingInviteControlCenter = async (
    openai: OpenAI,
    userId: string,
    timezone: string,
    messageHistoryObject: SkillMessageHistoryType,
    ...
    try {
        ...
        if (GMIRes?.query === 'completed') {
            ...
            const assistantMessage = await generateAssistantMessageToRequestUserForMissingFields(openai, GMIRes?.data as RequiredFieldsType, messageHistoryObject)

            messageHistoryObject.messages.push(assistantMessage)
            messageHistoryObject.query = 'missing_fields'
            messageHistoryObject.required = GMIRes?.data as RequiredFieldsType
