...
export const deletePriorityControlCenter = async (
    ...
    try {
        ...
        if (deletePriorityRes?.query === 'completed') {
            const assistantMessage = await generateAssistantMessageFromAPIResponseForUserQuery(openai, deletePriorityRes.data as string, messageHistoryObject)
            ...
            const assistantMessage = await generateAssistantMessageToRequestUserForMissingFields(openai, deletePriorityRes?.data as RequiredFieldsType, messageHistoryObject)

            messageHistoryObject.messages.push(assistantMessage)
            messageHistoryObject.query = 'missing_fields'
            messageHistoryObject.required = deletePriorityRes?.data as RequiredFieldsType
            ...
            messageHistoryObject.prevDateJsonBody = deletePriorityRes?.prevDateJsonBody
