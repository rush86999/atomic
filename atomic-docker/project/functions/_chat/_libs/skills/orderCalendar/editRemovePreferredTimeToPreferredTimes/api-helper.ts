...
export const ERPT2PTControlCenter = async (
    ...
    try {
        ...
        if (eRPT2PTsRes?.query === 'completed') {
            const assistantMessage = await generateAssistantMessageFromAPIResponseForUserQuery(openai, eRPT2PTsRes.data as string, messageHistoryObject)
            ...
            const assistantMessage = await generateAssistantMessageToRequestUserForMissingFields(openai, eRPT2PTsRes?.data as RequiredFieldsType, messageHistoryObject)
