...
const handler = async (req: Request<{}, {}, GenerateMcpAuthUrlRequestBody>, res: Response) => {
  try {
    ...
    if (!redirectUri) {
        ...
        return res.status(500).json({ message: 'Server configuration error: Missing redirect URL for Mcp integration.' });
    }

    if (!googleClientIdMcp) {
        console.error('GOOGLE_CLIENT_ID_MCP environment variable is not configured.');
        return res.status(500).json({ message: 'Server configuration error: Missing Client ID for Mcp integration.' });
    }
    ...
    if (!googleClientSecretMcp) {
        ...
        return res.status(500).json({ message: 'Server configuration error: Missing Client Secret for Mcp integration.' });
    }
    ...
    return res.status(500).json({
      message: 'Error generating Mcp authorization URL: ' + (e.message || 'Unknown error'),
    });
  }
};
