...
const handler = async (
  ...
  try {
    console.log(
      `[Disconnect Gmail] Attempting to delete tokens for user: ${userId}`
    );
    const result = await adminGraphQLClient.request(
      DELETE_GMAIL_TOKENS_MUTATION,
      { userId }
    );
    ...
    if (result.delete_user_gmail_tokens) {
      if (result.delete_user_gmail_tokens.affected_rows > 0) {
        console.log(
          `[Disconnect Gmail] Successfully deleted tokens for user: ${userId}`
        );
        return res
          .status(200)
          .json({
        ...
        console.log(
          `[Disconnect Gmail] No tokens found to delete for user: ${userId} (already disconnected or never connected).`
        );
      ...
      console.error(
        `[Disconnect Gmail] Unexpected response structure from Hasura for user ${userId}:`,
        result
      );
      return res
        .status(500)
    ...
    return res.status(500).json({
      success: false,
      message:
        'An error occurred while disconnecting Gmail account: ' +
