...
const handler = async (
  req: Request<{}, {}, GenerateGmailAuthUrlRequestBody>,
  res: Response
) => {
  try {
    ...
    if (!googleClientIdGmail) {
      console.error(
        'GOOGLE_CLIENT_ID_GMAIL environment variable is not configured.'
      );
      return res
        .status(500)
        .json({
          message:
            'Server configuration error: Missing Client ID for Gmail integration.',
        });
    }
    ...
    if (!googleClientSecretGmail) {
      console.error(
        'GOOGLE_CLIENT_SECRET_GMAIL environment variable is not configured.'
      );
      return res
        .status(500)
        .json({
          message:
            'Server configuration error: Missing Client Secret for Gmail integration.',
        });
    }
    ...
    return res.status(500).json({
      message:
        'Error generating Gmail authorization URL: ' +
