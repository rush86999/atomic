...
const handler = async (
  req: Request<{}, {}, SearchUserGmailRequestBody>,
  res: Response<SearchUserGmailResponse>
) => {
  ...
  try {
    ...
    const rawMessages: gmail_v1.Schema$Message[] = await searchUserEmails(
      userId,
      query,
      maxResults
    );
    ...
    const results: GmailSearchResultItem[] = rawMessages.map(
    ...
    const errorMessage =
      e.message || 'An unexpected error occurred while searching emails.';
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            'Gmail authentication failed or is required. Please connect/reconnect your Gmail account via settings.',
        });
    }
