...
const handler = async (
  ...
  try {
    const tokenRecordResult = await adminGraphQLClient.request(
      GET_USER_GMAIL_ACCESS_TOKEN_QUERY,
      { userId }
    );

    if (
      !tokenRecordResult.user_gmail_tokens ||
    ...
    if (!tokenRecord.encrypted_access_token) {
      ...
      console.warn(
        `[Connection Status] Token record exists for user ${userId} but no encrypted_access_token found.`
      );
      return res
        .status(200)
        .json({
    ...
    try {
      ...
      if (accessToken) {
        ...
        if (!isEffectivelyConnected) {
          console.warn(
            `[Connection Status] Could not retrieve user email via Google API for user ${userId}, though token was decrypted. Token might be invalid or lack scopes.`
          );
        }
      } else {
        console.error(
          `[Connection Status] Failed to decrypt access token for user ${userId}. Encryption key issue or corrupt data.`
        );
        // Cannot verify connection without decrypted token.
      }
    } catch (googleApiError: any) {
      console.warn(
        `[Connection Status] Google API error while verifying token for user ${userId}. Error: ${googleApiError.message}. Token might be stale or revoked.`
      );
