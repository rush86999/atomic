...
async def save_tokens(db_conn_pool, user_id: str, encrypted_access_token: bytes, encrypted_refresh_token: bytes, expires_at: datetime, scope: str):
    sql = """
        INSERT INTO user_trello_oauth_tokens (user_id, encrypted_access_token, encrypted_refresh_token, expires_at, scope, created_at, updated_at)
...
async def get_tokens(db_conn_pool, user_id: str):
    sql = "SELECT encrypted_access_token, encrypted_refresh_token, expires_at FROM user_trello_oauth_tokens WHERE user_id = %s;"
    try:
        with db_conn_pool.getconn() as conn:
            with conn.cursor() as cur:
...
async def delete_tokens(db_conn_pool, user_id: str):
    sql = "DELETE FROM user_trello_oauth_tokens WHERE user_id = %s;"
    try:
        with db_conn_pool.getconn() as conn:
            with conn.cursor() as cur:
                cur.execute(sql, (user_id,))
