...
from datetime import datetime # For /api/process-recorded-audio-note
...
try:
    ...
    # Logger not available yet, so print is okay here for critical startup failure
    print(f"CRITICAL: Error importing note_utils: {e}", file=sys.stderr)
...
try:
    ...
    app.logger.warning(f"Could not import search_routes_bp: {e}. Search routes will not be available.")
...
try:
    ...
    app.logger.warning(f"Could not import task_bp: {e}. Task routes will not be available.")
    task_bp = None
...
if search_routes_bp:
    ...
    app.logger.info("Successfully registered search_routes_bp with /api prefix.")
else:
    app.logger.info("Search routes blueprint (search_routes_bp) not registered as it failed to import.")
...
if task_bp:
    ...
    app.logger.info("Successfully registered task_bp with /api prefix.")
else:
    app.logger.info("Task routes blueprint (task_bp) not registered as it failed to import.")
...
def _init_clients_from_request_data(data: dict) -> dict | None:
    ...
    # deepgram_api_key = data.get('deepgram_api_key') # Deepgram init in note_utils is usually global or per-call
    ...
    # Deepgram client in note_utils is typically initialized globally or per-call with key from env/param.
    # No explicit init_deepgram(key) call needed here if note_utils handles it internally.
    # If a specific init_deepgram was used in note_utils and expected here, it's removed for now
    # based on current note_utils structure.
    return None
...
def create_note_route():
    ...
    if notion_result["status"] == "success":
        ...
        if not openai_api_key:
            ...
            app.logger.warning(f"For note {page_id}: {vector_error_message}")
                ...
                if embedding_response["status"] == "success":
                    ...
                    try:
                        ...
                        upsert_result = lancedb_service.upsert_note_vector(
                            ...
                            text_content=text_to_embed, vector_embedding=vector
                        )
                        if upsert_result["status"] == "success":
                            ...
                            app.logger.error(f"Error upserting vector for note {page_id}: {vector_error_message} (Code: {upsert_result.get('code')})")
                    except ImportError:
                        vector_update_status = "failed"
                        vector_error_message = "LanceDB service module not found or import error."
                        app.logger.error(f"Error for note {page_id}: {vector_error_message}")
                    except Exception as ldb_e:
                        vector_update_status = "failed"
                        vector_error_message = f"LanceDB operation error: {str(ldb_e)}"
                        app.logger.error(f"Error for note {page_id}: {vector_error_message}", exc_info=True)
                else:
                    vector_update_status = "failed"
                    vector_error_message = embedding_response.get("message", "Embedding generation failed.")
                    app.logger.error(f"Error generating embedding for note {page_id}: {vector_error_message} (Code: {embedding_response.get('code')})")
            except Exception as e:
                vector_update_status = "failed"
                vector_error_message = f"Unexpected error during vector processing: {str(e)}"
                app.logger.error(f"Error for note {page_id} during vector processing: {vector_error_message}", exc_info=True)
        else:
            if not page_id: app.logger.warning("No page_id returned from Notion, skipping vector upsert.")
            if not text_to_embed: app.logger.warning(f"No text content to embed for note {page_id}, skipping vector upsert.")
            vector_update_status = "skipped_no_content_or_id"

        return jsonify({
            "ok": True, "data": {"page_id": page_id, "url": page_url,
...
def create_audio_note_url_route():
    ...
    init_error = _init_clients_from_request_data(data) # Handles Notion init
    if init_error: return jsonify(init_error), 500
    ...
    notion_result = note_utils.process_audio_url_for_notion(
        audio_url=data['audio_url'], title=data['title'],
        notion_db_id=data.get('notion_db_id'),
        notion_source_text=data.get('notion_source', 'Audio URL Note'),
        linked_task_id=data.get('linked_task_id'), linked_event_id=data.get('linked_event_id'),
        deepgram_api_key=deepgram_api_key, openai_api_key=openai_api_key
    )
    ...
    if notion_result["status"] == "success":
        ...
        if not lancedb_uri:
            ...
            app.logger.warning(f"For audio note {page_id}: {vector_error_message}")
                ...
                if embedding_response["status"] == "success":
                    ...
                    try:
                        ...
                        if upsert_result["status"] == "success": vector_update_status = upsert_result.get("operation", "success")
                            ...
                            app.logger.error(f"Error upserting vector for audio note {page_id}: {vector_error_message}")
                    except ImportError:
                        vector_update_status = "failed"; vector_error_message = "LanceDB service module not found."
                        app.logger.error(f"Error for audio note {page_id}: {vector_error_message}")
                    except Exception as ldb_e:
                        vector_update_status = "failed"; vector_error_message = f"LanceDB error: {str(ldb_e)}"
                        app.logger.error(f"Error for audio note {page_id}: {vector_error_message}", exc_info=True)
                else:
                    vector_update_status = "failed"; vector_error_message = embedding_response.get("message")
                    app.logger.error(f"Error embedding audio note {page_id}: {vector_error_message}")
            except Exception as e:
                vector_update_status = "failed"; vector_error_message = f"Vector processing error: {str(e)}"
                app.logger.error(f"Error for audio note {page_id} during vector processing: {vector_error_message}", exc_info=True)
        else:
            if not page_id: app.logger.warning("No page_id for audio note, skipping vector upsert.")
            if not text_to_embed_for_audio: app.logger.warning(f"No text content to embed for audio note {page_id}.")
            vector_update_status = "skipped_no_content_or_id"

        return jsonify({
            "ok": True, "data": {"page_id": page_id, "url": page_url, "summary": summary, "key_points": key_points,
                                "vector_status": vector_update_status, "vector_message": vector_error_message}
        }), 201
    else:
...
def _ensure_notion_client(data: dict) -> dict | None:
    ...
    if not os.environ.get("DEEPGRAM_API_KEY"):
        app.logger.warning("DEEPGRAM_API_KEY is not set in the environment. Transcription may fail if not passed in request.")
    if not os.environ.get("OPENAI_API_KEY"):
        app.logger.warning("OPENAI_API_KEY is not set in the environment. Summarization/Embedding may fail if not passed in request.")
...
def process_recorded_audio_note_route():
    ...
    try:
        ...
        client_init_error = _ensure_notion_client({'notion_db_id': notion_db_id_override, 'notion_api_token': request.form.get('notion_api_token')})
        ...
        transcript_resp = note_utils.transcribe_audio_deepgram(audio_file_path=temp_audio_path) # Relies on env DEEPGRAM_API_KEY
        ...
        if transcript.strip():
            summarize_resp = note_utils.summarize_transcript_gpt(transcript=transcript) # Relies on env OPENAI_API_KEY
            if summarize_resp["status"] == "success":
                ...
                app.logger.warning(f"Summarization failed for transcript: {summarize_resp['message']}")
        else:
            summary = "No speech detected or transcript was empty."

        create_note_resp = note_utils.create_notion_note(
            title=title, content=f"Audio note recorded on {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            notion_db_id=notion_db_id_override, source="In-Person Audio Note via Agent",
            linked_event_id=linked_event_id, transcription=transcript, audio_file_link=audio_s3_url,
            summary=summary, key_points=key_points_str, decisions=decisions_list, action_items=action_items_list
        )
        ...
        if notion_page_id and transcript.strip():
            ...
            embedding_store_result = note_utils.embed_and_store_transcript_in_lancedb(
                notion_page_id=notion_page_id, transcript_text=transcript, meeting_title=title,
                meeting_date_iso=meeting_date_iso, user_id=user_id # Relies on env OPENAI_API_KEY & LANCEDB_URI
            )
            if embedding_store_result["status"] != "success":
                app.logger.warning(f"Failed to embed and store transcript in LanceDB: {embedding_store_result['message']}")

        return jsonify({
            "ok": True, "data": {"notion_page_id": notion_page_id, "notion_page_url": notion_page_url,
                                 "title": title, "summary_preview": summary[:200] if summary else "N/A",
                                 "transcript_preview": transcript[:200] if transcript else "N/A"}
        }), 201

    except Exception as e:
        app.logger.error(f"Unhandled exception in /api/process-recorded-audio-note: {e}", exc_info=True)
        raise
    finally:
        if temp_audio_path and os.path.exists(temp_audio_path):
            try: os.remove(temp_audio_path)
            except Exception as e_remove: app.logger.error(f"Error removing temporary audio file {temp_audio_path}: {e_remove}", exc_info=True)
        if temp_dir and os.path.exists(temp_dir):
            try: shutil.rmtree(temp_dir)
            except Exception as e_rmdir: app.logger.error(f"Error removing temporary directory {temp_dir}: {e_rmdir}", exc_info=True)
...
async def create_live_audio_note_route():
    ...
    required_params = ['meeting_id', 'title', 'platform_module_name', 'notion_api_token', 'deepgram_api_key', 'openai_api_key']
    missing_params = [param for param in required_params if param not in data]
    if missing_params:
        return jsonify({"ok": False, "error": {"code": "MISSING_PARAMETERS", "message": f"Missing parameters: {', '.join(missing_params)}"}}), 400
    ...
    if platform_module_name == "conceptual_zoom_agent_module": # pragma: no cover
        try: pass # Actual import would go here
        except ImportError: app.logger.warning(f"Could not import platform module '{platform_module_name}'.")

    if platform_module is None: # pragma: no cover
        class MockPlatformModule: # Basic mock for testing flow if module not found
            async def start_audio_capture(self, meeting_id_ignored):
                app.logger.info("MockPlatformModule: Simulating start_audio_capture with no audio data.")
                if False: yield b'' # Makes it an async generator
            async def stop_audio_capture(self): # Made async to match expected interface
                app.logger.info("MockPlatformModule: Simulating stop_audio_capture.")
        platform_module = MockPlatformModule()

    try:
        result = await note_utils.process_live_audio_for_notion(
            platform_module=platform_module, meeting_id=data['meeting_id'], notion_note_title=data['title'],
            deepgram_api_key=data['deepgram_api_key'], openai_api_key=data['openai_api_key'],
            notion_db_id=data.get('notion_db_id'), notion_source=data.get('notion_source', 'Live Meeting Transcription'),
            linked_task_id=data.get('linked_task_id'), linked_event_id=data.get('linked_event_id')
        )
        if result["status"] == "success":
        ...
        app.logger.error(f"Exception in /create-live-audio-note route: {e}", exc_info=True)
        raise
...
def search_similar_notes_route():
    ...
    query_text = data['query_text']; user_id = data['user_id']
    ...
    if not os.environ.get("OPENAI_API_KEY"): # Check env directly for server-side key
    ...
    embedding_response = note_utils.get_text_embedding_openai(text_to_embed=query_text) # Uses env OPENAI_API_KEY
    if embedding_response["status"] != "success":
        app.logger.error(f"Failed to get embedding for similarity search: {embedding_response.get('message')}")
    ...
    try:
        ...
        search_result = lancedb_service.search_similar_notes(
            db_path=lancedb_uri, query_vector=query_vector, user_id=user_id, limit=limit
        )
        if search_result["status"] == "success":
            return jsonify({"ok": True, "data": search_result.get("data", [])}), 200
        else:
            app.logger.error(f"LanceDB similarity search failed: {search_result.get('message')}")
...
if __name__ == '__main__': # pragma: no cover
    ...
    # Set up basic logging if running standalone for testing
    if not app.debug: # Only configure basic logging if not in debug mode (Flask might do it)
        # and if no handlers are already attached to root logger
        root_logger = app.logger.parent
