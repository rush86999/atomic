...
const getStoredUserMSTeamsTokens = async (
  userId: string
): Promise<{ refreshToken: string; account: AccountInfo } | null> => {
  logger.debug(
    `[MSTeamsService] Attempting to fetch stored MS Teams tokens for user ${userId}.`
  );
  ...
  try {
    ...
    logger.error(
      `[MSTeamsService] Error fetching MS Teams tokens for user ${userId}:`,
      error
    );
  }
  return null;
...
const storeUserMSTeamsTokens = async (
  userId: string,
  ...
  if (!authResult.account) {
    logger.error(
      '[MSTeamsService] Account info missing in AuthenticationResult. Cannot store tokens.'
    );
  ...
  try {
    ...
    logger.info(
      `[MSTeamsService] Tokens for user ${userId} stored/updated successfully.`
    );
  } catch (error) {
    logger.error(
      `[MSTeamsService] Error storing MS Teams tokens for user ${userId}:`,
      error
    );
  }
};
...
function initializeConfidentialClientApp(): ConfidentialClientApplication | null {
  ...
  if (
    ...
    logger.error(
      '[MSTeamsService] MS Graph API delegated client credentials not fully configured.'
    );
    return null;
  }
  if (!MSTEAMS_TOKEN_ENCRYPTION_KEY) {
    ...
    logger.error(
      '[MSTeamsService] MSTEAMS_TOKEN_ENCRYPTION_KEY is not configured. Cannot proceed with token operations.'
    );
  ...
  logger.info(
    '[MSTeamsService] MSAL ConfidentialClientApplication initialized for delegated flow.'
  );
  return msalConfidentialClientApp;
}
...
export async function getDelegatedMSGraphTokenForUser(
  userId: string
): Promise<GraphSkillResponse<MSGraphUserTokenDetails>> {
  ...
  if (!MSTEAMS_TOKEN_ENCRYPTION_KEY) {
    logger.error(
      '[MSTeamsService] MSTEAMS_TOKEN_ENCRYPTION_KEY is not configured. Cannot get/refresh tokens.'
    );
    return {
      ok: false,
      error: {
  ...
  if (
    ...
    logger.warn(
      `[MSTeamsService] No stored MS Teams refresh token or account info for user ${userId}. User needs to authenticate.`
    );
    return {
      ok: false,
      error: {
        ...
        message:
          'User authentication required for MS Teams (no refresh token or account info).',
      },
    };
  }
  ...
  try {
    ...
    try {
      logger.debug(
        `[MSTeamsService] Attempting to acquire token silently for user ${userId} with account ${storedTokenInfo.account.homeAccountId}`
      );
      authResult = await clientApp.acquireTokenSilent(silentRequest);
    } catch (error: any) {
      logger.warn(
        `[MSTeamsService] Silent token acquisition failed for user ${userId} (Error: ${error.errorCode || error.name}). Attempting refresh via stored refresh token if applicable.`
      );
      ...
      if (
        ...
        (error.errorCode === 'no_tokens_found' ||
          ...
          error.message?.includes(
            ' Při pokusu o získání tokenu z mezipaměti nebyl nalezen žádný platný přístupový token.'
          ))
      ) {
        // The last error message is an example of a localized error that might indicate no valid AT in cache.
        try {
          logger.info(
            `[MSTeamsService] Attempting acquireTokenByRefreshToken for user ${userId}.`
          );
          ...
          logger.error(
            `[MSTeamsService] Failed to acquire token by refresh token for user ${userId}:`,
            refreshError
          );
          // If refresh token itself is invalid/expired, user must re-authenticate.
          if (
            refreshError.name === 'InteractionRequiredAuthError' ||
            ...
            return {
              ...
              error: {
                ...
                message:
                  'Refresh token invalid or expired. User must re-authenticate with Microsoft Teams.',
              },
            };
          }
          return {
    ...
    if (authResult && authResult.accessToken) {
      ...
      logger.info(
        `[MSTeamsService] Successfully acquired/refreshed token for user ${userId}.`
      );
      ...
      if (authResult.account) {
        ...
        logger.info(
          `[MSTeamsService] Extracted AAD OID: ${userAadObjectId}, UPN: ${userPrincipalName} from MSAL AccountInfo for user ${userId}.`
        );
      ...
      logger.warn(
        `[MSTeamsService] Token acquisition yielded null/empty accessToken for user ${userId} without throwing. This is unexpected.`
      );
      return {
        ok: false,
        error: {
    ...
    logger.error(
      `[MSTeamsService] General error in getDelegatedMSGraphTokenForUser for ${userId}:`,
      error
    );
    if (
      ...
      return {
        ...
        error: {
          code: 'MSGRAPH_INTERACTION_REQUIRED',
          message:
            'User interaction required. Please re-authenticate with Microsoft Teams.',
        },
      };
    }
    return {
...
export async function generateTeamsAuthUrl(
  ...
  if (!clientApp) {
    logger.error(
      '[MSTeamsService] MSAL client not initialized, cannot generate auth URL.'
    );
    return null;
  }
  ...
  if (!MSGRAPH_DELEGATED_SCOPES || MSGRAPH_DELEGATED_SCOPES.length === 0) {
    logger.error(
      '[MSTeamsService] MSGRAPH_DELEGATED_SCOPES are not configured.'
    );
    return null;
  }
  if (!MSGRAPH_DELEGATED_REDIRECT_URL) {
    logger.error(
      '[MSTeamsService] MSGRAPH_DELEGATED_REDIRECT_URL is not configured.'
    );
    return null;
  }
  ...
  try {
    ...
    logger.info(
      `[MSTeamsService] Generated MS Teams auth URL for context user: ${userIdForContext}.`
    );
    return authUrl;
  } catch (error: any) {
    logger.error(
      `[MSTeamsService] Error generating MS Teams auth URL for context user ${userIdForContext}:`,
      error
    );
    return null;
  }
...
export async function handleTeamsAuthCallback(
...
): Promise<GraphSkillResponse<boolean>> {
  const clientApp = initializeConfidentialClientApp(); // Correctly use the confidential client app
  if (!clientApp) {
    logger.error(
      '[MSTeamsService] MSAL client not initialized, cannot handle auth callback.'
    );
    return {
      ok: false,
      error: {
  ...
  if (!MSTEAMS_TOKEN_ENCRYPTION_KEY) {
    logger.error(
      '[MSTeamsService] MSTEAMS_TOKEN_ENCRYPTION_KEY is not configured. Cannot store tokens post callback.'
    );
    return {
      ok: false,
      error: {
  ...
  if (!MSGRAPH_DELEGATED_SCOPES || MSGRAPH_DELEGATED_SCOPES.length === 0) {
    logger.error(
      '[MSTeamsService] MSGRAPH_DELEGATED_SCOPES are not configured for callback token request.'
    );
    return {
      ok: false,
      error: {
  ...
  if (!MSGRAPH_DELEGATED_REDIRECT_URL) {
    logger.error(
      '[MSTeamsService] MSGRAPH_DELEGATED_REDIRECT_URL is not configured for callback token request.'
    );
    return {
      ok: false,
      error: {
  ...
  try {
    ...
    if (authResult && authResult.accessToken && authResult.account) {
      ...
      logger.info(
        `[MSTeamsService] Successfully handled MS Teams auth callback and initiated token storage for user ${userId}.`
      );
      return { ok: true, data: true };
    } else {
      logger.error(
        `[MSTeamsService] Failed to acquire token by code for user ${userId}. AuthResult invalid or account missing.`,
        authResult
      );
      return {
        ok: false,
        error: {
          ...
          message:
            'Could not process auth code or vital token/account info missing.',
        },
      };
    }
  } catch (error: any) {
    logger.error(
      `[MSTeamsService] Error in handleTeamsAuthCallback for user ${userId}:`,
      error
    );
...
export async function listMicrosoftTeamsMeetings(
  ...
  logger.debug(
    `[MSTeamsService] listMicrosoftTeamsMeetings called for Atom user: ${atomUserId}, options:`,
    options
  );
  ...
  if (!requestUrl) {
    const selectParams =
      'id,subject,start,end,isOnlineMeeting,onlineMeeting,webLink,bodyPreview';
  ...
  try {
    logger.debug(
      `[MSTeamsService] Requesting meetings from MS Graph: ${requestUrl}`
    );
    ...
    logger.error(
      `[MSTeamsService] Error listing MS Teams meetings for Atom user ${atomUserId}:`,
      axiosError.message,
...
export async function getMicrosoftTeamsMeetingDetails(
  ...
  logger.debug(
    `[MSTeamsService] getMicrosoftTeamsMeetingDetails called for Atom user: ${atomUserId}, eventId: ${eventId}`
  );
  ...
  const selectParams =
    'id,subject,start,end,isOnlineMeeting,onlineMeeting,webLink,bodyPreview,body,attendees,location,locations,organizer';
  ...
  try {
    logger.debug(
      `[MSTeamsService] Requesting meeting details from MS Graph: ${requestUrl}`
    );
    ...
    const axiosError = error as AxiosError;
    logger.error(
      `[MSTeamsService] Error getting MS Teams meeting details for event ${eventId}, Atom user ${atomUserId}:`,
      axiosError.message,
      axiosError.response?.data
    );
    const errorData = axiosError.response?.data as any;
    if (axiosError.response?.status === 404) {
      return {
        ...
        error: {
          ...
          message:
            `Meeting event not found (ID: ${eventId}). Details: ${errorData?.error?.message || ''}`.trim(),
        },
      };
    }
    return {
...
export async function searchTeamsMessages(
  ...
  logger.debug(
    `[MSTeamsService] searchTeamsMessages called for Atom user ${atomUserId}, query: "${searchQuery}", maxResults: ${maxResults}, userAadObjectId: ${userAadObjectId}`
  );
  ...
  try {
    ...
    logger.info(
      `[MSTeamsService] Executing Graph Search API with KQL: "${searchQuery}" for user ${atomUserId}`
    );
    ...
    logger.info(
      `[MSTeamsService] Found ${messages.length} Teams messages for query "${searchQuery}" for Atom user ${atomUserId}.`
    );
    return { ok: true, data: messages };
  } catch (error: any) {
    const axiosError = error as AxiosError;
    logger.error(
      `[MSTeamsService] Error searching Teams messages for Atom user ${atomUserId}:`,
      axiosError.message,
      axiosError.response?.data
    );
    const errorData = axiosError.response?.data as any;
...
export async function getTeamsChatMessageContent(
  ...
  logger.debug(
    `[MSTeamsService] getTeamsChatMessageContent for user ${atomUserId}, chatId: ${chatId}, messageId: ${messageId}`
  );
  const tokenResponse = await getDelegatedMSGraphTokenForUser(atomUserId);
  if (!tokenResponse.ok || !tokenResponse.data)
  ...
  const selectFields =
    'id,replyToId,from,body,createdDateTime,lastModifiedDateTime,webUrl,attachments,mentions,chatId';
  ...
  try {
    ...
    const axiosError = error as AxiosError;
    logger.error(
      `[MSTeamsService] Error getting chat message content (chat ${chatId}, msg ${messageId}):`,
      axiosError.message,
      axiosError.response?.data
    );
    const errorData = axiosError.response?.data as any;
    ...
    return {
      ...
      error: {
        code: errorData?.error?.code || 'MSGRAPH_API_ERROR',
        message:
          errorData?.error?.message || 'Failed to get chat message content',
      },
    };
  }
}
...
export async function getTeamsChannelMessageContent(
  ...
  logger.debug(
    `[MSTeamsService] getTeamsChannelMessageContent for user ${atomUserId}, team ${teamId}, channel ${channelId}, msg ${messageId}`
  );
  const tokenResponse = await getDelegatedMSGraphTokenForUser(atomUserId);
  if (!tokenResponse.ok || !tokenResponse.data)
    return { ok: false, error: tokenResponse.error };
  ...
  const selectFields =
    'id,replyToId,from,body,createdDateTime,lastModifiedDateTime,webUrl,attachments,mentions,channelIdentity';
  ...
  try {
    ...
    const axiosError = error as AxiosError;
    logger.error(
      `[MSTeamsService] Error getting channel message content (team ${teamId}, channel ${channelId}, msg ${messageId}):`,
      axiosError.message,
      axiosError.response?.data
    );
    const errorData = axiosError.response?.data as any;
    ...
    return {
      ...
      error: {
        code: errorData?.error?.code || 'MSGRAPH_API_ERROR',
        message:
          errorData?.error?.message || 'Failed to get channel message content',
      },
    };
  }
}
