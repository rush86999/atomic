...
import { googleAuthRedirectUri, googleClientIdAtomicWeb, googleClientIdIos, googleClientIdWeb, googleClientSecretAtomicWeb, googleClientSecretWeb, googleTokenUrl, googleClientIdGmail, googleClientSecretGmail, googleClientIdMcp, googleClientSecretMcp } from './constants' // Added Mcp constants
dayjs.extend(utc)
...
export const getGmailUserTokens = async (code: string, redirectUri: string): Promise<google.auth.Credentials> => {
    try {
        const oauth2Client = new google.auth.OAuth2(
...
export const refreshGmailAccessToken = async (refreshToken: string): Promise<RefreshTokenResponseBodyType> => {
    try {
        const res: RefreshTokenResponseBodyType = await got.post(
            googleTokenUrl,
            {
...
export const getMcpUserTokens = async (code: string, redirectUri: string): Promise<google.auth.Credentials> => {
    try {
        const oauth2Client = new google.auth.OAuth2(
...
export const refreshMcpAccessToken = async (refreshToken: string): Promise<RefreshTokenResponseBodyType> => {
    try {
        const res: RefreshTokenResponseBodyType = await got.post(
            googleTokenUrl,
            {
