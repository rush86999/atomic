...
import { AddDailyFeaturesApplyEventTriggerType, AutopilotApiResponse, SkillError } from '@autopilot/_libs/types'; // Added AutopilotApiResponse, SkillError
...
const handler = async (req: Request, res: Response<AutopilotApiResponse<SuccessPayload | null>>) => {
    try {
        ...
        if (!eventItem?.body?.windowStartDate) validationErrors.push('Missing body.windowStartDate.');
        if (!eventItem?.body?.windowEndDate) validationErrors.push('Missing body.windowEndDate.');
        if (!eventItem?.body?.timezone) validationErrors.push('Missing body.timezone.');
        // Autopilot specific validations
        if (!eventItem?.autopilot?.userId) validationErrors.push('Missing autopilot.userId.'); // Corrected error message from "no timezone present"
        if (!eventItem?.autopilot?.scheduleAt) validationErrors.push('Missing autopilot.scheduleAt.');
        if (!eventItem?.autopilot?.timezone) validationErrors.push('Missing autopilot.timezone.');
        if (!eventItem?.autopilot?.payload) validationErrors.push('Missing autopilot.payload.');
        ...
        return res.status(500).json({
            ...
            error: {
                ...
                message: e.message || 'An unexpected error occurred while adding daily features.',
                details: e.toString()
            }
        });
    }
