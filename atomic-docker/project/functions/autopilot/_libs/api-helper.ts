...
import {
  ...
  SkillError,
} from './types'; // Added AutopilotApiResponse, SkillError
import {
  authApiToken,
  featuresApplyAdminUrl,
  hasuraAdminSecret,
  hasuraGraphUrl,
  hasuraMetadataUrl,
  onScheduledTriggerDailyFeaturesApplyWebhookProdAdminUrl,
  onScheduledTriggerDailyScheduleAssistWebhookProdAdminUrl,
  scheduleAssistUrl,
} from './constants';
...
export const getUserPreferences = async (
  userId: string
): Promise<AutopilotApiResponse<UserPreferenceType | null>> => {
  if (!userId) {
    return {
      ok: false,
      error: {
...
export const createDailyFeaturesApplyEventTrigger = async (
  ...
  try {
    ...
    if (res && res.message === 'success' && res.event_id) {
      console.log(
        `Successfully created scheduled event trigger: ${res.event_id}`
      );
      return { ok: true, data: res.event_id };
    } else {
      console.error(
        'Failed to create Hasura scheduled event, response was not success or event_id missing:',
        res
      );
      return {
        ok: false,
...
export const upsertAutopilotOne = async (
  ...
  try {
    ...
    const res: { data: { insert_Autopilot_one: AutopilotType } } = await got
      .post(hasuraGraphUrl, {
        headers: {
          'X-Hasura-Admin-Secret': hasuraAdminSecret,
          'Content-Type': 'application/json',
...
export const createInitialFeaturesApplyToEventTrigger = async (
  ...
  try {
    ...
    if (!userPreferencesResponse.ok || !userPreferencesResponse.data) {
      throw new Error(
        `Failed to get user preferences: ${userPreferencesResponse.error?.message || 'Unknown error'}`
      );
    }
    ...
    const dayOfWeekInt = getISODay(
      dayjs(oldBody.windowEndDate.slice(0, 19))
    ...
    if (!applicableStartTime) {
      throw new Error(
        `No applicable start time found for dayOfWeekInt ${dayOfWeekInt}.`
      );
    }
    const startHour = applicableStartTime.hour;
    const randomStartHour = getRandomIntInclusive(
      0,
      startHour > 0 ? startHour - 1 : 0
    ...
    const triggerResponse = await createDailyFeaturesApplyEventTrigger(
      newAutopilot,
      newBody
    );
    if (!triggerResponse.ok || !triggerResponse.data) {
      throw new Error(
        `Failed to create daily features trigger: ${triggerResponse.error?.message || 'Unknown error'}`
      );
    }
    ...
    if (!upsertResponse.ok) {
      ...
      throw new Error(
        `Failed to upsert autopilot data after creating trigger: ${upsertResponse.error?.message || 'Unknown error'}`
      );
    }
...
export const listAutopilotsGivenUserId = async (
  ...
  try {
    ...
    const res: { data: { Autopilot: AutopilotType[] } } = await got
      .post(hasuraGraphUrl, {
        headers: {
          'X-Hasura-Admin-Secret': hasuraAdminSecret,
          'Content-Type': 'application/json',
...
export const deleteAutopilotGivenId = async (
  ...
  try {
    ...
    const res: { data: { delete_Autopilot_by_pk: AutopilotType } } = await got
      .post(hasuraGraphUrl, {
        headers: {
          'X-Hasura-Admin-Secret': hasuraAdminSecret,
          'Content-Type': 'application/json',
...
export const onScheduleDailyFeaturesApply7DayWindowToEventTrigger = async (
  ...
  try {
    const dbAutopilotResponse = await listAutopilotsGivenUserId(
      oldAutopilot.userId
    );
    if (!dbAutopilotResponse.ok) {
      // Error fetching, critical
      throw new Error(
        `Failed to verify existing autopilot: ${dbAutopilotResponse.error?.message}`
      );
    }
    if (!dbAutopilotResponse.data?.id) {
      // Autopilot was deleted or not found
      console.log(
        `Autopilot for user ${oldAutopilot.userId} seems to have been deleted. Halting trigger.`
      );
    ...
    if (!triggerFeaturesResponse.ok) {
      throw new Error(
        `Failed to trigger features apply URL: ${triggerFeaturesResponse.error?.message}`
      );
    }
    ...
    if (!userPreferencesResponse.ok || !userPreferencesResponse.data) {
      throw new Error(
        `Failed to get user preferences: ${userPreferencesResponse.error?.message}`
      );
    }
    const userPreferences = userPreferencesResponse.data;
    ...
    if (!applicableStartTime) {
      throw new Error(
        `No applicable start time found for dayOfWeekInt ${dayOfWeekInt} for user ${oldBody.userId}.`
      );
    }
    const startHour = applicableStartTime.hour;
    const randomStartHour = getRandomIntInclusive(
    ...
    const createTriggerResponse = await createDailyFeaturesApplyEventTrigger(
      newAutopilot,
      newBody
    );
    if (!createTriggerResponse.ok || !createTriggerResponse.data) {
      throw new Error(
        `Failed to create new daily features trigger: ${createTriggerResponse.error?.message}`
      );
    }
    ...
    if (!deleteResponse.ok) {
      ...
      console.warn(
        `Failed to delete old autopilot ${dbAutopilot.id}: ${deleteResponse.error?.message}. New trigger ${newAutopilot.id} is already scheduled.`
      );
    }
    ...
    if (!upsertResponse.ok) {
      throw new Error(
        `Failed to upsert new autopilot ${newAutopilot.id}: ${upsertResponse.error?.message}`
      );
    }
    return { ok: true, data: undefined };
  } catch (e: any) {
    console.error(
      'Error in onScheduleDailyFeaturesApply7DayWindowToEventTrigger:',
      e
    );
    return {
      ok: false,
...
export const upsertAutopilotMany = async (
  ...
  try {
    ...
    const res: { data: { insert_Autopilot: AutopilotType[] } } = await got
      .post(hasuraGraphUrl, {
        headers: {
          'X-Hasura-Admin-Secret': hasuraAdminSecret,
          'Content-Type': 'application/json',
...
export const getAutopilotGivenId = async (
  ...
  try {
    ...
    const res: { data: { Autopilot_by_pk: AutopilotType } } = await got
      .post(hasuraGraphUrl, {
        headers: {
          'X-Hasura-Admin-Secret': hasuraAdminSecret,
          'Content-Type': 'application/json',
