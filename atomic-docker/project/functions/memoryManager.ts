...
if (openaiApiKey) {
  ...
  console.error(
    '[MemoryManager] OPENAI_API_KEY not found in environment variables. Real embeddings will not be generated by TS side MemoryManager.'
  );
}
...
export async function generateEmbedding(
  ...
  if (!openaiClient) {
    log(
      '[MemoryManager] OpenAI client not initialized. Cannot generate real embedding. Returning null.',
      'error'
    );
    return null;
  }
  if (!text || text.trim() === '') {
    log(
      '[MemoryManager] generateEmbedding called with empty or whitespace-only text. Returning null.',
      'warn'
    );
    return null;
  }
  ...
  log(
    `Generating OpenAI embedding for text (first 50 chars): "${trimmedText.substring(0, 50)}..."`
  );
  ...
  try {
    ...
    if (
      ...
      log(
        `Successfully generated OpenAI embedding. Dimension: ${response.data[0].embedding.length}`
      );
      if (response.data[0].embedding.length !== DEFAULT_VECTOR_DIMENSION) {
        log(
          `[MemoryManager] WARNING: OpenAI embedding dimension (${response.data[0].embedding.length}) does not match DEFAULT_VECTOR_DIMENSION (${DEFAULT_VECTOR_DIMENSION}). Check model and config.`,
          'warn'
        );
      }
      ...
      log(
        '[MemoryManager] OpenAI embedding response is missing expected data.',
        'error',
        response
      );
      return null;
 ...
 */
export async function processSTMToLTM(
  userId: string,
  ...
  if (recentTurns.length > 0) {
    summary += recentTurns
      .map(
        (turn) =>
          `User: ${turn.userInput} | Agent: ${JSON.stringify(turn.agentResponse).substring(0, 150)}...`
      )
      .join('; ');
  }
  ...
  try {
    ...
    if (!embedding) {
      log(
        'Failed to generate embedding for STM summary. Skipping LTM storage for this cycle.',
        'error'
      );
    ...
    log(
      'Successfully processed and stored STM snapshot to LTM knowledge_base.'
    );
  } catch (error) {
    log(
      'Error during STM to LTM processing or LanceDB operation.',
      'error',
      error
    );
...
export async function retrieveRelevantLTM(
  ...
  log(
    `Retrieving relevant LTM for query: "${queryText}" (User: ${userId || 'N/A'})`,
    options
  );
  ...
  try {
    ...
    if (!queryEmbedding) {
      log(
        'Failed to generate query embedding. Cannot retrieve from LTM.',
        'error'
      );
      return [];
    }
    ...
    if (targetTable === 'user_profiles') {
      ...
      if (userId) {
        ...
        log(
          'Warning: Searching user_profiles without a specific userId.',
          'info'
        );
      }
    ...
    log(
      `Querying table '${targetTable}' with vector column '${vectorColumnName}', limit ${fetchTopK}, filter: '${combinedFilter || 'None'}'`
    );

    const rawSearchResults = await searchTable(
      db,
      targetTable,
      queryEmbedding,
      fetchTopK,
    ...
    log(
      `Retrieved ${rawSearchResults.length} raw results from LTM table '${targetTable}'.`
    );
    ...
    if (options?.boostRecency && finalResults.length > 0) {
      ...
      const similarityWeight =
        options.similarityWeight !== undefined ? options.similarityWeight : 0.7;
      const recencyWeight =
        options.recencyWeight !== undefined ? options.recencyWeight : 0.3;
      ...
      log(
        `Re-ranking complete. Top score after re-ranking: ${finalResults[0]?.score}`
      );
    } else {
...
export interface ConversationStateActions {
  // Export if used by handler.ts directly
...
export async function loadLTMToSTM(
  ...
  log(
    `LTM context updated in conversation state with ${results.length} items.`
  );
  ...
  if (
    ...
    log(
      `Updating user goal from top LTM result's summaryGoal: ${topResult.metadata.summaryGoal}`
    );
    conversationStateActions.updateUserGoal(
      topResult.metadata.summaryGoal as string
    );
  }
