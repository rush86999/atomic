...
interface InternalHandleMessageOptions {
  ...
  ) => Promise<boolean>; // The actual function to send a command
}
...
import {
  handleSearchGmail,
  handleExtractInfoFromGmail,
  SearchGmailNluEntities,
  ExtractInfoFromGmailNluEntities,
} from './skills/gmailSkills';
// Import new Slack Query skills
import {
  handleSearchSlackMessages,
  handleExtractInfoFromSlackMessage,
  SearchSlackMessagesNluEntities,
  ExtractInfoFromSlackMessageNluEntities,
} from './skills/slackQuerySkills';
// Import new MS Teams Query skills
import {
  handleSearchMSTeamsMessages,
  handleExtractInfoFromMSTeamsMessage,
  SearchMSTeamsMessagesNluEntities,
  ExtractInfoFromMSTeamsMessageNluEntities,
} from './skills/msTeamsQuerySkills';
import {
  retrieveRelevantLTM,
  loadLTMToSTM,
  processSTMToLTM,
...
import {
  handlePrepareForMeeting,
  handleGenerateWeeklyDigest,
  handleSuggestFollowUps,
} from './skills/productivitySkills'; // Added for Smart Meeting Prep, Weekly Digest & Follow-up Suggester
...
import {
  handleGetHubSpotContactByEmail,
  handleCreateHubSpotContact,
} from './skills/hubspot';
import { handleSendSlackMessage, handleSlackMyAgenda } from './skills/slack';
import {
  handleListCalendlyEventTypes,
  handleListCalendlyScheduledEvents,
} from './skills/calendly';
import {
  handleListZoomMeetings,
  handleGetZoomMeetingDetails,
} from './skills/zoom';
import {
  handleListGoogleMeetEvents,
  handleGetGoogleMeetEventDetails,
} from './skills/googleMeet';
import {
  handleListMicrosoftTeamsMeetings,
  handleGetMicrosoftTeamsMeetingDetails,
} from './skills/msteams';
import {
...
} from './skills/quickbooks';
import { handleMeetingPrep } from './skills/meetingPrep';
import {
  handleCreateAsanaTask,
  handleQueryAsanaTasks,
  handleUpdateAsanaTask,
} from './skills/asana';
import {
  handleCreateJiraIssue,
  handleQueryJiraIssues,
  handleUpdateJiraIssue,
} from './skills/jira';
import {
  handleCreateTrelloCard,
  handleQueryTrelloCards,
  handleUpdateTrelloCard,
} from './skills/trello';
...
async function initializeLtmDatabase(): Promise<void> {
  try {
    ...
    if (ltmDbConnection) {
      ...
      console.error(
        '[Handler] LanceDB LTM connection failed to initialize (returned null).'
      );
    }
  } catch (error) {
    console.error(
      '[Handler] Error initializing LanceDB LTM connection:',
      error
    );
...
async function _internalHandleMessage(
  ...
  const agentSkillContext: AgentSkillContext = {
    userId: userId,
    sendCommandToClient:
      options?.sendCommandToClientFunction ||
  ...
  if (requestSource === 'ScheduledJobExecutor' && directIntentName) {
    console.log(
      `[InternalHandleMessage][${interfaceType}] Received direct execution request from ScheduledJobExecutor. ConversationID: ${conversationId}`
    );
    nluResponse = {
    ...
    console.log(
      `[InternalHandleMessage][${interfaceType}] Bypassing NLU for scheduled task. Intent: ${nluResponse.intent}, Entities: ${JSON.stringify(nluResponse.entities)}`
    );
  } else {
    if (ltmDbConnection) {
      try {
        const relevantLtm = await retrieveRelevantLTM(
          message,
          userId,
          ltmDbConnection,
          { table: LANCEDB_LTM_AGENT_TABLE_NAME }
        );
        if (relevantLtm && relevantLtm.length > 0) {
          console.log(
            `[Handler][${interfaceType}] Retrieved ${relevantLtm.length} items from LTM for query: ${message}. Storing in conversation state.`
          );
          ...
          const conversationStateActions: ConversationStateActions = {
            updateUserGoal: (goal) =>
              conversationManager.updateUserGoal(interfaceType, goal),
            updateIntentAndEntities: (intent, entities) =>
              conversationManager.updateIntentAndEntities(
                interfaceType,
                intent,
                entities
              ),
            updateLtmRepoContext: (context) =>
              conversationManager.updateLTMContext(interfaceType, context),
          };
        ...
        console.error(
          `[Handler][${interfaceType}] Error retrieving or loading LTM:`,
          error
        );
        conversationManager.updateLTMContext(interfaceType, null);
        conversationLtmContext = null;
      ...
      console.warn(
        `[Handler][${interfaceType}] LTM DB connection not available, skipping LTM retrieval.`
      );
      conversationManager.updateLTMContext(interfaceType, null);
      conversationLtmContext = null;
    }

    nluResponse = await understandMessage(
      message,
      undefined,
      conversationLtmContext,
      llm
    );
    console.log(
      `[InternalHandleMessage][${interfaceType}] NLU Response (with LTM context consideration):`,
  ...
  if (nluResponse.error && !nluResponse.intent) {
    ...
    console.error(
      `[InternalHandleMessage][${interfaceType}] NLU service critical error or no intent from scheduled task:`,
      nluResponse.error
    );
    textResponse =
      "Sorry, I'm having trouble understanding requests right now or processing the scheduled task. Please try again later.";
    const currentConvStateOnError =
      conversationManager.getConversationStateSnapshot(interfaceType);
    ...
    if (ltmContextForErrorResponse && ltmContextForErrorResponse.length > 0) {
      ...
      if (firstLtmItem && firstLtmItem.text) {
        ...
        console.log(
          `[Handler][${interfaceType}] Augmented error response with LTM context: ${ltmPreamble}`
        );
      }
    }
    return {
  ...
  if (nluResponse.intent) {
    ...
    console.log(
      `[Handler][${interfaceType}] Intent: ${nluResponse.intent}, Entities: ${JSON.stringify(entities)}`
    );

    try {
      switch (nluResponse.intent) {
        case 'GetCalendarEvents':
          textResponse = await handleGetCalendarEvents(
            userId,
            entities,
            agentSkillContext.settings.integrations
          );
          break;
        // ... (other cases)
        default:
      ...
      console.error(
        `[Handler][${interfaceType}] Error executing intent "${nluResponse.intent}":`,
        error.message,
        error.stack
      );
...
- And other general commands like "list emails", "read email <id>", "send email {JSON}", "search web <query>", "trigger zap <name> [with data {JSON}]".`;
  ...
  console.log(
    `[Handler][${interfaceType}] Future enhancement: Implement relevance check for LTM items before augmenting response.`
  );
  ...
  if (ltmContextForResponse && ltmContextForResponse.length > 0) {
    ...
    if (firstLtmItem && firstLtmItem.text) {
      ...
      textResponse = ltmPreamble + textResponse;
      console.log(
        `[Handler][${interfaceType}] Augmented response with LTM context: ${ltmPreamble}`
      );
    }
  }
 ...
 */
export async function activateConversationWrapper(): Promise<{
  status: string;
  active: boolean;
  message?: string;
}> {
  console.log(
    `[Handler][voice] Received request to activate conversation (assumed for voice).`
  );
 ...
 */
export async function deactivateConversationWrapper(
  reason: string = 'manual_deactivation'
): Promise<{ status: string; active: boolean; message?: string }> {
  console.log(
    `[Handler][voice] Received request to deactivate conversation. Reason: ${reason}`
  );
 ...
 */
export async function handleInterruptWrapper(): Promise<{
  status: string;
  message: string;
}> {
  ...
  return {
    status: 'success',
    message:
      'Interrupt signal processed for voice interface. Agent responding state set to false.',
  };
}
...
export async function handleConversationInputWrapper(
  ...
  HandleMessageResponse | { error: string; active: boolean; message?: string }
> {
  const interfaceType: InterfaceType = 'voice'; // This wrapper is for voice interactions
  const { text } = payload;
  console.log(
    `[Handler][${interfaceType}] Received conversation input: "${text}"`
  );
  ...
  if (!conversationManager.isConversationActive(interfaceType)) {
    console.log(
      `[Handler][${interfaceType}] Conversation is not active. Ignoring input.`
    );
  ...
  if (conversationManager.checkIfAgentIsResponding(interfaceType)) {
    console.warn(
      `[Handler][${interfaceType}] New input received while agent was still marked as responding.`
    );
  ...
  console.log(
    `[Handler][${interfaceType}] Conversation active. Processing message...`
  );
  ...
  const {
    ...
    {
      sendCommandToClientFunction:
        optionsFromCaller?.sendCommandToClientFunction,
  ...
  conversationManager.recordAgentResponse(
    interfaceType,
    text,
    { text: coreResponseText },
    currentTurnIntent,
  ...
  if (ltmDbConnection) {
    try {
      ...
      processSTMToLTM(userId, currentConversationState, ltmDbConnection)
        .then(() =>
          console.log(
            `[Handler][${interfaceType}] STM to LTM processing initiated.`
          )
        )
        .catch((err) =>
          console.error(
            `[Handler][${interfaceType}] Error in STM to LTM processing:`,
            err
          )
        );
    } catch (error) {
      console.error(
        `[Handler][${interfaceType}] Error initiating STM to LTM processing:`,
        error
      );
    }
  } else {
    console.warn(
      '[Handler] LTM DB connection not available, skipping STM to LTM processing.'
    );
  }
  ...
  try {
    ...
    if (ttsResponse.ok) {
      ...
      if (ttsResult.audio_url) {
        console.log(
          `[Handler][${interfaceType}] TTS synthesis successful. Audio URL generated.`
        );
        ...
        console.error(
          `[Handler][${interfaceType}] TTS response OK, but no audio_url:`,
          ttsResult
        );
      ...
      console.error(
        `[Handler][${interfaceType}] TTS request failed:`,
        ttsResponse.status,
        errorBody
      );
      conversationManager.setAgentResponding(interfaceType, false); // Agent finished processing (with error)
    ...
    console.error(
      `[Handler][${interfaceType}] Error calling TTS service:`,
      error.message,
      error.stack
    );
    conversationManager.setAgentResponding(interfaceType, false); // Agent finished processing (with error)
...
export async function handleMessage(
  ...
  conversationManager.deactivateConversation(
    interfaceType,
    'completed_text_interaction'
  );
...
export async function testActivateConversation() {
  ...
  return {
    status:
      'Conversation manually activated for testing (voice interface) via _test_setConversationActive.',
...
export async function testDeactivateConversation() {
  ...
  return {
    status:
      'Conversation manually deactivated for testing (voice interface) via _test_setConversationActive.',
    active: conversationManager.isConversationActive('voice'),
    agentResponding: conversationManager.checkIfAgentIsResponding('voice'),
  };
}
...
export async function getConversationStatus() {
  ...
  return {
    ...
    voice_agentResponding:
      conversationManager.checkIfAgentIsResponding('voice'),
