...
export async function handleUpdateTaskRequest(
  ...
  logger.info(
    `[UpdateNotionTaskCmdHandler] Handling request for user ${userId} to update task.`
  );
  logger.debug(
    `[UpdateNotionTaskCmdHandler] Received NLU entities: ${JSON.stringify(entities)}`
  );
  ...
  if (
    entities.update_action === 'complete' &&
    entities.new_status_text?.toLowerCase() !== 'done'
  ) {
    logger.info(
      `[UpdateNotionTaskCmdHandler] 'complete' action detected, overriding status to 'Done'.`
    );
  ...
  try {
    const skillResponse: SkillResponse<
      UpdateTaskData & { userMessage: string }
    > = await handleUpdateNotionTask(userId, {
    ...
    if (skillResponse.ok && skillResponse.data?.userMessage) {
      logger.info(
        `[UpdateNotionTaskCmdHandler] Task update successful: ${skillResponse.data.userMessage}`
      );
      ...
      if (
        skillResponse.error?.code === 'AMBIGUOUS_TASK' &&
        skillResponse.data?.userMessage
      ) {
        logger.warn(
          `[UpdateNotionTaskCmdHandler] Task update ambiguous: ${skillResponse.data.userMessage}`
        );
        return skillResponse.data.userMessage;
      }
      const errorMsg =
      ...
      logger.error(
        `[UpdateNotionTaskCmdHandler] Skill execution failed or task update unsuccessful: ${errorMsg}`,
        skillResponse.error
      );
    ...
    logger.error(
      `[UpdateNotionTaskCmdHandler] Critical error handling request: ${error.message}`,
      error
    );
