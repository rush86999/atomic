...
export async function handleGetDailyBriefingRequest(
  ...
  logger.info(
    `[DailyBriefingCommandHandler] Handling request for user ${userId} for briefing: "${dateContext}"`
  );
  logger.debug(
    `[DailyBriefingCommandHandler] Received NLU entities: ${JSON.stringify(entities)}`
  );

  try {
    const skillResponse: GetDailyPriorityBriefingSkillResponse =
      await generateDailyBriefing(userId, entities);

    if (skillResponse.ok && skillResponse.data) {
      const briefing = skillResponse.data;
      logger.info(
        `[DailyBriefingCommandHandler] Skill executed successfully for "${dateContext}". Found ${briefing.priority_items.length} items.`
      );
      ...
      logger.debug(
        `[DailyBriefingCommandHandler] Sending structured message to client.`
      );
      return structuredMessage;
    } else {
      logger.error(
        `[DailyBriefingCommandHandler] Skill execution failed for "${dateContext}": ${skillResponse.error?.message}`,
        skillResponse.error
      );
    ...
    logger.error(
      `[DailyBriefingCommandHandler] Critical error handling request for "${dateContext}": ${error.message}`,
      error
    );
