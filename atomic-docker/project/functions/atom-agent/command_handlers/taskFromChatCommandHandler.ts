...
export async function handleCreateTaskFromChatMessageRequest(
  ...
  try {
    const skillResponse: CreateTaskFromChatMessageSkillResponse = await createTaskFromChatMessage(userId, entities);
    ...
    if (skillResponse.ok && skillResponse.data?.success) {
      ...
      const errorMsg = skillResponse.error?.message || skillResponse.data?.message || 'Unknown error from skill';
