...
} from '../skills/nlu_email_helper';
import {
  searchMyEmails,
  readEmail,
  extractContractEndDate,
  Email,
...
import {
  ...
  readEmail,
  Email,
  sendEmail as sendEmailSkill,
  EmailDetails,
  SendEmailResponse,
} from '../skills/emailSkills'; // Added sendEmailSkill, EmailDetails, SendEmailResponse
 ...
 */
export async function handleEmailInquiry(
  request: ParsedNluEmailRequest
): Promise<string> {
  const { userId, rawEmailSearchQuery, actionRequested, targetEmailId } =
    request;
  let messageToUser = '';
  ...
  try {
    ...
    if (targetEmailId) {
      console.log(
        `Agent: Attempting to read specified email ID ${targetEmailId} for user ${userId}.`
      );
      ...
      console.log(
        `Agent: Understanding email search query: "${rawEmailSearchQuery}" for user ${userId}`
      );
      const structuredSearchParams =
        await understandEmailSearchQueryLLM(rawEmailSearchQuery);
      ...
      console.log(
        `Agent: Searching emails for user ${userId} with LLM-derived query: ${gmailQueryString}`
      );
      ...
      console.log(
        `Agent: Found email ID ${singleEmailSummary.id}. Subject: "${singleEmailSummary.subject}". Fetching details...`
      );
      ...
      const emailDetailsResponse = await readEmail(
        userId,
        singleEmailSummary.id
      );
      if (emailDetailsResponse.success && emailDetailsResponse.email) {
        targetEmail = emailDetailsResponse.email;
      } else {
    ...
    switch (actionRequested.actionType) {
      ...
      case 'GET_DATE':
        ...
        const formattedDate = targetEmail.timestamp
          ? new Date(targetEmail.timestamp).toLocaleDateString(
              undefined,
              dateOptions
            )
          : 'an unknown date';
      ...
      case 'FIND_SPECIFIC_INFO':
        ...
        const extractionInputKeywords =
          keywordsToExtract.length > 0
            ? keywordsToExtract
            : nlQuestion
              ? [nlQuestion]
              : [];
        ...
        if (extractionInputKeywords.length === 1) {
          ...
          if (resultValue) {
            if (
              resultValue.startsWith('Keyword "') &&
              resultValue.endsWith(
                '" found. (Further analysis needed for specific value).'
              )
            ) {
          ...
          for (const keyword of extractionInputKeywords) {
            ...
            if (resultValue) {
              if (
                ...
                responseParts.push(
                  `- For "${keyword}", I found: ${resultValue}.`
                );
              }
              foundAny = true;
            } else {
    ...
    console.error(
      `Agent: Error in handleFindEmailAndExtractInfo for user ${userId}:`,
      error
    );
...
export async function handleSendEmailRequest(
  request: ParsedNluSendEmailRequest
): Promise<string> {
  const { userId, emailDetails } = request;
  let messageToUser = '';

  logger.info(
    `Agent: Attempting to send email for user ${userId} with details:`,
    emailDetails
  );
  ...
  if (
    ...
    logger.warn(
      `Agent: Missing details for send email request for user ${userId}. Details:`,
      emailDetails
    );
    return messageToUser;
  }
  ...
  try {
    ...
    if (sendResponse.success) {
      ...
      logger.info(
        `Agent: Email sent successfully for user ${userId}. Response:`,
        sendResponse
      );
    } else {
      messageToUser = `I tried to send the email, but something went wrong. ${sendResponse.message || 'An unknown error occurred.'}`;
      logger.error(
        `Agent: Failed to send email for user ${userId}. Response:`,
        sendResponse
      );
    }
  } catch (error: any) {
    logger.error(
      `Agent: Critical error in handleSendEmailRequest for user ${userId}:`,
      error
    );
