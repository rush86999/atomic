...
describe('SemanticSearchSkills & handleSemanticSearchMeetingNotesSkill', () => {
  ...
  const baseArgs: Omit<SkillArgs, 'params' | 'raw_message'> = {
  ...
  describe('handleSemanticSearchMeetingNotesSkill', () => {
    it('should return formatted results on successful search with data', async () => {
      ...
      } as AxiosResponse<BackendSearchResponse>);
    ...
    it('should return "no results" message on successful search with empty data', async () => {
      const mockApiResponse: BackendSearchResponse = {
        status: 'success',
        data: [],
      };
      mockedAxios.post.mockResolvedValueOnce({
      ...
      expect(result).toBe(
        "Sorry, I couldn't find any meeting notes matching your query."
      );
    });
    ...
    it('should return error message if backend returns status "error"', async () => {
      ...
      expect(result).toBe(
        'I encountered an error while searching: Backend search exploded'
      );
    });
    ...
    it('should return generic error message if backend response is malformed (e.g. unexpected structure)', async () => {
      ...
      const result = await handleSemanticSearchMeetingNotesSkill(args);
      expect(result).toBe(
        'I received an unexpected response from the search service. Please try again later.'
      );
    });
    ...
    it('should return generic error message on axios network error', async () => {
      ...
      expect(result).toBe(
        "I'm sorry, I ran into a problem trying to search your meeting notes. Please try again later."
      );
    });
    ...
    it('should ask for a query if query parameter is missing in params', async () => {
      ...
      const result = await handleSemanticSearchMeetingNotesSkill(args);
      expect(result).toBe(
        "Please provide a search query. For example, 'search_meeting_notes what were the decisions about project alpha?'"
      );
