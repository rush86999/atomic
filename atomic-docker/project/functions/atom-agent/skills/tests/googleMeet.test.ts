...
describe('googleMeet skill', () => {
  ...
  describe('handleListGoogleMeetEvents', () => {
    it('should return a list of Google Meet events', async () => {
      ...
      (
        calendarSkills.listUpcomingGoogleMeetEvents as jest.Mock
  ...
  describe('handleGetGoogleMeetEventDetails', () => {
    it('should return the details of a Google Meet event', async () => {
      ...
      (calendarSkills.getGoogleMeetEventDetails as jest.Mock).mockResolvedValue(
        mockResponse
      );
    ...
    it('should return an error message when the event ID is missing', async () => {
      ...
      expect(result).toBe(
        'Google Calendar Event ID is required to get details via NLU.'
      );
    });

    it('should return an error message when an error occurs', async () => {
      (calendarSkills.getGoogleMeetEventDetails as jest.Mock).mockRejectedValue(
      ...
      expect(result).toBe(
        'Sorry, an unexpected error occurred while fetching details for event 123 (NLU path).'
      );
    });
  });
});
