...
describe('msteams skill', () => {
  ...
  describe('handleListMicrosoftTeamsMeetings', () => {
    ...
    it('should return a message when there are no meetings', async () => {
      ...
      expect(result).toBe(
        'No Microsoft Teams meetings found matching your criteria (via NLU).'
      );
    });

    it('should return an error message when an error occurs', async () => {
      (msTeamsSkills.listMicrosoftTeamsMeetings as jest.Mock).mockRejectedValue(
      ...
      expect(result).toBe(
        'Sorry, an unexpected error occurred while fetching your Microsoft Teams meetings (NLU path).'
      );
    });
  });
  ...
  describe('handleGetMicrosoftTeamsMeetingDetails', () => {
    it('should return the details of a Microsoft Teams meeting', async () => {
      ...
      (
        msTeamsSkills.getMicrosoftTeamsMeetingDetails as jest.Mock
    ...
    it('should return an error message when the event ID is missing', async () => {
      const result = await handleGetMicrosoftTeamsMeetingDetails(
        'test-user',
        {}
      );

      expect(result).toBe(
        'Microsoft Graph Event ID is required to get Teams meeting details via NLU.'
      );
    });
    ...
    it('should return an error message when an error occurs', async () => {
      ...
      expect(result).toBe(
        'Sorry, an unexpected error occurred while fetching details for Teams meeting 123 (NLU path).'
      );
    });
  });
});
