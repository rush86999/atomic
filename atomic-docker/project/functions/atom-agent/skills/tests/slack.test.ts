...
describe('slack skill', () => {
  ...
  describe('handleSendSlackMessage', () => {
    ...
    it('should return an error message when the channel is missing', async () => {
      ...
      expect(result).toBe(
        'Slack channel/user ID is required to send a message via NLU.'
      );
    });
    ...
    it('should return an error message when the message text is missing', async () => {
      ...
      expect(result).toBe(
        'Message text is required to send a Slack message via NLU.'
      );
    });

    it('should return an error message when an error occurs', async () => {
      (slackSkills.sendSlackMessage as jest.Mock).mockRejectedValue(
      ...
      expect(result).toBe(
        'Sorry, there was an issue sending your Slack message.'
      );
    });
  });
  ...
  describe('handleSlackMyAgenda', () => {
    ...
    it('should send a message when there are no events', async () => {
      ...
      expect(result).toBe(
        "I've checked your calendar; no upcoming events. Sent a note to your Slack DM (NLU path)."
      );
    });

    it('should return an error message when an error occurs', async () => {
      (calendarSkills.listUpcomingEvents as jest.Mock).mockRejectedValue(
        new Error('Test Error')
      );
      ...
      expect(result).toBe(
        'Sorry, an error occurred while processing your agenda for Slack (NLU path).'
      );
    });
  });
});
