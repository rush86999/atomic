...
const mockListUpcomingEvents =
  calendarSkills.listUpcomingEvents as jest.MockedFunction<
    typeof calendarSkills.listUpcomingEvents
  >;
const mockSearchNotionRaw = notionSkills.searchNotionRaw as jest.MockedFunction<
  typeof notionSkills.searchNotionRaw
>;
const mockQueryNotionTasks =
  notionSkills.queryNotionTasks as jest.MockedFunction<
    typeof notionSkills.queryNotionTasks
  >;
const mockSearchEmailsNLU = emailSkills.searchEmailsNLU as jest.MockedFunction<
  typeof emailSkills.searchEmailsNLU
>;
...
describe('productivitySkills', () => {
  ...
  describe('handlePrepareForMeeting', () => {
    ...
    it('should return MEETING_NOT_FOUND if no meeting matches the identifier', async () => {
      ...
      const response = await handlePrepareForMeeting(
        mockUserId,
        'NonExistent Meeting'
      );
      ...
      expect(response.error?.message).toContain(
        'Could not find the specified meeting'
      );
    });
    ...
    it('should pick the next upcoming meeting if identifier is "next meeting"', async () => {
      mockListUpcomingEvents.mockResolvedValue(
        [upcomingEvent1, upcomingEvent2].sort(
          (a, b) =>
    ...
    it('should successfully prepare for a meeting with data from all sources', async () => {
      ...
      expect(response.data?.relatedNotionPages?.[0].title).toBe(
        'Q3 Planning Notes'
      );
      expect(response.data?.relatedEmails).toHaveLength(1);
      expect(response.data?.relatedEmails?.[0].subject).toBe(
        'Prep for Q3 Sync'
      );
      expect(response.data?.relatedTasks).toHaveLength(1);
      expect(response.data?.relatedTasks?.[0].description).toBe(
        'Draft Q3 roadmap'
      );
    ...
    it('should include error messages if a data source fetch fails, but still return overall OK with partial data', async () => {
      ...
      expect(response.data?.relatedEmails).toHaveLength(1);
      expect(response.data?.errorMessage).toContain(
        'Could not fetch Notion documents: Notion API timeout.'
      );
    });
    ...
    it('should limit the number of results for each context type to MAX_RESULTS_PER_CONTEXT', async () => {
      ...
      const manyNotionPages: NotionSearchResultData[] = Array(5)
        .fill(null)
      ...
      const manyEmails: Email[] = Array(5)
        .fill(null)
        .map((_, i) => ({
          id: `email${i}`,
          subject: `Email ${i}`,
      ...
      const manyTasks: NotionTask[] = Array(5)
        .fill(null)
        .map((_, i) => ({
          id: `task${i}`,
          description: `Task ${i}`,
  ...
  describe('determineDateRange', () => {
    ...
    it('should correctly determine "this week" as Mon-Fri if current day is past Friday 5pm or weekend', () => {
      ...
      const { startDate, endDate, displayRange } =
        determineDateRange('this week');
      ...
      const { startDate: satStart, endDate: satEnd } =
        determineDateRange('this week');
    ...
    it('should default to "this week" if no timePeriod is provided', () => {
      ...
      expect(defaultRange.startDate.toISOString()).toBe(
        thisWeekRange.startDate.toISOString()
      );
      expect(defaultRange.endDate.toISOString()).toBe(
        thisWeekRange.endDate.toISOString()
      );
  ...
  describe('handleGenerateWeeklyDigest', () => {
    ...
    const mockCompletedTask: NotionTask = {
      id: 'ct1',
    ...
    const mockAttendedMeeting: CalendarEvent = {
      id: 'am1',
      summary: 'Budget Review',
      startTime: '2024-07-24T14:00:00Z',
      endTime: '2024-07-24T15:00:00Z',
    };
    const mockUpcomingTask: NotionTask = {
      id: 'ut1',
    ...
    const mockUpcomingMeeting: CalendarEvent = {
      id: 'um1',
    ...
    it('should generate a digest for "this week" successfully', async () => {
      ...
      const response = await handleGenerateWeeklyDigest(
        mockUserId,
        'this week'
      );
      expect(response.ok).toBe(true);
    ...
    it('should handle missing ATOM_NOTION_TASKS_DATABASE_ID gracefully', async () => {
      ...
      expect(digest?.errorMessage).toContain(
        'Notion tasks database ID not configured'
      );
    ...
    it('should handle errors from underlying services and report them', async () => {
      ...
      mockListUpcomingEvents.mockRejectedValue(
        new Error('Calendar service down')
      );
      ...
      const digest = response.data?.digest;
      expect(digest?.errorMessage).toContain(
        'Could not fetch completed tasks: Notion API error for completed'
      );
      expect(digest?.errorMessage).toContain(
  ...
  describe('handleSuggestFollowUps', () => {
    ...
    const mockNotionDoc: NotionSearchResultData = {
      ...
      content:
        'Action Item: Alice to update roadmap. Decision: Q2 budget approved. Question: When is Phase 2 starting?',
    ...
    it('should return CONTEXT_NOT_FOUND if no meeting or project document is found', async () => {
      ...
      const response = await handleSuggestFollowUps(
        mockUserId,
        'Unknown Context'
      );
      expect(response.ok).toBe(false);
    ...
    it('should process meeting context, call LLM, and suggest follow-ups', async () => {
      ...
      expect(response.ok).toBe(true);
      expect(mockAnalyzeTextForFollowUps).toHaveBeenCalledWith(
        mockNotionDoc.content,
        mockNotionDoc.title
      );
      expect(response.data?.suggestions).toHaveLength(3);
      expect(
        response.data?.suggestions.find((s) => s.type === 'action_item')
          ?.description
      ).toBe('Alice to update roadmap');
      expect(
        response.data?.suggestions.find((s) => s.type === 'action_item')
          ?.existingTaskFound
      ).toBe(false);
      expect(
        response.data?.suggestions.find((s) => s.type === 'decision')
          ?.description
      ).toBe('Q2 budget approved');
      expect(
        response.data?.suggestions.find((s) => s.type === 'question')
          ?.description
      ).toBe('When is Phase 2 starting?');
      expect(response.data?.contextName).toBe(
        `Meeting: ${mockNotionDoc.title} on ${new Date(mockMeetingContext.startTime).toLocaleDateString()}`
      );
    });
    ...
    it('should identify an existing task for a suggested action item', async () => {
      ...
      expect(response.ok).toBe(true);
      const actionItem = response.data?.suggestions.find(
        (s) => s.type === 'action_item'
      );
    ...
    it('should handle LLM analysis failure gracefully', async () => {
      ...
      expect(response.data?.errorMessage).toContain(
        'LLM analysis failed: LLM API unavailable'
      );
      expect(response.data?.suggestions).toEqual([]);
    });
    ...
    it('should process project context if specified', async () => {
      const projectNotionDoc: NotionSearchResultData = {
        ...
        content:
          'Action: Schedule kickoff with client. Decision: Use standard template. Question: Who is main PoC?',
      ...
      const response = await handleSuggestFollowUps(
        mockUserId,
        'Client Onboarding Project Plan',
        'project'
      );
      expect(response.ok).toBe(true);
      expect(mockAnalyzeTextForFollowUps).toHaveBeenCalledWith(
        projectNotionDoc.content,
        projectNotionDoc.title
      );
      expect(response.data?.suggestions).toHaveLength(3);
      expect(response.data?.contextName).toBe(
        `Project: ${projectNotionDoc.title}`
      );
    });
  });
});
