import { HubSpotContactProperties, CreateHubSpotContactResponse, HubSpotContact } from '../../types';
...
export async function handleGetHubSpotContactByEmail(userId: string, entities: any): Promise<string> {
    try {
        ...
        if (!emailEntity || typeof emailEntity !== 'string' || emailEntity.trim() === '') {
            ...
            const contact: HubSpotContact | null = await getByEmail(userId, emailEntity);
            if (contact) {
...
export async function handleCreateHubSpotContact(userId: string, entities: any): Promise<string> {
    try {
        ...
        if (!email || typeof email !== 'string') {
            ...
            const hubspotResponse: CreateHubSpotContactResponse = await create(userId, contactDetails);
