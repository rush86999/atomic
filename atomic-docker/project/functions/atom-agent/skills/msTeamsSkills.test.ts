...
describe('msTeamsSkills', () => {
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(constants, 'ATOM_MSGRAPH_CLIENT_ID', { value: 'test-client-id', configurable: true });
    Object.defineProperty(constants, 'ATOM_MSGRAPH_CLIENT_SECRET', { value: 'test-client-secret', configurable: true });
    Object.defineProperty(constants, 'ATOM_MSGRAPH_TENANT_ID', { value: 'test-tenant-id', configurable: true });
    Object.defineProperty(constants, 'ATOM_MSGRAPH_AUTHORITY', { value: `https://login.microsoftonline.com/test-tenant-id`, configurable: true });
    Object.defineProperty(constants, 'ATOM_MSGRAPH_SCOPES', { value: ['https://graph.microsoft.com/.default'], configurable: true });
  ...
  describe('MSAL Token Logic (implicitly via skill calls)', () => {
    ...
    it('should refresh MSAL token if expired', async () => {
      ...
      mockAcquireTokenByClientCredential.mockResolvedValueOnce({ // For the refresh
  ...
  describe('listMicrosoftTeamsMeetings', () => {
    beforeEach(() => { // Ensure a valid token for these specific tests
    ...
    it('should list Teams meetings successfully with default filter', async () => {
      const mockEvents: Partial<MSGraphEvent>[] = [{ subject: 'Teams Meeting 1', id: 'evt1' }];
      mockedAxios.get.mockResolvedValueOnce({ data: { value: mockEvents, '@odata.nextLink': 'nextLink123' } });

      const result = await listMicrosoftTeamsMeetings(userPrincipalNameOrId, { limit: 5 });
      expect(result.ok).toBe(true);
      expect(result.events).toEqual(mockEvents);
      expect(result.nextLink).toBe('nextLink123');
      expect(mockedAxios.get).toHaveBeenCalledWith(
        expect.stringContaining(`/users/${userPrincipalNameOrId}/calendar/events`),
        expect.objectContaining({ headers: { 'Authorization': 'Bearer valid_graph_token' } })
      );
    ...
    it('should list meetings without Teams filter if options.filterForTeams is false', async () => {
      ...
      await listMicrosoftTeamsMeetings(userPrincipalNameOrId, { filterForTeams: false });
      const calledUrl = mockedAxios.get.mock.calls[0][0];
    ...
    it('should use nextLink for pagination if provided', async () => {
      ...
      mockedAxios.get.mockResolvedValueOnce({ data: { value: [{ id: 'evt2' }] } });
      await listMicrosoftTeamsMeetings(userPrincipalNameOrId, { nextLink: mockNextLink });
      expect(mockedAxios.get).toHaveBeenCalledWith(
    ...
    it('should handle API error when listing meetings', async () => {
      const graphError = { isAxiosError: true, response: { data: { error: { message: 'Graph API Error' } } }, message: 'Request failed' } as AxiosError;
  ...
  describe('getMicrosoftTeamsMeetingDetails', () => {
    ...
    it('should get meeting details successfully with specific select fields', async () => {
      const mockEventDetail: Partial<MSGraphEvent> = { id: 'evtDetail1', subject: 'Detailed Teams Meeting' };
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith(
        expectedUrl,
        expect.objectContaining({ headers: { 'Authorization': 'Bearer valid_graph_token_for_details' } })
      );
    });

    it('should handle meeting not found (404 error)', async () => {
      const eventId = 'nonExistentEventId';
      const graphError = { isAxiosError: true, response: { status: 404, data: { error: { message: 'Resource not found' } } }, message: 'Request failed' } as AxiosError;
      mockedAxios.get.mockRejectedValueOnce(graphError);
      const result = await getMicrosoftTeamsMeetingDetails(userPrincipalNameOrId, eventId);
    ...
    it('should handle other API errors when getting details', async () => {
      ...
      const graphError = { isAxiosError: true, response: { data: { error: { message: 'Some other Graph API Error' } } }, message: 'Request failed' } as AxiosError;
      mockedAxios.get.mockRejectedValueOnce(graphError);
      const result = await getMicrosoftTeamsMeetingDetails(userPrincipalNameOrId, eventId);
      expect(result.ok).toBe(false);
