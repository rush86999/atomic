...
async function findTargetMeeting(
  ...
  if (meetingDateTime) {
    ...
    console.warn(
      `[findTargetMeeting] Date/time based filtering for "${meetingDateTime}" is not fully implemented here. Relies on NLU or specific date.`
    );
  }
...
export async function handlePrepareForMeeting(
  ...
  console.log(
    `[handlePrepareForMeeting] User: ${userId}, Meeting ID: "${meetingIdentifier}", DateTime: "${meetingDateTime}"`
  );
  const targetMeeting = await findTargetMeeting(
    userId,
    meetingIdentifier,
    meetingDateTime
  );
  ...
  console.log(
    `[handlePrepareForMeeting] Target meeting found: "${targetMeeting.summary}" at ${targetMeeting.startTime}`
  );
  ...
  if (targetMeeting.description) {
    meetingKeywordsArray.push(
      ...targetMeeting.description.split(' ').slice(0, 10)
  ...
  try {
    ...
    console.log(
      `[handlePrepareForMeeting] Querying Notion with: "${notionQuery}"`
    );
    const notionSearchResponse = await searchNotionRaw(
      userId,
      notionQuery,
      MAX_RESULTS_PER_CONTEXT
    );
    ...
    if (
      ...
      preparationData.relatedNotionPages = notionSearchResponse.data
        .map((p: NotionSearchResultData) => ({
          ...
          briefSnippet:
            ...
            p.properties?.Description?.rich_text?.[0]?.plain_text.substring(
              0,
              150
            ),
        }))
      ...
      console.log(
        `[handlePrepareForMeeting] Found ${preparationData.relatedNotionPages?.length || 0} Notion pages.`
      );
    } else if (notionSearchResponse && !notionSearchResponse.ok) {
      console.warn(
        `[handlePrepareForMeeting] Notion search failed: ${notionSearchResponse.error?.message}`
      );
      accumulatedErrorMessages += `Could not fetch Notion documents: ${notionSearchResponse.error?.message}. `;
    } else {
      console.log(
        `[handlePrepareForMeeting] No Notion pages found or unexpected response format from searchNotionRaw.`
      );
    }
  } catch (e: any) {
    console.error(
      '[handlePrepareForMeeting] Error fetching Notion context:',
      e.message,
      e.stack
    );
  ...
  try {
    ...
    emailQueryParts.push(
      `between ${fromDate.toISOString().split('T')[0]} and ${toDate.toISOString().split('T')[0]}`
    );
    const emailQuery = emailQueryParts.join(' ');

    console.log(
      `[handlePrepareForMeeting] Querying Emails with: "${emailQuery}"`
    );
    // searchEmailsNLU should ideally handle parsing this query or accept structured input.
    const emailsResponse: SkillResponse<Email[]> = await searchEmailsNLU(
      userId,
      emailQuery,
      MAX_RESULTS_PER_CONTEXT
    );
    ...
    if (emailsResponse && emailsResponse.ok && emailsResponse.data) {
      preparationData.relatedEmails = emailsResponse.data
        ...
        .slice(0, MAX_RESULTS_PER_CONTEXT);
      console.log(
        `[handlePrepareForMeeting] Found ${preparationData.relatedEmails?.length || 0} emails.`
      );
    } else if (emailsResponse && !emailsResponse.ok) {
      console.warn(
        `[handlePrepareForMeeting] Email search failed: ${emailsResponse.error?.message}`
      );
      accumulatedErrorMessages += `Could not fetch relevant emails: ${emailsResponse.error?.message}. `;
    } else {
      console.log(
        `[handlePrepareForMeeting] No emails found or unexpected response format from email search.`
      );
    }
  } catch (e: any) {
    console.error(
      '[handlePrepareForMeeting] Error fetching Email context:',
      e.message,
      e.stack
    );
    accumulatedErrorMessages +=
  ...
  try {
    ...
    if (!notionTasksDbId) {
      console.warn(
        '[handlePrepareForMeeting] ATOM_NOTION_TASKS_DATABASE_ID is not set. Skipping task search.'
      );
      ...
      console.log(
        `[handlePrepareForMeeting] Querying Tasks with params:`,
        taskQueryParams
      );
      ...
      if (tasksResponse.success && tasksResponse.tasks) {
        preparationData.relatedTasks = tasksResponse.tasks
          ...
          .slice(0, MAX_RESULTS_PER_CONTEXT);
        console.log(
          `[handlePrepareForMeeting] Found ${preparationData.relatedTasks?.length || 0} tasks.`
        );
      } else if (!tasksResponse.success) {
        console.warn(
          `[handlePrepareForMeeting] Task query failed: ${tasksResponse.error}`
        );
        accumulatedErrorMessages += `Could not fetch related tasks: ${tasksResponse.error}. `;
      } else {
        console.log(
          `[handlePrepareForMeeting] No tasks found or unexpected response from task query.`
        );
      }
    }
  } catch (e: any) {
    console.error(
      '[handlePrepareForMeeting] Error fetching Task context:',
      e.message,
      e.stack
    );
  ...
  console.log(
    `[handlePrepareForMeeting] Preparation data compiled:`,
    JSON.stringify(preparationData).substring(0, 500) + '...'
  );
...
export function determineDateRange(
  timePeriod?: 'this week' | 'last week' | string
): DateRange {
  const now = new Date();
...
export async function handleGenerateWeeklyDigest(
  ...
  console.log(
    `[handleGenerateWeeklyDigest] User: ${userId}, Time Period: "${timePeriodInput}"`
  );
  ...
  const {
    ...
    nextPeriodEndDate,
    displayRange,
  } = determineDateRange(timePeriodInput);
  console.log(
    `[handleGenerateWeeklyDigest] Determined date range: ${startDate.toISOString()} to ${endDate.toISOString()}`
  );
  console.log(
    `[handleGenerateWeeklyDigest] Next period range: ${nextPeriodStartDate.toISOString()} to ${nextPeriodEndDate.toISOString()}`
  );
  ...
  if (!notionTasksDbId) {
    console.warn(
      '[handleGenerateWeeklyDigest] ATOM_NOTION_TASKS_DATABASE_ID is not set. Skipping completed task search.'
    );
    accumulatedErrorMessages +=
    ...
    try {
      ...
      console.log(
        `[handleGenerateWeeklyDigest] Querying completed tasks with params:`,
        completedTaskParams
      );
      ...
      if (taskResponse.success && taskResponse.tasks) {
        digestData.completedTasks = taskResponse.tasks
          .filter((task) => {
            ...
            if (!relevantDate) {
              console.warn(
                `[handleGenerateWeeklyDigest] Task ID ${task.id} has no last_edited_time. Cannot accurately determine if completed in period.`
              );
              return false;
            }
        ...
        console.log(
          `[handleGenerateWeeklyDigest] Found ${digestData.completedTasks.length} completed tasks in period (after date filtering).`
        );
      ...
      console.error(
        '[handleGenerateWeeklyDigest] Error fetching completed tasks:',
        e.message,
        e.stack
      );
      accumulatedErrorMessages +=
  ...
  try {
    ...
    const meetingsAttended = await listUpcomingEvents(
      userId,
      MAX_DIGEST_ITEMS * 2,
    ...
    if (meetingsAttended) {
      ...
      digestData.attendedMeetings = meetingsAttended
        ...
        .slice(0, MAX_DIGEST_ITEMS);
      console.log(
        `[handleGenerateWeeklyDigest] Found ${digestData.attendedMeetings.length} attended meetings.`
      );
    }
  } catch (e: any) {
    console.error(
      '[handleGenerateWeeklyDigest] Error fetching attended meetings:',
      e.message,
      e.stack
    );
    accumulatedErrorMessages +=
  ...
  if (!notionTasksDbId) {
    console.warn(
      '[handleGenerateWeeklyDigest] ATOM_NOTION_TASKS_DATABASE_ID is not set. Skipping upcoming task search.'
    );
    accumulatedErrorMessages +=
    ...
    try {
      ...
      console.log(
        `[handleGenerateWeeklyDigest] Querying upcoming critical tasks with params:`,
        upcomingTaskParams
      );
      const upcomingTaskResponse = await queryNotionTasks(
        userId,
        upcomingTaskParams
      );
      if (upcomingTaskResponse.success && upcomingTaskResponse.tasks) {
        digestData.upcomingCriticalTasks = upcomingTaskResponse.tasks.slice(
          0,
          MAX_DIGEST_ITEMS
        );
        console.log(
          `[handleGenerateWeeklyDigest] Found ${digestData.upcomingCriticalTasks.length} upcoming critical tasks.`
        );
      ...
      console.error(
        '[handleGenerateWeeklyDigest] Error fetching upcoming tasks:',
        e.message,
        e.stack
      );
      accumulatedErrorMessages +=
  ...
  try {
    ...
    const upcomingMeetings = await listUpcomingEvents(
      userId,
      MAX_DIGEST_ITEMS * 2,
      nextPeriodStartDate.toISOString(),
      nextPeriodEndDate.toISOString()
    );
    if (upcomingMeetings) {
          ...
          return (
            ...
            (event.attendees &&
              event.attendees.some(
                (a) => a.email && !a.email.endsWith('@yourcompany.com')
              ))
          );
        })
        .slice(0, MAX_DIGEST_ITEMS);
      console.log(
        `[handleGenerateWeeklyDigest] Found ${digestData.upcomingCriticalMeetings.length} upcoming critical meetings.`
      );
    }
  } catch (e: any) {
    console.error(
      '[handleGenerateWeeklyDigest] Error fetching upcoming meetings:',
      e.message,
      e.stack
    );
    accumulatedErrorMessages +=
...
export async function handleSuggestFollowUps(
  ...
  console.log(
    `[handleSuggestFollowUps] User: ${userId}, Context: "${contextIdentifier}", Type: "${contextType}"`
  );
  ...
  try {
    if (
      ...
      const meeting = await findTargetMeeting(
        userId,
        contextIdentifier.replace(/meeting|call|sync/i, '').trim()
      ); // findTargetMeeting might need adjustment for "last meeting"
      if (meeting) {
        ...
        if (notionSearchResponse?.ok && notionSearchResponse.data?.[0]) {
          ...
          console.log(
            `[handleSuggestFollowUps] Found Notion page for meeting: ${sourceDocumentTitle}`
          );
        } else {
      ...
      if (notionSearchResponse?.ok && notionSearchResponse.data?.[0]) {
        ...
        console.log(
          `[handleSuggestFollowUps] Found Notion page for project: ${sourceDocumentTitle}`
        );
      } else {
        return {
          ok: false,
    ...
    console.error(
      '[handleSuggestFollowUps] Error retrieving context:',
      e.message,
      e.stack
    );
    return {
  ...
  let extractedItems: ExtractedFollowUpItems = {
    action_items: [],
    decisions: [],
    questions: [],
  };
  try {
    console.log(
      `[handleSuggestFollowUps] Analyzing text (length: ${sourceDocumentText.length}) for "${sourceDocumentTitle}" with LLM.`
    );
    const llmAnalysis = await analyzeTextForFollowUps(
      sourceDocumentText,
      sourceDocumentTitle
    ); // Conceptual LLM call
    if (llmAnalysis.error) {
      ...
      console.log(
        `[handleSuggestFollowUps] LLM extracted: ${extractedItems.action_items.length} actions, ${extractedItems.decisions.length} decisions, ${extractedItems.questions.length} questions.`
      );
    }
  } catch (e: any) {
    console.error(
      '[handleSuggestFollowUps] Error during LLM analysis:',
      e.message,
      e.stack
    );
  ...
  const mapToPotentialFollowUp = async (
    item: { description: string; assignee?: string },
    type: PotentialFollowUpType
  ): Promise<PotentialFollowUp> => {
    ...
    if (type === 'action_item' && notionTasksDbId) {
      try {
        ...
        if (
          ...
          console.log(
            `[handleSuggestFollowUps] Found existing task '${existingTask.description}' for action item '${item.description}'`
          );
        }
      } catch (taskError: any) {
        console.warn(
          `[handleSuggestFollowUps] Error checking for existing task for "${item.description}": ${taskError.message}`
        );
      }
    }
    return followUp;
  ...
  if (extractedItems.action_items) {
    for (const action of extractedItems.action_items) {
      followUpData.suggestions.push(
        await mapToPotentialFollowUp(action, 'action_item')
      );
    }
  }
  if (extractedItems.decisions) {
    for (const decision of extractedItems.decisions) {
      followUpData.suggestions.push(
        await mapToPotentialFollowUp(decision, 'decision')
      );
    }
  }
  if (extractedItems.questions) {
    for (const question of extractedItems.questions) {
      followUpData.suggestions.push(
        await mapToPotentialFollowUp(question, 'question')
      );
    }
  }

  followUpData.suggestions = followUpData.suggestions.slice(
    0,
    MAX_FOLLOW_UPS_TO_SUGGEST
  );
  ...
  console.log(
    `[handleSuggestFollowUps] Final suggestions compiled for "${followUpData.contextName}". Count: ${followUpData.suggestions.length}`
  );
  return { ok: true, data: followUpData };
}
