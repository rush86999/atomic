...
describe('Autopilot Skills', () => {
  ...
  describe('enableAutopilot', () => {
    ...
    it('should enable autopilot successfully when all API calls succeed', async () => {
      (
        apiHelper.createDailyFeaturesApplyEventTrigger as jest.Mock
      ).mockResolvedValue({ ok: true, data: 'event-id-123' });
      (apiHelper.upsertAutopilotOne as jest.Mock).mockResolvedValue({
        ok: true,
    ...
    it('should call deleteScheduledEventForAutopilot if upsertAutopilotOne fails', async () => {
      ...
      (
        apiHelper.deleteScheduledEventForAutopilot as jest.Mock
      ).mockResolvedValue({ ok: true, data: { success: true } }); // Mock cleanup

      const result = await enableAutopilot(userId, mockQueryComplex);

      expect(apiHelper.createDailyFeaturesApplyEventTrigger).toHaveBeenCalled();
      expect(apiHelper.upsertAutopilotOne).toHaveBeenCalled();
      expect(apiHelper.deleteScheduledEventForAutopilot).toHaveBeenCalledWith(
        'event-id-123'
      );
      expect(result.ok).toBe(false);
    ...
    it('should handle simple string query for default behavior', async () => {
      (
        apiHelper.createDailyFeaturesApplyEventTrigger as jest.Mock
      ).mockResolvedValue({ ok: true, data: 'event-id-default' });
      (apiHelper.upsertAutopilotOne as jest.Mock).mockResolvedValue({
        ok: true,
      ...
      expect(
        apiHelper.createDailyFeaturesApplyEventTrigger
  ...
  describe('disableAutopilot', () => {
    ...
    it('should disable autopilot successfully when all API calls succeed', async () => {
      ...
      (apiHelper.deleteAutopilotGivenId as jest.Mock).mockResolvedValue({
        ok: true,
        data: { id: eventId } as AutopilotType,
    ...
    it('should return error if eventId is not provided in query', async () => {
      ...
      expect(result.error?.message).toContain(
        'eventId (autopilotId) is required'
      );
  ...
  describe('getAutopilotStatus', () => {
    ...
    it('should call getAutopilotGivenId if autopilotId is provided in query', async () => {
      (apiHelper.getAutopilotGivenId as jest.Mock).mockResolvedValue({
        ok: true,
      ...
      expect(apiHelper.getAutopilotGivenId).toHaveBeenCalledWith(
        specificAutopilotId
      );
    ...
    it('should call listAutopilotsGivenUserId if no autopilotId is provided in query', async () => {
      ...
      (apiHelper.listAutopilotsGivenUserId as jest.Mock).mockResolvedValue({
        ok: true,
