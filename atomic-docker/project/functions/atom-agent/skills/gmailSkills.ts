...
import {
  ...
  StructuredEmailQuery,
  parseRelativeDateQuery,
} from './nlu_email_helper'; // Assuming nlu_email_helper.ts will export these
import {
  searchMyEmails as searchMyEmailsBackend,
  readEmail as readEmailBackend,
  extractInformationFromEmailBody,
} from './emailSkills';
...
export async function handleSearchGmail(
  ...
  logger.info(
    `[handleSearchGmail] User: ${userId}, Query: "${rawUserQuery}", Limit: ${limit}`
  );

  try {
    let structuredQuery: Partial<StructuredEmailQuery> =
      await understandEmailSearchQueryLLM(rawUserQuery);
    logger.info(
      `[handleSearchGmail] LLM structured query: ${JSON.stringify(structuredQuery)}`
    );

    if (Object.keys(structuredQuery).length === 0 && rawUserQuery.length > 0) {
      logger.warn(
        `[handleSearchGmail] LLM returned empty structured query for input: "${rawUserQuery}". Using raw query as custom query for backend.`
      );
    ...
    logger.info(
      `[handleSearchGmail] Constructed Gmail API query string: "${gmailApiQueryString}"`
    );
    ...
    if (!gmailApiQueryString || gmailApiQueryString.trim() === '') {
      return {
        ...
        data: {
          emails: [],
          userMessage:
            "I couldn't determine specific search criteria from your request. Please try rephrasing.",
        },
      };
    }

    const emails: Email[] = await searchMyEmailsBackend(
      userId,
      gmailApiQueryString,
      limit
    );
...
export async function getRecentUnreadEmailsForBriefing(
  ...
  SkillResponse<{ results: GmailMessageSnippet[]; query_executed?: string }>
> {
  logger.info(
    `[getRecentUnreadEmailsForBriefing] User: ${userId}, TargetDate: ${targetDate.toISOString().split('T')[0]}, Count: ${count}`
  );
  ...
  try {
    ...
    logger.info(
      `[getRecentUnreadEmailsForBriefing] Constructed Gmail API query string: "${gmailApiQueryString}"`
    );

    // searchMyEmailsBackend expects a raw query string and returns Email[]
    const backendResults: Email[] = await searchMyEmailsBackend(
      userId,
      gmailApiQueryString,
      count
    );
    ...
    logger.info(
      `[getRecentUnreadEmailsForBriefing] Found ${results.length} unread emails for the target date.`
    );
    return { ok: true, data: { results, query_executed: gmailApiQueryString } };
  } catch (error: any) {
    logger.error(
      `[getRecentUnreadEmailsForBriefing] Error: ${error.message}`,
      error
    );
    return {
      ok: false,
      error: {
        ...
        message:
          error.message || 'Failed to fetch recent unread emails for briefing.',
      },
    };
  }
}
...
> {
  logger.info(
    `[searchEmailsForPrep] User: ${userId}, Params: ${JSON.stringify(params)}, MeetingContext: ${meetingContext?.summary}`
  );
  ...
  try {
    ...
    if (meetingContext) {
      ...
      if (meetingContext.attendees && meetingContext.attendees.length > 0) {
        ...
        if (attendeeEmailQueries.length > 0) {
          ...
          structuredQuery.customQuery = structuredQuery.customQuery
            ...
            : attendeeQueryPart;
          logger.info(
            `[searchEmailsForPrep] Added attendee query part: ${attendeeQueryPart}`
          );
        }
      }
      ...
      if (meetingContext.start && isDateQueryGeneric) {
        ...
        if (meetingContext.end) {
          try {
            ...
            if (isNaN(meetingEndDateForQuery.getTime())) {
              logger.warn(
                `[searchEmailsForPrep] Invalid meetingContext.end date: ${meetingContext.end}. Falling back to start date.`
              );
              meetingEndDateForQuery = new Date(meetingContext.start);
            }
          } catch (e) {
            logger.warn(
              `[searchEmailsForPrep] Error parsing meetingContext.end date: ${meetingContext.end}. Falling back to start date.`,
              e
            );
            meetingEndDateForQuery = new Date(meetingContext.start);
          }
        ...
        logger.info(
          `[searchEmailsForPrep] Date range from meeting context: after:${structuredQuery.after} before:${structuredQuery.before}`
        );
      }
    }
    ...
    if (
      !(structuredQuery.after || structuredQuery.before) &&
      params.date_query
    ) {
      logger.info(
        `[searchEmailsForPrep] Attempting to parse params.date_query: "${params.date_query}"`
      );
      ...
      if (parsedDates) {
        ...
        logger.info(
          `[searchEmailsForPrep] Parsed date query to: after:${parsedDates.after}, before:${parsedDates.before}`
        );
      } else {
        ...
        if (
          ...
          structuredQuery.customQuery = structuredQuery.customQuery
            ...
            : params.date_query;
          logger.info(
            `[searchEmailsForPrep] Using params.date_query as custom query part: "${params.date_query}"`
          );
        } else {
          logger.warn(
            `[searchEmailsForPrep] Could not parse date_query: "${params.date_query}" and it doesn't look like a direct Gmail date term.`
          );
        }
      }
    }
    ...
    const gmailApiQueryString = buildGmailSearchQuery(structuredQuery);
    logger.info(
      `[searchEmailsForPrep] Constructed Gmail API query string: "${gmailApiQueryString}"`
    );

    if (!gmailApiQueryString || gmailApiQueryString.trim() === '') {
      logger.warn(
        '[searchEmailsForPrep] Empty Gmail API query string generated. Returning no results.'
      );
      return {
        ok: true,
...
> {
  logger.info(
    `[handleExtractInfoFromGmail] User: ${userId}, EmailRef: "${emailIdOrReference}", Keywords: [${informationKeywords.join(', ')}]`
  );
  ...
  try {
    if (!emailBody) {
      ...
      logger.info(
        `[handleExtractInfoFromGmail] Attempting to fetch email ID: ${emailIdOrReference}`
      );
    ...
    const extractedInfo = await extractInformationFromEmailBody(
      emailBody,
      informationKeywords
    );
