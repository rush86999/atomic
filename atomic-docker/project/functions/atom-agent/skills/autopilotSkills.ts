...
function parseQuery(query: string): AutopilotQuery {
  try {
    ...
    console.warn(
      `Query string is not a valid JSON: ${query}. Treating as simple string or relying on defaults.`
    );
...
export async function enableAutopilot(
  ...
  console.log(
    `Attempting to enable Autopilot for user ${userId} with query: ${query}`
  );
  ...
  const bodyData: ScheduleAssistWithMeetingQueueBodyType =
    parsedQuery.payload || {
      userId,
  ...
  const autopilotData: AutopilotType = {
    ...
    userId,
    scheduleAt:
      parsedQuery.scheduleAt || dayjs(bodyData.windowStartDate).utc().format(),
  ...
  try {
    ...
    const eventTriggerResponse = await createDailyFeaturesApplyEventTrigger(
      autopilotData,
      bodyData
    );
    if (!eventTriggerResponse.ok || !eventTriggerResponse.data) {
      console.error(
        'Failed to create scheduled event trigger:',
        eventTriggerResponse.error
      );
      return {
        ok: false,
    ...
    if (!upsertResponse.ok) {
      console.error(
        'Failed to upsert autopilot configuration:',
      ...
      console.warn(
        `Cleaned up scheduled event ${autopilotData.id} after failed upsert.`
      );
      return {
        ok: false,
    ...
    console.log(
      `Autopilot enabled successfully for user ${userId}. Autopilot ID (Event ID): ${autopilotData.id}`
    );
...
export async function disableAutopilot(
  ...
  console.log(
    `Attempting to disable Autopilot for user ${userId} with query: ${query}`
  );
  const parsedQuery = parseQuery(query);
  ...
  if (!eventId) {
    console.error(
      'Error disabling Autopilot: eventId (autopilotId) not provided in query.'
    );
    return {
      ok: false,
      error: {
  ...
  try {
    ...
    if (!deleteEventResponse.ok) {
      console.warn(
        `Failed to delete scheduled event ${eventId}. It might have already been processed or deleted. Proceeding to delete DB record.`,
    ...
    if (!deleteAutopilotRecordResponse.ok) {
      console.error(
        `Failed to delete Autopilot database record ${eventId}:`,
        deleteAutopilotRecordResponse.error
      );
    ...
    console.log(
      `Autopilot (ID: ${eventId}) disabled successfully for user ${userId}.`
    );
    ...
    return {
      ...
      error: {
        ...
        message:
          error.message || 'An unexpected error occurred during disable.',
      },
    };
  }
}
...
export async function getAutopilotStatus(
  userId: string,
  query: string
): Promise<AutopilotApiResponse<AutopilotType | AutopilotType[] | null>> {
  console.log(
    `Getting Autopilot status for user ${userId} with query: ${query}`
  );
  const parsedQuery = parseQuery(query);
  ...
  try {
    if (autopilotId) {
      ...
      if (!response.ok) {
        console.error(
          `Error fetching status for Autopilot ID ${autopilotId}:`,
          response.error
        );
      }
      return response;
      ...
      if (!response.ok) {
        console.error(
          `Error fetching Autopilot statuses for user ${userId}:`,
          response.error
        );
      }
      return response;
