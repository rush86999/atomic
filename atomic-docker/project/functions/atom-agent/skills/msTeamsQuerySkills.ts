...
import {
  understandMSTeamsSearchQueryLLM,
  StructuredMSTeamsQuery,
} from './llm_msteams_query_understander';
...
export async function handleSearchMSTeamsMessages(
  ...
  logger.info(
    `[handleSearchMSTeamsMessages] User: ${userId}, Query: "${rawUserQuery}", Limit: ${limit}`
  );

  try {
    const structuredQuery: Partial<StructuredMSTeamsQuery> =
      await understandMSTeamsSearchQueryLLM(rawUserQuery);
    logger.info(
      `[handleSearchMSTeamsMessages] LLM structured query for MS Teams: ${JSON.stringify(structuredQuery)}`
    );
    ...
    const kqlQueryString = buildMSTeamsSearchQuery(
      structuredQuery,
      rawUserQuery
    );
    logger.info(
      `[handleSearchMSTeamsMessages] Constructed MS Teams KQL query string: "${kqlQueryString}"`
    );
    ...
    if (!kqlQueryString || kqlQueryString.trim() === '') {
      return {
        ...
        data: {
          messages: [],
          userMessage:
            "I couldn't determine specific search criteria for MS Teams from your request. Please try rephrasing.",
        },
      };
    }

    const messages: MSTeamsMessage[] = await searchMyMSTeamsMessagesBackend(
      userId,
      kqlQueryString,
      limit
    );
    ...
    logger.error(
      `[handleSearchMSTeamsMessages] Error: ${error.message}`,
      error
    );
    return {
      ok: false,
...
async function resolveMessageReferenceToIdentifiers(
  ...
  logger.debug(
    `[resolveMessageReferenceToIdentifiers] Attempting to resolve reference: "${reference}"`
  );

  const teamsPermalinkRegex =
  ...
  if (permalinkMatch) {
    ...
    logger.info(
      `[resolveMessageReferenceToIdentifiers] Parsed from permalink: Channel/Chat ID: ${channelOrChatId}, Message ID: ${messageId}`
    );
    ...
    if (
      ...
      if (
        channelOrChatId.startsWith('19:') &&
        channelOrChatId.includes('@thread.tacv2')
      ) {
        // Likely a channel
        logger.warn(
          `[resolveMessageReferenceToIdentifiers] Permalink gives channel-like ID (${channelOrChatId}) but teamId is missing for full context.`
        );
      ...
      logger.warn(
        `[resolveMessageReferenceToIdentifiers] Could not reliably determine full context (chatId vs teamId/channelId) from permalink fragment.`
      );
  ...
  if (reference.includes(':') || reference.includes('/')) {
    ...
    if (reference.length > 50) {
      ...
      logger.warn(
        `[resolveMessageReferenceToIdentifiers] Assuming reference "${reference}" is a direct messageID. Context (chat/channel) might be missing for precise lookup.`
      );
  ...
  logger.warn(
    `[resolveMessageReferenceToIdentifiers] Natural language reference resolution for "${reference}" is not yet implemented. Please use a permalink or message ID with context.`
  );
  return null;
}
...
> {
  logger.info(
    `[handleExtractInfoFromMSTeams] User: ${userId}, MessageRef: "${messageReferenceText}", Keywords: [${informationKeywords.join(', ')}]`
  );
  ...
  try {
    if (!messageContent) {
      const identifiers = await resolveMessageReferenceToIdentifiers(
        userId,
        messageReferenceText
      );
      if (!identifiers || !identifiers.messageId) {
        throw new Error(
          `Could not identify the specific MS Teams message from your reference: "${messageReferenceText}". Please use a permalink or provide more specific identifiers.`
        );
      }

      // Construct the GetMSTeamsMessageDetailInput based on what resolveMessageReferenceToIdentifiers could parse
      const detailInput: GetMSTeamsMessageDetailInput = {
      ...
      if (teamsMessage && teamsMessage.content) {
        ...
        throw new Error(
          `Could not find or read the content of the MS Teams message (ID: ${identifiers.messageId}).`
        );
      }
    }
    ...
    const extractedInfo = await extractInformationFromMSTeamsMessage(
      messageContent,
      informationKeywords
    );
    ...
    logger.error(
      `[handleExtractInfoFromMSTeams] Error: ${error.message}`,
      error
    );
    return {
      ok: false,
