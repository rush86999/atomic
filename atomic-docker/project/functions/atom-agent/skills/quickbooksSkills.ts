...
function getOAuthClient(): OAuthClient {
  if (!oauthClientInstance) {
    if (!ATOM_QB_CLIENT_ID || !ATOM_QB_CLIENT_SECRET || !ATOM_QB_REDIRECT_URI) {
      throw new Error(
        'QuickBooks OAuth client credentials (ID, Secret, Redirect URI) not configured.'
      );
    }
...
export function getAuthUri(stateCSRFToken: string): QBSkillResponse<string> {
  if (
    ...
    const errorMsg =
      'QuickBooks OAuth client credentials or scopes for Auth URI generation not configured.';
  ...
  if (
    ...
    const errorMsg =
      'A valid state CSRF token must be provided for generating the Auth URI.';
    console.error(errorMsg);
    return {
      ok: false,
...
async function getStoredQBTokens(
  userId: string
): Promise<QBSkillResponse<QuickBooksAuthTokens | null>> {
  console.log(
    `Retrieving QBO tokens for userId: ${userId}, service: ${QBO_SERVICE_NAME}`
  );
  ...
  try {
    ...
    }>(query, variables, operationName, userId);
    if (
      !response ||
    ...
    console.error(
      `Exception during getStoredQBTokens for userId ${userId}:`,
      error
    );
...
async function saveQBTokens(
  userId: string,
  tokenDataFromOAuth: OAuthClient.TokenResponseData
): Promise<QBSkillResponse<void>> {
  ...
  const accessTokenExpiresAt =
    tokenObtainedAt + tokenDataFromOAuth.expires_in! * 1000;
  const refreshTokenExpiresAt =
    tokenObtainedAt + tokenDataFromOAuth.x_refresh_token_expires_in! * 1000;
...
async function getValidTokens(
  userId: string
): Promise<QBSkillResponse<QuickBooksAuthTokens>> {
  ...
  if (now >= currentTokens.accessTokenExpiresAt - 5 * 60 * 1000) {
    console.log(
      `QBO access token for user ${userId} expired/nearing expiry. Refreshing...`
    );
    if (
    ...
    try {
      ...
      const refreshedTokenResponse = await authClient.refreshUsingToken(
        currentTokens.refreshToken
      );
      if (
      ...
      const errorDetails =
        e.originalError?.error_description ||
        e.message ||
...
async function getQboClient(
  userId: string
): Promise<QBSkillResponse<QuickBooks>> {
...
export async function listQuickBooksInvoices(
  ...
  if (options?.status) {
    switch (options.status) {
      ...
      case 'Overdue':
        conditions.push(
          `DueDate < '${new Date().toISOString().split('T')[0]}' AND Balance > 0`
        );
        break;
      case 'Pending':
...
export async function getQuickBooksInvoiceDetails(
  userId: string,
  invoiceId: string
): Promise<QBSkillResponse<QuickBooksInvoice | null>> {
  const qboClientResponse = await getQboClient(userId);
  if (!qboClientResponse.ok || !qboClientResponse.data) {
    return {
      ok: false,
...
export async function handleQuickBooksCallback(
  ...
  if (!HASURA_GRAPHQL_URL || !HASURA_ADMIN_SECRET) {
    return {
      ...
      error: {
        code: 'CONFIG_ERROR',
        message:
          'GraphQL client is not configured for QBO token saving during callback.',
      },
    };
  }
  ...
  try {
    const callbackParams = new URLSearchParams(
      urlWithCode.substring(urlWithCode.indexOf('?'))
    );
    ...
    if (originalState) {
      if (!receivedState || receivedState !== originalState) {
        console.error(
          `QBO OAuth callback state mismatch for user ${userId}. Expected: '${originalState}', Received: '${receivedState}'`
        );
        return {
          ok: false,
          error: {
      ...
      console.warn(
        `QBO OAuth callback for user ${userId}: originalState not provided. CSRF protection is weakened. Received state: '${receivedState}'`
      );
      // Depending on security policy, you might choose to fail here if originalState is mandatory.
    }
  } catch (e: any) {
    console.error(
      `Error parsing state from QBO callback URL for user ${userId}: ${urlWithCode}`,
      e
    );
    return {
      ok: false,
  ...
  try {
    ...
    if (!authResponse || !authResponse.getJson()?.access_token) {
      console.error(
        'QBO OAuth createToken response was invalid or missing access_token for user:',
        userId,
      ...
      return {
        ...
        error: {
          ...
          message:
            'Failed to exchange authorization code for token: Invalid response from server.',
  ...
  if (!tokenDataFromOAuth.realmId) {
    console.error(
      `QBO OAuth callback for user ${userId} did not return realmId.`,
      tokenDataFromOAuth
    );
    return {
      ok: false,
  ...
  // 8. Success
  console.log(
    `QuickBooks authorization successful and tokens saved for user ${userId}, realmId ${tokenDataFromOAuth.realmId}.`
  );
  return {
    ok: true,
    data: {
