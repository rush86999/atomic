...
function mapStripePaymentIntentToInternal(
...
export async function listStripePayments(
  ...
  try {
    ...
    const mappedPayments = paymentIntentsResponse.data.map(
      mapStripePaymentIntentToInternal
    );
...
export async function getStripePaymentDetails(
...
): Promise<StripeSkillResponse<StripePaymentIntent | null>> {
  ...
  try {
    ...
    console.error(
      `Error getting Stripe payment details for ID ${paymentIntentId}:`,
      error
    );
