import { ListGoogleMeetEventsResponse, GetGoogleMeetEventDetailsResponse } from '../../types';
import { listUpcomingGoogleMeetEvents as listEvents, getGoogleMeetEventDetails as getDetails } from './calendarSkills';
import { handleError } from '../../_utils/errorHandler';

export async function handleListGoogleMeetEvents(userId: string, entities: any): Promise<string> {
    try {
        const limit = entities?.limit && typeof entities.limit === 'number' ? entities.limit : 5;
        const response: ListGoogleMeetEventsResponse = await listEvents(userId, limit);
...
export async function handleGetGoogleMeetEventDetails(userId: string, entities: any): Promise<string> {
    try {
        const { event_id } = entities;
        if (!event_id || typeof event_id !== 'string') {
            return "Google Calendar Event ID is required to get details via NLU.";
        } else {
            const response: GetGoogleMeetEventDetailsResponse = await getDetails(userId, event_id);
