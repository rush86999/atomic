...
export async function generateLearningPlan(userId: string, notionDatabaseId: string): Promise<void> {
    const articles = await getPocketArticles(userId);

    const articleTitles = articles.map(article => article.resolved_title).join('\n');
