...
export async function handleStartInPersonAudioNoteDirect(
  ...
  console.log(
    `Agent Skill (Direct): Handling START_IN_PERSON_AUDIO_NOTE for user ${context.userId}`
  );
  ...
  const noteTitleFromEntities = nluResult.entities?.NOTE_TITLE as
    | string
    | undefined;
  const originalNoteTitleProvided =
    noteTitleFromEntities && noteTitleFromEntities.trim() !== '';
  ...
  const calendarEventContextFromEntities = nluResult.entities
    ?.CALENDAR_EVENT_CONTEXT as string | undefined;
  if (
    calendarEventContextFromEntities &&
    calendarEventContextFromEntities.trim() !== ''
  ) {
    const contextQuery = calendarEventContextFromEntities.trim().toLowerCase();
    console.log(
      `Agent Skill (Direct): Calendar event context found: "${contextQuery}". Attempting resolution.`
    );
    ...
    try {
      ...
      const eventsResponse = await listUpcomingEvents(
        context.userId,
        15,
        timeMin,
        timeMax
      ...
      if (eventsResponse.ok && eventsResponse.data) {
        if (eventsResponse.data.length > 0) {
          ...
          const matchingEvents = allFetchedEvents.filter(
            (event) =>
              (event.summary &&
          ...
          if (matchedEvent) {
            ...
            console.log(
              `Agent Skill (Direct): Resolved calendar context "${contextQuery}" to event: "${resolvedEventSummary}" (ID: ${linkedEventId})`
            );
            ...
            console.log(
              `Agent Skill (Direct): Could not resolve calendar context "${contextQuery}" to a specific event.`
            );
          ...
          console.log(
            'Agent Skill (Direct): No calendar events found in the time window for context resolution.'
          );
        ...
        console.warn(
          'Agent Skill (Direct): Failed to fetch calendar events for context resolution:',
      ...
      console.error(
        'Agent Skill (Direct): Error during calendar event context resolution:',
        error.message,
        error.stack
      );
      calendarFeedbackMessage =
  ...
  try {
    ...
    console.log(
      'Agent Skill (Direct): Sent START_RECORDING_SESSION command to client:',
      startCommand
    );
    ...
    console.error(
      'Agent Skill (Direct): Error sending START_RECORDING_SESSION command to client:',
      error.message
    );
    return {
      success: false,
      message:
        "Sorry, I couldn't initiate the audio recording session due to an internal communication error.",
    };
  }
}
...
export async function handleStopInPersonAudioNoteDirect(
  ...
  console.log(
    `Agent Skill (Direct): Handling STOP_IN_PERSON_AUDIO_NOTE for user ${context.userId}`
  );
  ...
  try {
    ...
    console.log(
      'Agent Skill (Direct): Sent STOP_RECORDING_SESSION command to client:',
      stopCommand
    );
    ...
    return {
      success: true,
      message:
        "Okay, I've sent the command to stop the recording. The audio should be processed shortly if recording was active.",
    };
  } catch (error: any) {
    console.error(
      'Agent Skill (Direct): Error sending STOP_RECORDING_SESSION command to client:',
      error.message
    );
    return {
      success: false,
      message:
        "Sorry, I couldn't send the stop command due to an internal communication error.",
    };
  }
}
...
export async function handleCancelInPersonAudioNoteDirect(
  ...
  console.log(
    `Agent Skill (Direct): Handling CANCEL_IN_PERSON_AUDIO_NOTE for user ${context.userId}`
  );
  ...
  try {
    ...
    console.log(
      'Agent Skill (Direct): Sent CANCEL_RECORDING_SESSION command to client:',
      cancelCommand
    );
    ...
    console.error(
      'Agent Skill (Direct): Error sending CANCEL_RECORDING_SESSION command to client:',
      error.message
    );
    return {
      success: false,
      message:
        "Sorry, I couldn't send the cancel command due to an internal communication error.",
    };
  }
}
...
export const InPersonAudioNoteDirectSkillManifest = {
  ...
  description:
    'Handles voice commands to record in-person audio notes with direct client control.',
};
