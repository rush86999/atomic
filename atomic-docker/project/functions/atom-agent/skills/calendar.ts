...
export async function handleGetCalendarEvents(userId: string, entities: any, integrations: any): Promise<string> {
    try {
        ...
        if (time_query) console.log(`[handleGetCalendarEvents] time_query found - ${time_query}`);
        if (query_type) console.log(`[handleGetCalendarEvents] query_type found - ${query_type}`);
