...
export async function getRepoCommitActivity(userId: string, owner: string, repo: string): Promise<any> {
...
export async function getRepoPullRequestActivity(userId: string, owner: string, repo: string): Promise<any> {
    const apiKey = await getGithubApiKey(userId);
    if (!apiKey) {
        throw new Error("GitHub API key not configured for this user.");
    }
