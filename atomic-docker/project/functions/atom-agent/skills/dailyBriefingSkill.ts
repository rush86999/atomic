  ...
  if (input === 'today') {
    ...
    if (yyyyMmDdMatch) {
      ...
      if (
        ...
        logger.warn(
          `[dailyBriefingSkill] Invalid YYYY-MM-DD date string: ${input}`
        );
      }
    }
    ...
    if (!successfullyParsed) {
      ...
      if (relativeMatch) {
        ...
        if (direction === 'next') {
          ...
          determinedTargetDate.setUTCDate(
            determinedTargetDate.getUTCDate() + daysToAdd
          );
        } else {
          // "last"
          ...
          determinedTargetDate.setUTCDate(
            determinedTargetDate.getUTCDate() - daysToSubtract
          );
        }
        ...
        if (input.includes(' at ')) {
          logger.info(
            `[dailyBriefingSkill] Time part in "${originalInput}" is currently ignored. Using start of day.`
          );
        }
      }
    }
    ...
    if (!successfullyParsed) {
      ...
      if (monthDayMatch) {
        ...
        if (month !== undefined && day >= 1 && day <= 31) {
          ...
          if (
            determinedTargetDate.getUTCFUTCDate() !== day ||
            determinedTargetDate.getUTCMonth() !== month ||
            determinedTargetDate.getUTCFullYear() !== year
          ) {
            logger.warn(
              `[dailyBriefingSkill] Invalid Month-Day-Year combination: ${input}`
            );
          } else {
            ...
            if (
              ...
              if (determinedTargetDate.getUTCMonth() !== month) {
                ...
                logger.warn(
                  `[dailyBriefingSkill] Invalid Month-Day for next year: ${input}`
                );
          ...
          logger.warn(
            `[dailyBriefingSkill] Could not parse month/day from: ${input}`
          );
        }
      }
    }
  ...
  switch (item.type) {
    case 'meeting':
      ...
      if (meeting && meeting.start) {
        ...
        if (getUTCDateYYYYMMDD(meetingStartDate) === targetDateISO) {
          ...
          const hoursFromStartOfDayToMeetingStart =
            meetingStartDate.getUTCHours() +
            meetingStartDate.getUTCMinutes() / 60;
          const timeProximityScore = Math.max(
            0,
            (24 - hoursFromStartOfDayToMeetingStart) * 2.5
          );
          score += Math.min(timeProximityScore, 40); // Cap time proximity bonus

          if (
            targetDateISO === getUTCDateYYYYMMDD(nowForContext) &&
            meetingStartDate > nowForContext
          ) {
            const hoursUntilMeeting =
              (meetingStartDate.getTime() - nowForContext.getTime()) /
              (1000 * 60 * 60);
            if (hoursUntilMeeting < 1) score += 5;
            else if (hoursUntilMeeting < 3) score += 3;
          }
    ...
    case 'task':
      ...
      if (task) {
        ...
        const priorityBonusMap: {
          [key in NotionTaskPriority | string]: number;
        ...
        if (task.dueDate) {
          ...
          if (dueDateOnlyISO < targetDateOnlyISO) {
            ...
            const diffDays =
              (dueDateObj.getTime() - targetDateObj.getTime()) /
              (1000 * 60 * 60 * 24);
          ...
          if (activityDateStr) {
            try {
              ...
              const daysSinceActivity =
                (nowForContext.getTime() - activityDate.getTime()) /
                (1000 * 60 * 60 * 24);
              ...
              logger.warn(
                `[calculateUrgencyScore] Could not parse task activity date: ${activityDateStr}`
              );
            }
          }
        }
    ...
    case 'email':
      ...
      if (email && email.date) {
        ...
        if (targetDateISO === getUTCDateYYYYMMDD(nowForContext)) {
          const hoursAgoReceived =
            (nowForContext.getTime() - emailDate.getTime()) / (1000 * 60 * 60);
    ...
    case 'slack_message':
      ...
      if (slackMsg && slackMsg.timestamp) {
        try {
          ...
          if (targetDateISO === getUTCDateYYYYMMDD(nowForContext)) {
            const hoursAgoReceived =
              (nowForContext.getTime() - messageDate.getTime()) /
              (1000 * 60 * 60);
            if (hoursAgoReceived >= 0 && hoursAgoReceived < 2) score += 5;
          }
        } catch (e) {
          logger.warn(
            `[calculateUrgencyScore] Could not parse Slack message timestamp: ${slackMsg.timestamp}`
          );
        }
      }
      break;
    case 'teams_message':
      ...
      if (teamsMsg && teamsMsg.createdDateTime) {
        try {
          ...
          logger.warn(
            `[calculateUrgencyScore] Could not parse MS Teams message createdDateTime: ${teamsMsg.createdDateTime}`
          );
        }
      }
      break;
...
function getFriendlyDateString(
  date: Date,
  baseDateForComparison: Date = new Date()
): string {
...
export async function generateDailyBriefing(
  ...
  logger.info(
    `[dailyBriefingSkill] Generating daily briefing for user ${userId} for dateContext: "${nluEntities.date_context || 'today'}", resolved to: ${parsedDateInfo.targetDateISO}`
  );
  logger.debug(
    `[dailyBriefingSkill] NLU Entities: ${JSON.stringify(nluEntities)}`
  );
  logger.debug(
    `[dailyBriefingSkill] Parsed Date Info: ${JSON.stringify(parsedDateInfo)}`
  );
  ...
  try {
    ...
    if (focusAreas.includes('tasks')) {
      logger.info(
        `[dailyBriefingSkill] Fetching tasks for briefing for targetDate: ${parsedDateInfo.targetDateISO}.`
      );
      if (!ATOM_NOTION_TASKS_DATABASE_ID) {
        logger.error(
          '[dailyBriefingSkill] ATOM_NOTION_TASKS_DATABASE_ID is not configured. Cannot fetch tasks.'
        );
        briefingData.errors_encountered?.push({
          source_area: 'tasks',
        ...
        try {
          ...
          const [overdueTasksResponse, dueOnTargetDateTasksResponse] =
            await Promise.all([
          ...
          if (overdueTasksResponse.success && overdueTasksResponse.tasks) {
            ...
            logger.error(
              `[dailyBriefingSkill] Error fetching overdue tasks: ${overdueTasksResponse.error}`
            );
            briefingData.errors_encountered?.push({
              source_area: 'tasks',
          ...
          if (
            dueOnTargetDateTasksResponse.success &&
            ...
            logger.error(
              `[dailyBriefingSkill] Error fetching tasks due on ${parsedDateInfo.targetDateISO}: ${dueOnTargetDateTasksResponse.error}`
            );
            briefingData.errors_encountered?.push({
              source_area: 'tasks',
          ...
          if (fetchedTasks.length > 0) {
            fetchedTasks.sort((a, b) => {
              const aIsOverdue =
                a.dueDate && a.dueDate < parsedDateInfo.targetDateISO;
              const bIsOverdue =
                b.dueDate && b.dueDate < parsedDateInfo.targetDateISO;
            ...
            logger.info(
              `[dailyBriefingSkill] Fetched and processed ${fetchedTasks.length} tasks.`
            );
          } else {
            logger.info(
              `[dailyBriefingSkill] No relevant tasks found for briefing.`
            );
          }
        } catch (e: any) {
          logger.error(
            `[dailyBriefingSkill] Exception during task fetching: ${e.message}`,
            e
          );
          briefingData.errors_encountered?.push({
            source_area: 'tasks',
      ...
      briefingData.priority_items = briefingData.priority_items.filter(
        (item) => item.type !== 'task'
      );
    }
    ...
    if (focusAreas.includes('meetings')) {
      logger.info(
        `[dailyBriefingSkill] Fetching meetings for briefing for targetDate: ${parsedDateInfo.targetDateISO}.`
      );
      try {
        // Use timeMinISO and timeMaxISO from parsedDateInfo
        const eventsResponse = await calendarSkills.listUpcomingEvents(
          userId,
          10,
          parsedDateInfo.timeMinISO,
          parsedDateInfo.timeMaxISO
        );
        ...
        if (eventsResponse.ok && eventsResponse.data) {
          ...
          if (meetings.length > 0) {
            ...
            logger.info(
              `[dailyBriefingSkill] Fetched ${meetings.length} meetings.`
            );
          } else {
            logger.info(
              `[dailyBriefingSkill] No meetings found for ${dateContext}.`
            );
          }
        } else {
          logger.error(
            `[dailyBriefingSkill] Error fetching meetings: ${eventsResponse.error?.message}`
          );
          briefingData.errors_encountered?.push({
        ...
        logger.error(
          `[dailyBriefingSkill] Exception during meeting fetching: ${e.message}`,
          e
        );
        briefingData.errors_encountered?.push({
          source_area: 'meetings',
    ...
    if (focusAreas.includes('urgent_emails')) {
      logger.info(
        `[dailyBriefingSkill] Fetching urgent emails for briefing for targetDate: ${parsedDateInfo.targetDateISO}.`
      );
      try {
        const emailResponse =
          await gmailSkills.getRecentUnreadEmailsForBriefing(
            userId,
        ...
        if (emailResponse.ok && emailResponse.data?.results) {
          ...
          if (emails.length > 0) {
            ...
            logger.info(
              `[dailyBriefingSkill] Fetched ${emails.length} urgent/recent unread emails.`
            );
          } else {
            logger.info(
              `[dailyBriefingSkill] No urgent/recent unread emails found for ${parsedDateInfo.targetDateISO}.`
            );
          }
        } else {
          logger.error(
            `[dailyBriefingSkill] Error fetching urgent emails: ${emailResponse.error?.message}`
          );
          briefingData.errors_encountered?.push({
            source_area: 'emails',
            message:
        ...
        logger.error(
          `[dailyBriefingSkill] Exception during email fetching: ${e.message}`,
          e
        );
        briefingData.errors_encountered?.push({
          source_area: 'emails',
    ...
    if (focusAreas.includes('urgent_slack_messages')) {
      logger.info(
        `[dailyBriefingSkill] Fetching urgent Slack messages for briefing for targetDate: ${parsedDateInfo.targetDateISO}.`
      );
      try {
        const slackResponse =
          await slackSkills.getRecentDMsAndMentionsForBriefing(
            userId, // atomUserId
        ...
        if (slackResponse.ok && slackResponse.data?.results) {
          const slackMessages: SlackMessageSnippet[] =
            slackResponse.data.results; // SlackMessage is compatible with SlackMessageSnippet for these fields
          if (slackMessages.length > 0) {
            ...
            logger.info(
              `[dailyBriefingSkill] Fetched ${slackMessages.length} urgent/recent Slack messages.`
            );
          } else {
            logger.info(
              `[dailyBriefingSkill] No urgent/recent Slack messages found for ${parsedDateInfo.targetDateISO}.`
            );
          }
        } else {
          logger.error(
            `[dailyBriefingSkill] Error fetching urgent Slack messages: ${slackResponse.error?.message}`
          );
          briefingData.errors_encountered?.push({
            source_area: 'slack',
            message:
              slackResponse.error?.message ||
        ...
        logger.error(
          `[dailyBriefingSkill] Exception during Slack message fetching: ${e.message}`,
          e
        );
        briefingData.errors_encountered?.push({
          source_area: 'slack',
    ...
    if (focusAreas.includes('urgent_teams_messages')) {
      logger.info(
        `[dailyBriefingSkill] Fetching urgent MS Teams messages for briefing for targetDate: ${parsedDateInfo.targetDateISO}.`
      );
      try {
        // Assuming getRecentChatsAndMentionsForBriefing exists and works similarly to Slack's
        const teamsResponse =
          await teamsSkills.getRecentChatsAndMentionsForBriefing(
            userId,
            parsedDateInfo.targetDate,
        ...
        if (teamsResponse.ok && teamsResponse.data?.results) {
          ...
          if (teamsMessages.length > 0) {
            ...
            logger.info(
              `[dailyBriefingSkill] Fetched ${teamsMessages.length} urgent/recent MS Teams messages.`
            );
          } else {
            logger.info(
              `[dailyBriefingSkill] No urgent/recent MS Teams messages found for ${parsedDateInfo.targetDateISO}.`
            );
          }
        } else {
          logger.error(
            `[dailyBriefingSkill] Error fetching urgent MS Teams messages: ${teamsResponse.error?.message}`
          );
          briefingData.errors_encountered?.push({
        ...
        logger.error(
          `[dailyBriefingSkill] Exception during MS Teams message fetching: ${e.message}`,
          e
        );
        briefingData.errors_encountered?.push({
          source_area: 'teams',
    ...
    briefingData.priority_items.sort((a, b) => {
      ...
      if (a.type === 'task' && b.type === 'task') {
        ...
        const aDueDate = (a.raw_item as NotionTask)?.dueDate
          ? new Date((a.raw_item as NotionTask).dueDate!).getTime()
          : Infinity;
        const bDueDate = (b.raw_item as NotionTask)?.dueDate
          ? new Date((b.raw_item as NotionTask).dueDate!).getTime()
          : Infinity;
      ...
      if (a.type === 'email' && b.type === 'email') {
        const aDate = (a.raw_item as GmailMessageSnippet)?.date
          ? new Date((a.raw_item as GmailMessageSnippet).date!).getTime()
          : 0;
        const bDate = (b.raw_item as GmailMessageSnippet)?.date
          ? new Date((b.raw_item as GmailMessageSnippet).date!).getTime()
          : 0;
    ...
    logger.info(
      `[dailyBriefingSkill] Sorted ${briefingData.priority_items.length} priority items.`
    );

    // Generate overall_summary_message
    // Generate overall_summary_message
    const numTasks = briefingData.priority_items.filter(
      (item) => item.type === 'task'
    ).length;
    const numMeetings = briefingData.priority_items.filter(
      (item) => item.type === 'meeting'
    ).length;
    const numEmails = briefingData.priority_items.filter(
      (item) => item.type === 'email'
    ).length;
    const numSlackMessages = briefingData.priority_items.filter(
      (item) => item.type === 'slack_message'
    ).length;
    const numTeamsMessages = briefingData.priority_items.filter(
      (item) => item.type === 'teams_message'
    ).length;
    ...
    if (focusAreas.includes('tasks')) {
      if (numTasks > 0) {
        const overdueTasks = briefingData.priority_items.filter(
          (item) =>
            item.type === 'task' &&
            item.raw_item &&
    ...
    if (focusAreas.includes('urgent_slack_messages')) {
      if (numSlackMessages > 0) {
        summaryParts.push(
          `${numSlackMessages} recent Slack message(s) (DMs/mentions).`
        );
      } else {
    ...
    if (focusAreas.includes('urgent_teams_messages')) {
      if (numTeamsMessages > 0) {
        summaryParts.push(
          `${numTeamsMessages} recent MS Teams message(s) (chats/mentions).`
        );
      } else {
    ...
    if (
      parsedDateInfo.status === 'unparseable' &&
      ...
      parsedDateInfo.status === 'defaulted' &&
    ...
    logger.info(
      `[dailyBriefingSkill] Generated summary: ${briefingData.overall_summary_message}`
    );

    return { ok: true, data: briefingData };
  } catch (error: any) {
    logger.error(
      `[dailyBriefingSkill] Error generating daily briefing: ${error.message}`,
      error
    );
    briefingData.errors_encountered?.push({
      source_area: 'overall',
