import { ListStripePaymentsResponse, GetStripePaymentDetailsResponse } from '../../types';
import { listStripePayments as list, getStripePaymentDetails as getDetails } from './stripeSkills';
import { handleError } from '../../_utils/errorHandler';

export async function handleListStripePayments(userId: string, entities: any): Promise<string> {
    try {
...
export async function handleGetStripePaymentDetails(userId: string, entities: any): Promise<string> {
    try {
        const { payment_intent_id } = entities;
        if (!payment_intent_id || typeof payment_intent_id !== 'string') {
            return "Stripe PaymentIntent ID is required to get details via NLU.";
        } else {
            const response: GetStripePaymentDetailsResponse = await getDetails(payment_intent_id);
