...
import {
  listCalendlyEventTypes as listTypes,
  listCalendlyScheduledEvents as listScheduled,
} from './calendlySkills';
import { handleError } from '../../_utils/errorHandler';

export async function handleListCalendlyEventTypes(
  userId: string,
  entities: any
): Promise<string> {
  try {
    const calendlyUserId =
      entities?.user_id && typeof entities.user_id === 'string'
        ? entities.user_id
        : userId;
    const response: ListCalendlyEventTypesResponse =
      await listTypes(calendlyUserId);
    ...
    return handleError(
      error,
      'Sorry, an unexpected error occurred while fetching your Calendly event types (NLU path).'
    );
  }
}

export async function handleListCalendlyScheduledEvents(
  userId: string,
  entities: any
): Promise<string> {
  try {
    const calendlyUserId =
      entities?.user_id && typeof entities.user_id === 'string'
        ...
        : userId;
    const count =
      entities?.count && typeof entities.count === 'number'
        ? entities.count
        : 10;
    const status =
      entities?.status && typeof entities.status === 'string'
        ? (entities.status as 'active' | 'canceled')
        : 'active';
    const sort =
      entities?.sort && typeof entities.sort === 'string'
        ? entities.sort
        : 'start_time:asc';
    const options = { count, status, sort, user: calendlyUserId };
    const response: ListCalendlyScheduledEventsResponse = await listScheduled(
      calendlyUserId,
      options
    );
    if (response.ok && response.collection && response.collection.length > 0) {
    ...
    return handleError(
      error,
      'Sorry, an unexpected error occurred while fetching your Calendly bookings (NLU path).'
    );
  }
}
