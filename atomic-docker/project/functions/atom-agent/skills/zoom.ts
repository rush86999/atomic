...
import {
  listZoomMeetings as list,
  getZoomMeetingDetails as getDetails,
} from './zoomSkills';
import { handleError } from '../../_utils/errorHandler';

export async function handleListZoomMeetings(
  userId: string,
  entities: any
): Promise<string> {
  try {
    ...
    const type = (
      entities?.type && typeof entities.type === 'string'
        ? entities.type
        : 'upcoming'
    ) as
      | 'live'
      ...
      | 'previous_meetings';
    const page_size =
      entities?.page_size && typeof entities.page_size === 'number'
        ? entities.page_size
        : 30;
    const next_page_token =
      entities?.next_page_token && typeof entities.next_page_token === 'string'
        ? entities.next_page_token
        : undefined;
    const options = { type, page_size, next_page_token };
    const response: ListZoomMeetingsResponse = await list(
      userIdForZoom,
      options
    );
...
export async function handleGetZoomMeetingDetails(
  userId: string,
  entities: any
): Promise<string> {
  try {
    ...
    if (!meeting_id || typeof meeting_id !== 'string') {
      ...
      const response: GetZoomMeetingDetailsResponse =
        await getDetails(meeting_id);
    ...
    return handleError(
      error,
      `Sorry, an unexpected error occurred while fetching details for Zoom meeting ${entities.meeting_id} (NLU path).`
    );
  }
}
