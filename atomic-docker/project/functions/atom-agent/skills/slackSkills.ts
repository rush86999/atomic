...
async function _enrichSlackMessagesWithNames(
  ...
  if (userIdsToFetch.size > 0) {
    logger.debug(
      `[SlackSkills_Enrich] Fetching info for ${userIdsToFetch.size} user/bot IDs.`
    );
      ...
      Array.from(userIdsToFetch).map(async (uid) => {
        ...
        try {
          ...
          if (response.ok && response.user) {
            userCache.set(
              uid,
              response.user.real_name || response.user.name || uid
            );
          } else {
            ...
            if (uid.startsWith('B')) {
              ...
              try {
                ...
                if (botInfo.ok && botInfo.bot) {
                  ...
                  logger.warn(
                    `[SlackSkills_Enrich] bots.info call not ok for bot ${uid}: ${botInfo.error}`
                  );
                  userCache.set(uid, uid);
                }
              } catch (botError: any) {
                logger.warn(
                  `[SlackSkills_Enrich] Failed to fetch info for bot ${uid} via bots.info: ${botError.message}`
                );
                userCache.set(uid, uid);
              }
            } else {
              logger.warn(
                `[SlackSkills_Enrich] users.info call not ok for ${uid}: ${response.error}`
              );
              userCache.set(uid, uid);
            }
          }
          ...
          logger.warn(
            `[SlackSkills_Enrich] Failed to fetch info for user/bot ${uid}: ${e.message}`
          );
          userCache.set(uid, uid);
        }
      })
  ...
  if (channelIdsToFetch.size > 0) {
    logger.debug(
      `[SlackSkills_Enrich] Fetching info for ${channelIdsToFetch.size} channel IDs.`
    );
    await Promise.allSettled(
      Array.from(channelIdsToFetch).map(async (cid) => {
        ...
        try {
          ...
          if (convInfo.ok && convInfo.channel) {
            ...
            if (
              !cName &&
              (convInfo.channel as any).is_im &&
              (convInfo.channel as any).user
            ) {
              ...
              if (userCache.has(otherUserId)) {
                ...
                try {
                  ...
                  logger.warn(
                    `[SlackSkills_Enrich] Failed to fetch user info for DM partner ${otherUserId} in channel ${cid}`
                  );
                  cName = otherUserId;
                }
              }
            ...
            logger.warn(
              `[SlackSkills_Enrich] conversations.info call not ok for ${cid}: ${convInfo.error}`
            );
            channelCache.set(cid, cid);
          }
        } catch (e: any) {
          logger.warn(
            `[SlackSkills_Enrich] Failed to fetch info for channel ${cid}: ${e.message}`
          );
          channelCache.set(cid, cid);
        }
      })
...
export async function listSlackChannels(
  ...
  logger.debug(
    `[SlackSkills] listSlackChannels called by userId: ${userId}, limit: ${limit}, cursor: ${cursor}`
  );
  ...
  try {
    ...
    if (!result.ok || !result.channels) {
      ...
      logger.error(
        `[SlackSkills] Slack API error while listing channels (result.ok is false): ${slackError}`
      );
      return {
        ok: false,
        error: {
    ...
    logger.error(
      `[SlackSkills] Error listing Slack channels for userId ${userId}:`,
      error
    );
...
async function getChannelIdByNameOrUser(
  ...
  logger.debug(
    `[SlackSkills] getChannelIdByNameOrUser called for identifier: ${identifier}`
  );
    ...
    try {
      while (attempts < 10) {
        ...
        if (!response.ok || !response.data?.channels) {
          logger.error(
            `[SlackSkills] Error fetching channels to find ID for "#${channelName}":`,
            response.error
          );
          return {
            ok: false,
        ...
        const foundChannel = response.data.channels.find(
          (ch) => ch.name === channelName && (ch.is_channel || ch.is_group)
        );
      ...
      logger.error(
        `[SlackSkills] Exception in getChannelIdByNameOrUser for "#${channelName}":`,
        error
      );
      return {
        ok: false,
    ...
    try {
      if (isSlackId(userNameOrId)) {
        ...
        if (dmResponse.ok && dmResponse.channel?.id) {
          ...
          logger.error(
            `[SlackSkills] Failed to open DM with user ID "${userNameOrId}": ${dmResponse.error}`
          );
          return {
            ok: false,
            error: {
      ...
      logger.warn(
        `[SlackSkills] Identifier "${identifier}" for DM is not a user ID. Robust name-to-ID resolution for DMs is not implemented in this helper.`
      );
      return {
        ok: true,
        data: null,
      ...
      logger.error(
        `[SlackSkills] Exception trying to open DM for "${identifier}":`,
        error
      );
      return {
        ok: false,
  ...
  logger.warn(
    `[SlackSkills] Channel identifier "${identifier}" format not recognized for ID resolution (expected #channel or @user).`
  );
  return { ok: true, data: null };
}
...
export async function sendSlackMessage(
  ...
  logger.debug(
    `[SlackSkills] sendSlackMessage called by userId: ${userId} to channelIdentifier: ${channelIdentifier}, text: "${text.substring(0, 50)}..."`
  );
  const client = await getSlackClient(userId);
  if (!client) {
    return {
  ...
  if (!isSlackId(channelIdentifier)) {
    logger.debug(
      `[SlackSkills] Channel identifier "${channelIdentifier}" is not an ID. Attempting to resolve...`
    );
    const idResponse = await getChannelIdByNameOrUser(
      client,
      channelIdentifier,
      userId
    );
    if (!idResponse.ok) {
      logger.error(
        `[SlackSkills] Failed to resolve channel identifier "${channelIdentifier}":`,
        idResponse.error
      );
      return {
        ok: false,
    ...
    if (!idResponse.data) {
      logger.warn(
        `[SlackSkills] Channel identifier "${channelIdentifier}" not found or could not be resolved to an ID.`
      );
      return {
        ok: false,
        error: {
    ...
    logger.debug(
      `[SlackSkills] Resolved identifier "${channelIdentifier}" to ID "${targetChannelId}"`
    );
  }
  ...
  try {
    ...
    if (!result.ok || !result.ts) {
      ...
      logger.error(
        `[SlackSkills] Slack API error sending message to channel ${targetChannelId}: ${slackError}`
      );
      return {
        ok: false,
        error: {
    ...
    logger.info(
      `[SlackSkills] Message sent successfully to channel ${targetChannelId} by user ${userId}. TS: ${result.ts}`
    );
    return {
      ok: true,
      data: {
    ...
    logger.error(
      `[SlackSkills] Error sending Slack message for userId ${userId} to channel ${targetChannelId}:`,
      error
    );
    if (error instanceof SlackAPIError) {
      return {
...
export async function searchMySlackMessages(
  ...
  logger.debug(
    `[SlackSkills] searchMySlackMessages direct API call for Atom user ${atomUserId}, query: "${searchQuery}", limit: ${limit}`
  );

  const client = await getSlackClient(atomUserId);
  if (!client) {
    logger.error(
      '[SlackSkills] Slack client not available for searchMySlackMessages.'
    );
    return [];
  }
  ...
  try {
    ...
    if (!response.ok || !response.messages?.matches) {
      const slackError =
        (response as any).error || 'unknown_slack_search_error';
      logger.error(
        `[SlackSkills] Slack API error during search.messages: ${slackError}`,
        response
      );
      return [];
    }

    let results: SlackMessage[] = (response.messages.matches || []).map(
      (match: any): SlackMessage => {
        return {
          id: match.ts,
    ...
    if (results.length > 0) {
      ...
      results = await _enrichSlackMessagesWithNames(
        client,
        results,
        userCache,
        channelCache
    ...
    logger.info(
      `[SlackSkills] searchMySlackMessages direct API call found and enriched ${results.length} messages.`
    );
    return results;
  } catch (error: any) {
    logger.error(
      `[SlackSkills] Error in searchMySlackMessages direct API call for Atom user ${atomUserId}, query "${searchQuery}":`,
      error
    );
    if (error instanceof SlackAPIError) {
      logger.error(
        `[SlackSkills] SlackAPIError code: ${error.code}, data:`,
        error.data
      );
    }
    return [];
...
export async function readSlackMessage(
  ...
  logger.debug(
    `[SlackSkills] readSlackMessage direct API call for Atom user ${atomUserId}, channelId: ${channelId}, messageTs: ${messageTs}`
  );

  const client = await getSlackClient(atomUserId);
  if (!client) {
    logger.error(
      '[SlackSkills] Slack client not available for readSlackMessage.'
    );
    return null;
  }
  ...
  try {
    ...
    if (!response.ok || !response.messages || response.messages.length === 0) {
      const slackError =
        (response as any).error || 'message_not_found_or_access_denied';
      logger.warn(
        `[SlackSkills] Slack API error or message not found during conversations.history for ts ${messageTs} in channel ${channelId}: ${slackError}`,
        response
      );
      return null;
    }
    ...
    try {
      ...
      logger.warn(
        `[SlackSkills] Could not fetch permalink for message ${msgData.ts} in channel ${channelId}:`,
        permalinkError
      );
    }
    ...
    const userCache = new Map<string, string | null>();
    const channelCache = new Map<string, string | null>();
    const enrichedMessages = await _enrichSlackMessagesWithNames(
      client,
      [message],
      userCache,
      channelCache
    ...
    logger.info(
      `[SlackSkills] Successfully read and enriched Slack message ${messageTs} from channel ${channelId}.`
    );
    return enrichedMessages.length > 0 ? enrichedMessages[0] : null;
  } catch (error: any) {
    logger.error(
      `[SlackSkills] Error in readSlackMessage direct API call for Atom user ${atomUserId}, channel ${channelId}, ts ${messageTs}:`,
      error
    );
    if (error instanceof SlackAPIError) {
      logger.error(
...
function getSlackExtractionOpenAIClient(): OpenAI {
  ...
  if (!ATOM_OPENAI_API_KEY) {
    logger.error(
      '[SlackSkills] OpenAI API Key not configured for LLM Slack Extractor.'
    );
...
export async function extractInformationFromSlackMessage(
  ...
  if (!infoKeywords || infoKeywords.length === 0) {
    logger.debug(
      '[SlackSkills] extractInformationFromSlackMessage: No infoKeywords provided.'
    );
    return {};
  }
  if (!messageText || messageText.trim() === '') {
    logger.debug(
      '[SlackSkills] extractInformationFromSlackMessage: Empty messageText provided.'
    );
  ...
  logger.debug(
    `[SlackSkills] LLM Extractor: Attempting to extract from Slack message for keywords: [${infoKeywords.join(', ')}]`
  );
  const client = getSlackExtractionOpenAIClient();

  const keywordsJsonArrayString = JSON.stringify(infoKeywords);
  const systemPrompt = SLACK_EXTRACTION_SYSTEM_PROMPT_TEMPLATE.replace(
  ...
  try {
    ...
    if (!llmResponse) {
      logger.error(
        '[SlackSkills] LLM Extractor: Received an empty response from AI for Slack message.'
      );
      throw new Error('LLM Extractor (Slack): Empty response from AI.');
    }

    logger.debug(
      '[SlackSkills] LLM Extractor: Raw LLM JSON response for Slack:',
      llmResponse
    );
    ...
    logger.debug(
      '[SlackSkills] LLM Extractor: Parsed and reconciled extraction from Slack:',
      result
    );
    return result;
  } catch (error: any) {
    logger.error(
      '[SlackSkills] LLM Extractor: Error processing information extraction from Slack message with OpenAI:',
      error.message
    );
    if (error instanceof SyntaxError) {
      logger.error(
        '[SlackSkills] LLM Extractor: Failed to parse JSON response from LLM for Slack message.'
      );
      throw new Error(
        'LLM Extractor (Slack): Failed to parse response from AI.'
      );
    }
...
export async function getRecentDMsAndMentionsForBriefing(
  ...
  SlackSkillResponse<{ results: SlackMessage[]; query_executed?: string }>
> {
  logger.debug(
    `[SlackSkills] getRecentDMsAndMentionsForBriefing for Atom user: ${atomUserId}, TargetDate: ${targetDate.toISOString().split('T')[0]}, Count: ${count}`
  );
  ...
  try {
    ...
    try {
      ...
      if (authTestResponse.ok && authTestResponse.user_id) {
        ...
        logger.info(
          `[SlackSkills] Resolved Slack user ID for search: ${userSlackId}`
        );
      } else {
        logger.warn(
          `[SlackSkills] Could not resolve Slack user ID via auth.test. Mentions search might be impacted. Error: ${authTestResponse.error}`
        );
      }
    } catch (authError: any) {
      logger.warn(
        `[SlackSkills] Exception during client.auth.test: ${authError.message}. Mentions search might be impacted.`
      );
    }
    ...
    if (userSlackId) {
      querySegments.push(
        `(@${userSlackId} OR to:${userSlackId} OR in:${userSlackId})`
      );
    } else {
      querySegments.push(`(is:dm)`);
      logger.warn(
        '[SlackSkills] No specific Slack User ID for DMs/Mentions search, results might be less targeted if using a bot token without specific user context for DMs.'
      );
    }
    ...
    logger.info(
      `[SlackSkills] Constructed Slack search query for briefing: "${fullSearchQueryWithSort}"`
    );

    const searchResults: SlackMessage[] = await searchMySlackMessages(
      atomUserId,
      fullSearchQueryWithSort,
      count
    );

    logger.info(
      `[SlackSkills] Found ${searchResults.length} Slack messages for briefing.`
    );
    return {
      ok: true,
    ...
    logger.error(
      `[SlackSkills] Error in getRecentDMsAndMentionsForBriefing for Atom user ${atomUserId}: ${error.message}`,
      error
    );
    return {
      ok: false,
      error: {
        ...
        message:
          error.message || 'Failed to fetch recent DMs/mentions for briefing.',
      },
    };
  }
}
...
export async function getSlackMessagePermalink(
  ...
  logger.debug(
    `[SlackSkills] getSlackMessagePermalink direct API call for Atom user ${atomUserId}, channelId: ${channelId}, messageTs: ${messageTs}`
  );

  const client = getSlackClient();
  if (!client) {
    logger.error(
      '[SlackSkills] Slack client not available for getSlackMessagePermalink.'
    );
    return null;
  }
  ...
  try {
    ...
    if (!response.ok || !response.permalink) {
      ...
      logger.warn(
        `[SlackSkills] Slack API error or permalink not found for ts ${messageTs} in channel ${channelId}: ${slackError}`,
        response
      );
      return null;
    }
    logger.info(
      `[SlackSkills] Successfully fetched permalink for message ${messageTs}.`
    );
    return response.permalink as string;
  } catch (error: any) {
    logger.error(
      `[SlackSkills] Error in getSlackMessagePermalink direct API call for Atom user ${atomUserId}, channel ${channelId}, ts ${messageTs}:`,
      error
    );
    if (error instanceof SlackAPIError) {
      logger.error(
