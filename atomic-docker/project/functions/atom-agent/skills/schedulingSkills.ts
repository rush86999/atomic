...
export async function scheduleTask(
  ...
  try {
    ...
    if (isRecurring && repeatInterval) {
      ...
      console.log(
        `Recurring task for intent '${originalUserIntent}' for user '${userId}' scheduled. Interval: ${repeatInterval}. Data:`,
      ...
      console.log(
        `One-time task for intent '${originalUserIntent}' for user '${userId}' scheduled for '${when}'. Data:`,
        JSON.stringify(jobData)
      );
    }
    ...
    console.error(
      `Error scheduling task for intent '${originalUserIntent}' for user '${userId}':`,
      errorMessage,
      error
    );
...
async function _fetchUserWorkTimes(userId: string): Promise<UserWorkTime[]> {
  ...
  try {
    ...
    console.error(
      `[_fetchUserWorkTimes] Error fetching work times for user ${userId}:`,
      error
    );
    return [];
  }
...
export async function getUsersAvailability(
  ...
  console.log(
    `[schedulingSkills] Getting availability for users: ${userIds.join(', ')} within window: ${windowStart} to ${windowEnd}`
  );
  ...
  for (const userId of userIds) {
    try {
      ...
      if (eventsResponse.ok && eventsResponse.data) {
        ...
        relevantEvents = eventsResponse.data.filter((event) => {
          try {
            ...
            console.warn(
              `[getUsersAvailability] Could not parse event dates for event ID ${event.id}, user ${userId}:`,
              dateParseError
            );
            return false;
          }
        ...
        console.warn(
          `[getUsersAvailability] Could not fetch calendar events for user ${userId}: ${eventsResponse.error?.message}`
        );
        errors.push({
      ...
      console.error(
        `[getUsersAvailability] Critical error getting availability for user ${userId}: ${error.message}`
      );
      errors.push({
...
export async function handleScheduleSkillActivation(
  userId: string,
  entities: any
): Promise<string> {
...
export async function initiateTeamMeetingScheduling(
  ...
  console.log(
    'initiateTeamMeetingScheduling called with:',
    userId,
    meetingDetails
  );
  ...
  return {
    success: false,
    message:
      'initiateTeamMeetingScheduling not implemented yet. This would call OptaPlanner.',
  };
}
