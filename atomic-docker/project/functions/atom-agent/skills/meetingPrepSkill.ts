...
export async function fetchMeetingPrepInfo(
  ...
  logger.info(
    `[meetingPrepSkill] Starting meeting preparation for user ${userId} and meeting: ${nluEntities.meeting_reference}`
  );
  ...
  try {
    logger.info(
      `[meetingPrepSkill] Attempting to resolve meeting_reference: "${nluEntities.meeting_reference}" using findEventByFuzzyReference.`
    );
    ...
    if (eventResponse.ok && eventResponse.data) {
      ...
      logger.info(
        `[meetingPrepSkill] Successfully resolved meeting reference to event: "${resolvedEvent.summary}" (ID: ${resolvedEvent.id})`
      );
    } else if (eventResponse.error) {
      logger.warn(
        `[meetingPrepSkill] Could not resolve meeting_reference "${nluEntities.meeting_reference}" to a specific event. Error: ${eventResponse.error.message}`
      );
      ...
      logger.info(
        `[meetingPrepSkill] No specific calendar event found for reference: "${nluEntities.meeting_reference}". Proceeding without specific event context.`
      );
    }
  } catch (error: any) {
    logger.error(
      `[meetingPrepSkill] Critical error during findEventByFuzzyReference for "${nluEntities.meeting_reference}": ${error.message}`,
      error
    );
    aggregatedResults.errors_encountered?.push({
      source_attempted: 'calendar_lookup',
  ...
  for (const request of nluEntities.information_requests) {
    ...
    try {
      logger.debug(
        `[meetingPrepSkill] Processing source: ${request.source} with params: ${JSON.stringify(request.search_parameters)}`
      );
      switch (request.source) {
        case 'gmail':
          const gmailParams =
            request.search_parameters as GmailSearchParameters;
          logger.info(
            `[meetingPrepSkill] Calling searchEmailsForPrep with params: ${JSON.stringify(gmailParams)} and meeting context: ${aggregatedResults.identified_calendar_event?.summary}`
          );
          ...
          if (gmailSearchResponse.ok && gmailSearchResponse.data) {
            ...
            logger.info(
              `[meetingPrepSkill] searchEmailsForPrep returned ${sourceResultEntry.count} results. Query: ${sourceResultEntry.search_query_executed}`
            );
          } else {
            logger.error(
              `[meetingPrepSkill] searchEmailsForPrep failed: ${gmailSearchResponse.error?.message}`
            );
        ...
        case 'slack':
          const slackParams =
            request.search_parameters as SlackSearchParameters;
          logger.info(
            `[meetingPrepSkill] Calling Slack skill with params:`,
            slackParams
          );
            ...
            sourceResultEntry.results = [
              {
                ...
                permalink:
                  'https://app.slack.com/client/T123ABC/C123ABC/p1234567890123456',
              },
            ];
          ...
          logger.warn(
            `[meetingPrepSkill] Slack source processing is currently using placeholder data.`
          );
          // End Placeholder
          break;
        case 'notion':
          const notionParams =
            request.search_parameters as NotionSearchParameters;
          logger.info(
            `[meetingPrepSkill] Calling Notion skill with params:`,
            notionParams
          );
          ...
          if (
            notionParams.page_title_keywords?.includes('Acme Project') ||
            ...
            sourceResultEntry.results = [
              {
                ...
                preview_text:
                  'Initial project plan and deliverables for Acme Corp...',
              },
            ];
            sourceResultEntry.count = 1;
          } else {
          ...
          logger.warn(
            `[meetingPrepSkill] Notion source processing is currently using placeholder data.`
          );
          // End Placeholder
          break;
        case 'calendar_events':
          const calendarParams =
            request.search_parameters as CalendarEventsSearchParameters;
          logger.info(
            `[meetingPrepSkill] Calling Calendar Events skill with params:`,
            calendarParams
          );
          ...
          if (
            calendarParams.related_to_attendees_of_meeting_reference &&
            aggregatedResults.identified_calendar_event?.summary?.includes(
              'Acme Corp'
            )
          ) {
            sourceResultEntry.results = [
              {
                ...
                ).toISOString(), // Approx a month ago
                end: new Date(
                  Date.now() - 30 * 24 * 3600 * 1000 + 3600 * 1000
                ).toISOString(),
                htmlLink:
                  'https://calendar.google.com/calendar/event?eid=sim_456',
              },
            ];
            sourceResultEntry.count = 1;
          } else {
          ...
          logger.warn(
            `[meetingPrepSkill] Calendar Events source processing is currently using placeholder data.`
          );
          // End Placeholder
          break;
        default:
          logger.warn(
            `[meetingPrepSkill] Unknown source type: ${request.source}`
          );
          sourceResultEntry.error_message = `Unknown source type: ${request.source}`;
          const unkError: PrepErrorMessage = {
      ...
      logger.error(
        `[meetingPrepSkill] Error processing source ${request.source}: ${error.message}`,
        error
      );
      sourceResultEntry.error_message = `Error processing source ${request.source}: ${error.message}`;
      const procError: PrepErrorMessage = {
        source_attempted: request.source,
  ...
  try {
    const foundItems = aggregatedResults.results_by_source.reduce(
      (acc, rs) => acc + rs.count,
      0
    );
    if (foundItems > 0) {
      ...
      if (summaryParts.length > 0) {
        ...
        logger.info(
          `[meetingPrepSkill] Generated basic summary: ${aggregatedResults.overall_summary_notes}`
        );
      } else {
        ...
        logger.info(
          `[meetingPrepSkill] Items found but no counts per source for summary.`
        );
      }
      ...
      logger.info(
        `[meetingPrepSkill] No data found by any source, skipping summary generation.`
      );
      aggregatedResults.overall_summary_notes =
    ...
    logger.error(
      `[meetingPrepSkill] Error during AI summarization: ${summaryError.message}`,
      summaryError
    );
    aggregatedResults.errors_encountered?.push({
  ...
  logger.info(
    `[meetingPrepSkill] Finished meeting preparation for user ${userId}. Found ${aggregatedResults.results_by_source.reduce((acc, r) => acc + r.count, 0)} items across ${aggregatedResults.results_by_source.length} sources.`
  );
