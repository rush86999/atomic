...
import {
  listMicrosoftTeamsMeetings as listMeetings,
  getMicrosoftTeamsMeetingDetails as getDetails,
} from './msTeamsSkills';
import { handleError } from '../../_utils/errorHandler';

export async function handleListMicrosoftTeamsMeetings(
  userId: string,
  entities: any
): Promise<string> {
  try {
    ...
    const limit =
      entities?.limit && typeof entities.limit === 'number'
        ...
        : 10;
    const next_link =
      entities?.next_link && typeof entities.next_link === 'string'
        ? entities.next_link
        : undefined;
    const options = { limit, nextLink: next_link, filterForTeams: true };
    const response: ListMSTeamsMeetingsResponse = await listMeetings(
...
export async function handleGetMicrosoftTeamsMeetingDetails(
  userId: string,
  entities: any
): Promise<string> {
  try {
    ...
    if (!event_id || typeof event_id !== 'string') {
      ...
      const response: GetMSTeamsMeetingDetailsResponse = await getDetails(
        userPrincipalNameOrId,
        event_id
      );
    ...
    return handleError(
      error,
      `Sorry, an unexpected error occurred while fetching details for Teams meeting ${entities.event_id} (NLU path).`
    );
  }
}
