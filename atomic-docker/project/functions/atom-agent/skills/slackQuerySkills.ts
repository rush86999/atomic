...
import { understandSlackSearchQueryLLM, StructuredSlackQuery } from './llm_slack_query_understander';
...
export async function handleSearchSlackMessages(
  ...
  try {
    const structuredQuery: Partial<StructuredSlackQuery> = await understandSlackSearchQueryLLM(rawUserQuery);
    ...
    if (!slackApiQueryString || slackApiQueryString.trim() === "") {
        return {
            ok: true,
            data: { messages: [], userMessage: "I couldn't determine specific search criteria for Slack from your request. Please try rephrasing." }
        };
    }

    const messages: SlackMessage[] = await searchMySlackMessagesBackend(userId, slackApiQueryString, limit);

    let userMessage = "";
    if (messages.length === 0) {
      userMessage = "I couldn't find any Slack messages matching your search criteria.";
    } else {
    ...
    return {
        ok: false,
        error: { code: 'SLACK_SEARCH_FAILED', message: error.message || "Failed to search Slack messages." },
        data: { messages: [], userMessage: `Sorry, I encountered an error while searching Slack: ${error.message}` }
    };
  }
}
...
export async function handleExtractInfoFromSlackMessage(
  ...
  try {
    if (!messageText) {
      ...
      } else if (messageReference.includes('/')) { // Basic check for "channelId/messageTs"
          const parts = messageReference.split('/');
          if (parts.length === 2 && parts[0] && parts[1]) { // rudimentary check
    ...
    if (!messageText || messageText.trim() === "") {
        return {
            ok: false,
            error: { code: 'SLACK_MESSAGE_EMPTY', message: `The Slack message body for "${messageDescription}" is empty.`},
            data: { extractedInfo: {}, userMessage: `The Slack message body for "${messageDescription}" is empty, so I couldn't extract information.`}
        };
    }
    ...
    logger.error(`[handleExtractInfoFromSlack] Error: ${error.message}`, error);
     return {
        ok: false,
        error: { code: 'SLACK_EXTRACT_FAILED', message: error.message || "Failed to extract information from Slack message." },
        data: { extractedInfo: {}, userMessage: `Sorry, I encountered an error while extracting information from the Slack message: ${error.message}` }
    };
  }
}
