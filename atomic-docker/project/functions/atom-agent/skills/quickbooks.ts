import { ListQuickBooksInvoicesResponse, GetQuickBooksInvoiceDetailsResponse } from '../../types';
...
export async function handleGetQuickBooksAuthUrl(userId: string, entities: any): Promise<string> {
    try {
...
export async function handleListQuickBooksInvoices(userId: string, entities: any): Promise<string> {
    try {
        ...
        if (status && typeof status === 'string') console.log(`[handleListQuickBooksInvoices] NLU: received status filter: ${status}. Currently illustrative, skill may not filter by it.`);
        const response: ListQuickBooksInvoicesResponse = await listInvoices(options);
...
export async function handleGetQuickBooksInvoiceDetails(userId: string, entities: any): Promise<string> {
    try {
        const { invoice_id } = entities;
        if (!invoice_id || typeof invoice_id !== 'string') {
            return "Invoice ID is required to get QuickBooks invoice details via NLU.";
        } else {
            const response: GetQuickBooksInvoiceDetailsResponse = await getInvoiceDetails(invoice_id);
