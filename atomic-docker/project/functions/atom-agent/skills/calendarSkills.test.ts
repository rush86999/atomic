import {
  ...
  CalendarEvent,
  CreateEventResponse,
} from './calendarSkills'; // Adjust import if types are from '../types'
...
describe('Calendar Skills', () => {
  ...
  describe('Token Management (getStoredUserTokens, saveUserTokens - tested via getGoogleCalendarClient)', () => {
    ...
    it('getGoogleCalendarClient should successfully retrieve and use stored tokens', async () => {
      ...
      const clientResponse = await (
        calendarSkills as any
      ).getGoogleCalendarClient(userId); // Cast to any to call private
    ...
    it('getGoogleCalendarClient should return TOKEN_FETCH_FAILED on GraphQL error during token fetch', async () => {
      mockExecuteGraphQLQuery.mockRejectedValueOnce(
  ...
  describe('listUpcomingEvents', () => {
    ...
    it('should correctly map conferenceData when present in listUpcomingEvents', async () => {
      const eventWithMeet = createMockGoogleApiEvent(
        'evt1',
        'Meet Event',
        '2024-01-01T10:00:00Z',
        '2024-01-01T11:00:00Z',
        mockMeetConferenceData
      );
      mockEventsList.mockResolvedValue({ data: { items: [eventWithMeet] } });

      const events = await calendarSkills.listUpcomingEvents(
        'mock_user_id_for_meet_list'
      );
      expect(events.length).toBe(1);
      expect(events[0].conferenceData).toBeDefined();
      expect(events[0].conferenceData?.conferenceSolution?.key?.type).toBe(
        'hangoutsMeet'
      );
      expect(events[0].conferenceData?.entryPoints?.[0]?.uri).toBe(
        'https://meet.google.com/xyz-pdq-abc'
      );
    });
    ...
    it('should return empty array and log error if Google API call fails for list', async () => {
      ...
      const events = await calendarSkills.listUpcomingEvents(
        'mock_user_id_for_list_fail'
      );

      expect(events).toEqual([]);
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error fetching Google Calendar events'),
        'Google API List Error'
      );
    ...
    it('should handle "invalid_grant" error specifically for list', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Token error (invalid_grant)')
      );
      consoleErrorSpy.mockRestore();
    });
  });
  ...
  describe('getGoogleMeetEventDetails', () => {
    it('should fetch and map a single event with Google Meet data', async () => {
      ...
      const apiEvent = createMockGoogleApiEvent(
        mockEventId,
        'Detailed Meet Event',
      ...
      const result = await calendarSkills.getGoogleMeetEventDetails(
        'mock_user_id',
        mockEventId
      );
      ...
      expect(result.event?.conferenceData?.conferenceSolution?.key?.type).toBe(
        'hangoutsMeet'
      );
      expect(result.event?.conferenceData?.entryPoints?.[0]?.uri).toBe(
        'https://meet.google.com/xyz-pdq-abc'
      );
    ...
    it('should return error if event not found (API returns 404)', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining(`Error fetching event ${mockEventId}`),
        'Not Found'
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        `Event ${mockEventId} not found.`
      );
      consoleErrorSpy.mockRestore();
    });
    ...
    it('should return the event even if it is not a Google Meet event', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        `Event ${mockEventId} was found but does not appear to be a Google Meet event based on conferenceData.`
      );
    });
    ...
    it('should return error if getCalendarEventById returns null due to token issue', async () => {
      ...
      mockEventsGet.mockImplementation(async () => {
        ...
        (calendarSkills as any).getStoredUserTokens = jest
          .fn()
      ...
      mockEventsGet.mockRejectedValue(
        new Error('Simulated token failure for getCalendarEventById')
      );
  ...
  describe('listUpcomingGoogleMeetEvents', () => {
    it('should return only events with valid Google Meet links', async () => {
      const event1Meet = createMockGoogleApiEvent(
        'ev1',
        'Meet Event 1',
        '2024-01-01T10:00:00Z',
        '2024-01-01T11:00:00Z',
      ...
      const event2NonMeet = createMockGoogleApiEvent(
        'ev2',
        'Non-Meet Event',
      ...
      const event3NoConf = createMockGoogleApiEvent(
        'ev3',
        'No Conference',
        '2024-01-01T14:00:00Z',
        '2024-01-01T15:00:00Z',
      ...
      const event4MeetNoVideoUri = createMockGoogleApiEvent(
        'ev4',
        'Meet No Video URI',
      ...
      const result =
        await calendarSkills.listUpcomingGoogleMeetEvents('mock_user_id');
      expect(result.ok).toBe(true);
      expect(result.events).toBeDefined();
      expect(result.events?.length).toBe(1);
      expect(result.events?.[0].id).toBe('ev1');
      expect(
        result.events?.[0].conferenceData?.conferenceSolution?.key?.type
      ).toBe('hangoutsMeet');
    });

    it('should respect the limit after filtering Meet events', async () => {
      const meetEventsData = [
        createMockGoogleApiEvent(
          'm1',
          'Meet 1',
          '2024-01-01T10:00:00Z',
          '2024-01-01T11:00:00Z',
      ...
      const result = await calendarSkills.listUpcomingGoogleMeetEvents(
        'mock_user_id',
        2
      );
      expect(result.ok).toBe(true);
    ...
    it('should handle errors from listUpcomingEvents', async () => {
      mockEventsList.mockRejectedValue(
        new Error('Failed to fetch from Google API')
      );
      const consoleErrorSpy = jest
        .spyOn(console, 'error')
        .mockImplementation(() => {});
      ...
      expect(result.error).toContain(
        'Failed to list Google Meet events: Failed to fetch from Google API'
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error listing Google Meet events:',
        'Failed to fetch from Google API'
      );
      consoleErrorSpy.mockRestore();
    });
  });
  ...
  describe('createCalendarEvent', () => {
    ...
    it('should return auth error if no tokens are found', async () => {
      const response = await calendarSkills.createCalendarEvent(
        'unknown_user_id',
        eventDetails
      );
    ...
    it('should return config error if client ID or secret is missing', async () => {
      ...
      let response = await calendarSkills.createCalendarEvent(
        'mock_user_id',
        eventDetails
      );
      expect(response.success).toBe(false);
      expect(response.message).toBe(
        'Server configuration error for calendar service.'
      );
      ...
      response = await calendarSkills.createCalendarEvent(
        'mock_user_id',
        eventDetails
      );
      expect(response.success).toBe(false);
    ...
    it('should call Google API and return success on event creation', async () => {
      ...
      expect(response.message).toContain(
        'Calendar event created successfully with Google Calendar.'
      );
    });
    ...
    it('should return failure and log error if Google API call fails for insert', async () => {
      ...
      const response = await calendarSkills.createCalendarEvent(
        'mock_user_id_for_create_fail',
        eventDetails
      );

      expect(response.success).toBe(false);
      expect(response.message).toContain(
        'Failed to create event with Google Calendar: Google API Insert Error'
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error creating Google Calendar event'),
        'Google API Insert Error'
      );
      consoleErrorSpy.mockRestore();
    });
    ...
    it('should handle "invalid_grant" error specifically for insert', async () => {
      ...
      await calendarSkills.createCalendarEvent(
        'mock_user_id_for_invalid_grant_insert',
        eventDetails
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Token error (invalid_grant)')
  ...
  describe('findEventByFuzzyReference', () => {
    ...
    it('should return the first event for "next meeting"', async () => {
      ...
      const result = await calendarSkills.findEventByFuzzyReference(
        userId,
        'next meeting'
      );
      expect(result.ok).toBe(true);
    ...
    it('should handle "today" lookback period', async () => {
      ...
      await calendarSkills.findEventByFuzzyReference(
        userId,
        'Team Sync Q1',
        'today'
      );
  ...
  describe('slackMyAgenda', () => {
    ...
    it('should send formatted agenda to Slack when events are found', async () => {
      ...
      expect(sentMessage).toContain(
        '🔗 Google Meet: https://meet.google.com/xyz-pdq-abc'
      );
    ...
    it('should send "no upcoming events" message if no events are found', async () => {
      ...
      expect(sendSlackMessageSpy).toHaveBeenCalledWith(
        userId,
        userId,
        'You have no upcoming events in your Google Calendar for the requested period.'
      );
    });
    ...
    it('should use default channel if userId is not suitable as channelId and ATOM_DEFAULT_SLACK_CHANNEL_FOR_AGENDA is set', async () => {
      ...
      Object.defineProperty(
        ...
        { value: defaultChannel, configurable: true }
      );
    ...
    it('should return error if listUpcomingEvents fails', async () => {
      ...
      expect(result.ok).toBe(false);
      expect(result.error).toContain(
        'Failed to fetch calendar events: API_ERROR: Failed to fetch'
      );
      ...
      expect(sendSlackMessageSpy).toHaveBeenCalledWith(
        ...
        expect.stringContaining(
          "Sorry, I couldn't fetch your agenda. Error: API_ERROR: Failed to fetch"
        )
      );
    });
    ...
    it('should return error if sendSlackMessage fails', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Failed to send Slack message for agenda'),
        slackErrorMsg
      );
      consoleErrorSpy.mockRestore();
    });
    ...
    it('should return error if no slack channel can be determined and no events', async () => {
      ...
      expect(result.error).toBe(
        'No Slack channel ID to send the "no events" message to.'
      );
