  ...
  if (response.user_tokens && response.user_tokens.length > 0) {
    ...
    if (Date.now() > expiresAt) {
      ...
      if (!zoomClientId || !zoomClientSecret) {
        console.error(
          'Zoom environment variables not configured for token refresh.'
        );
        return null;
      }
...
export async function listZoomMeetings(
  ...
  console.log(
    `listZoomMeetings called for userId: ${userId} with options:`,
    options
  );
  ...
  try {
    const response = await axios.get<
      { meetings: ZoomMeeting[] } & ListZoomMeetingsData
    ...
    console.error(
      `Error listing Zoom meetings for userId ${userId}:`,
      axiosError.message
    );
...
export async function getZoomMeetingDetails(
  userId: string,
  meetingId: string
): Promise<ZoomSkillResponse<ZoomMeeting>> {
  ...
  if (!token) {
    return {
      ...
      error: {
        code: 'ZOOM_AUTH_ERROR',
        message:
          'Failed to obtain Zoom access token for getting meeting details.',
      },
    };
  }
  ...
  try {
    ...
    const axiosError = error as AxiosError;
    console.error(
      `Error getting Zoom meeting details for meetingId ${meetingId}:`,
      axiosError.message
    );
    const errorData = axiosError.response?.data as any;
    ...
    if (axiosError.response?.status === 404) {
      return {
        ...
        error: {
          ...
          message:
            `Meeting not found (ID: ${meetingId}). ${errorData?.message || ''}`.trim(),
          details: errorData,
        },
      };
    }
