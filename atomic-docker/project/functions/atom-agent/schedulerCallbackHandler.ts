...
async function sendUserNotification(
  ...
  logger.info(
    `[sendUserNotification] Attempting to send Slack DM to userId: ${userId}`
  );
  ...
  try {
    ...
    if (slackResponse.ok) {
      logger.info(
        `[sendUserNotification] Successfully sent Slack DM to userId: ${userId}. Message ID: ${slackResponse.ts}`
      );
      return true;
    } else {
      logger.error(
        `[sendUserNotification] Failed to send Slack DM to userId: ${userId}. Error: ${slackResponse.error}`
      );
      return false;
    }
  } catch (error: any) {
    logger.error(
      `[sendUserNotification] Exception while sending Slack DM to userId: ${userId}. Error: ${error.message}`,
      error.stack
    );
    return false;
  }
...
export async function handleSchedulerCallback(
  req: Request,
  res: Response
): Promise<void> {
  logger.info(
    '[schedulerCallbackHandler.handleSchedulerCallback] Received a callback request.'
  );

  // --- NEW TOKEN VALIDATION LOGIC ---
  if (!EXPECTED_CALLBACK_TOKEN) {
    logger.error(
      '[schedulerCallbackHandler.handleSchedulerCallback] CRITICAL: CALLBACK_SECRET_TOKEN is not configured in the environment. Cannot securely process callbacks.'
    );
    res
      .status(500)
      .send({
  ...
  if (!receivedToken) {
    logger.warn(
      '[schedulerCallbackHandler.handleSchedulerCallback] Unauthorized: Missing X-Callback-Token in request headers.'
    );
  ...
  if (receivedToken !== EXPECTED_CALLBACK_TOKEN) {
    logger.warn(
      '[schedulerCallbackHandler.handleSchedulerCallback] Forbidden: Invalid X-Callback-Token received.'
    );
  ...
  logger.info(
    '[schedulerCallbackHandler.handleSchedulerCallback] Callback token validated successfully.'
  );
  ...
  if (!req.body) {
    logger.warn(
      '[schedulerCallbackHandler.handleSchedulerCallback] Callback request body is empty after token validation.'
    );
  ...
  logger.info(
    '[schedulerCallbackHandler.handleSchedulerCallback] Processing solution for fileKey:',
    solution.fileKey,
    'hostId:',
    solution.hostId
  );
  ...
  if (!solution.fileKey || !solution.hostId || !solution.eventPartList) {
    logger.warn(
      '[schedulerCallbackHandler.handleSchedulerCallback] Invalid payload: Missing fileKey, hostId, or eventPartList.',
      solution
    );
    res
      .status(400)
  ...
  if (!requestContext) {
    logger.warn(
      `[schedulerCallbackHandler.handleSchedulerCallback] No pending request found for fileKey: ${solution.fileKey}. It might have been already processed, timed out, or is invalid.`
    );
    res
      .status(200)
      .send({
        message:
          'Callback received, but no matching pending request found or already processed.',
      });
    return;
  }
  ...
  logger.info(
    `[schedulerCallbackHandler.handleSchedulerCallback] Found context for fileKey ${solution.fileKey}: userId=${userId}, singletonId=${singletonId}, submittedAt=${submittedAt.toISOString()}`
  );
  if (originalQuery) {
    logger.info(
      `[schedulerCallbackHandler.handleSchedulerCallback] Original query: "${originalQuery}"`
    );
  }
  ...
  try {
    ...
    for (const part of solution.eventPartList) {
      ...
      if (part.timeslot) {
        ...
        scheduledItems.push(
          `- '${eventSummary}' scheduled ${scheduledTimeInfo.trim()}`
        );
      } else {
    ...
    const notificationSent = await sendUserNotification(
      userId,
    ...
    if (notificationSent) {
      logger.info(
        `[schedulerCallbackHandler.handleSchedulerCallback] User notification successful for userId: ${userId}, fileKey: ${solution.fileKey}`
      );
    } else {
      logger.warn(
        `[schedulerCallbackHandler.handleSchedulerCallback] User notification FAILED for userId: ${userId}, fileKey: ${solution.fileKey}. The scheduling solution was processed, but the user was not notified via Slack.`
      );
    ...
    logger.info(
      `[schedulerCallbackHandler.handleSchedulerCallback] Processed and cleaned up pending request for fileKey: ${solution.fileKey}`
    );
    ...
    res
      ...
      .send({
        message:
          'Callback processed successfully. User notification attempted.',
      });
  } catch (processingError: any) {
    logger.error(
      `[schedulerCallbackHandler.handleSchedulerCallback] Error processing solution for fileKey ${solution.fileKey}:`,
    ...
    res
      ...
      .send({
        error:
          'Callback received, but an internal error occurred during solution processing.',
      });
  }
  // --- NEW LOGIC ENDS HERE ---
}
