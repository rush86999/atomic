...
import {
  ...
  MessageQueueType,
} from '@schedule_assist/_libs/types/scheduleMeetingWorker/types';
...
import {
  ...
  getGlobalCalendar,
} from '@schedule_assist/_libs/api-helper';
import {
  EventPlusType,
  EventType,
...
} from '@schedule_assist/_libs/types';
import { ReturnValueForEachMeetingAssistType } from '@schedule_assist/_libs/types';
import ip from 'ip';
import {
  kafkaScheduleAssistGroupId,
  kafkaScheduleAssistTopic,
  ...
  try {
    ...
    for (let i = 0; i < attendees.length; i++) {
      ...
      const newMeetingEvent = generateNewMeetingEventForAttendee(
        ...
        hostTimezone,
        calendarId,
        preferredTimesRanges?.[randonNumber]
      );
      ...
      console.log(
        newMeetingEvent,
        ' newMeetingEvent inside processEachFutureMeetingAssist'
      );
    }
...
const processScheduleAssistWithMeetingAssist = async (
...
const processQueueMessage = async (
  ...
  try {
    ...
    if (!windowStartDate) {
      throw new Error(
        'no window start date provided inside atomic meeting assist'
      );
    }

    if (!windowEndDate) {
      throw new Error(
        'no window end date provided inside atomic meeting assist '
      );
    }
  ...
  try {
    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        ...
        const body: ScheduleAssistWithMeetingQueueBodyType = JSON.parse(
