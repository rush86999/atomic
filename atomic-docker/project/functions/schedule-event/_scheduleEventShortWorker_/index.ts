  ...
  try {
    ...
    if (results?._id && !event?.userModifiedCategories) {
      ...
      if (!previousEvent) {
        console.log(
          results?._id,
          'results?._id inside !previousEvent results?._id && !event?.userModifiedCategories'
        );
        ...
        console.log(
          newEvent,
          ' newEvent for processUserEventWithFoundPreviousEvent'
        );
        console.log(
          newReminders,
          ' newReminders for processUserEventWithFoundPreviousEvent'
        );
        userModifiedEvents.push(newEvent);
        newModifiedReminders.push({
          eventId: newEvent?.id,
    ...
    if (results?._id && event?.userModifiedCategories) {
      ...
      if (!previousEvent) {
        console.log(
          previousEvent,
          'previousEvent - old deleted event in doc search'
        );
        await deleteDocInSearch3(results?._id);
        ...
        if (categories?.[0]?.id) {
          ...
          console.log(
            newEvent,
            ' newEvent for processUserEventWithFoundPreviousEventWithUserModifiedCategories'
          );
          console.log(
            newReminders,
            ' newReminders for processUserEventWithFoundPreviousEventWithUserModifiedCategories'
          );
          userModifiedEvents.push(newEvent);
          newModifiedReminders.push({
            eventId: newEvent?.id,
