import { authApiToken, classificationUrl, dayOfWeekIntToString, eventVectorName, externalMeetingLabel, googleCalendarResource, googleClientIdAndroid, googleClientIdAtomicWeb, googleClientIdIos, googleClientIdWeb, googleClientSecretAtomicWeb, googleClientSecretWeb, googleTokenUrl, hasuraAdminSecret, hasuraGraphUrl, meetingLabel, minThresholdScore, onOptaPlanCalendarAdminCallBackUrl, openSearchEndPoint, optaPlannerPassword, optaPlannerUrl, optaPlannerUsername, optaplannerDuration, searchIndex, text2VectorUrl, vectorDimensions, zoomBaseTokenUrl, zoomBaseUrl, zoomClientId, zoomClientSecret, zoomIVForPass, zoomPassKey, zoomResourceName, zoomSaltForPass } from "@post_process_calendar/_libs/constants"
import {
    ...
    CalendarType, WorkTimeType, TimeSlotType, MonthType, DayType, MM, DD, MonthDayType, Time, EventPartPlannerRequestBodyType, InitialEventPartType, InitialEventPartTypePlus, UserPlannerRequestBodyType,
...
export const convertEventToVectorSpace2 = async (event: EventType): Promise<number[]> => {
    try {
        ...
        if (!text || text.trim() === ':') { // Handle cases where summary and notes might be empty or just ":"
        ...
        console.log(vector ? `Vector generated with length: ${vector.length}` : 'No vector generated', ' vector inside convertEventToVectorSpace2');
        return vector;
...
export const formatPlannerEventsToEventAndAdjustTime = (events: EventPlannerResponseBodyType[], oldEvent: EventPlusType, hostTimezone: string): EventPlusType => {
    ...
    console.log(oldEvent.id, startDate, firstPart?.timeslot?.dayOfWeek, firstPart?.timeslot?.monthDay, firstPart?.timeslot?.startTime, startHour, startMinute, dayjs(oldEvent.startDate.slice(0, 19)).tz(oldEvent.timezone, true).month(month)
        .date(day)
  ...
  .hour(startHour).minute(startMinute).format(),`)
    ...
    console.log(oldEvent.id, endDate, lastPart?.timeslot?.dayOfWeek, lastPart?.timeslot?.monthDay, lastPart?.timeslot?.endTime, endHour, endMinute, dayjs(oldEvent.endDate.slice(0, 19)).tz(oldEvent.timezone, true)
        .month(month)
        .date(day)
    ...
    try {
        ...
        if (method === 'update') {
            ...
            if (resource === googleCalendarResource) {
                ...
                console.log(idResponseBody, newEvent?.endDate,
                    ...
                    if (eventToUpdate?.method === 'create') {
                        ...
                        if (eventToUpdate?.meetingId) {
                            ...
                            if (meetingAssist?.userId !== eventToUpdate?.userId) {
                                ...
                                const attendeesToBeInsert: AttendeeType[] = attendees?.map(a => ({
                                    id: a?.id,
                                    userId: a?.userId,
