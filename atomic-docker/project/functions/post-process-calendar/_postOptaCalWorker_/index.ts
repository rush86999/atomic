...
import {
  streamToString,
  updateAllCalendarEventsPostPlanner,
} from '@post_process_calendar/_libs/api-helper';

import {
  MessageQueueType,
  PostProcessQueueBodyForCalendarType,
} from '@post_process_calendar/_libs/types/postOptaCalWorker/types';
...
const processPostOptaPlanQueueBody = async (
  ...
  try {
    ...
    newReminders.forEach((r) =>
      console.log(r, ' newReminders before updateAllCalendarEventsPostPlanner')
    );
    const eventsToUpdate: EventPlannerResponseBodyType[][] = [];
    if (plannerBodyResponse?.userList?.[0]?.id) {
      const eventPartParentFiltered = _.uniqBy(
        plannerBodyResponse.eventPartList,
        'eventId'
      );
      for (const eventPartParent of eventPartParentFiltered) {
        ...
        for (const eventPartChild of plannerBodyResponse.eventPartList) {
          if (eventPartParent.eventId === eventPartChild.eventId) {
            const oldEventPart = allEvents.find(
              (e) => e.id === eventPartChild.eventId
            );
            eventPartList.push({
              ...eventPartChild,
  ...
  try {
    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        ...
        const body: PostProcessQueueBodyForCalendarType =
