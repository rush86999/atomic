...
const CREATE_FAILED_PHYSICAL_ID_MARKER =
    'AWSCDK::CustomResourceProviderFramework::CREATE_FAILED',
  MISSING_PHYSICAL_ID_MARKER =
...
async function handler(event, context) {
  ...
  if (
    ...
    (exports.external.log(
      'ignoring DELETE event caused by a failed CREATE event'
    ),
...
function renderResponse(cfnRequest, handlerResponse = {}) {
  const physicalResourceId =
    handlerResponse.PhysicalResourceId ??
    cfnRequest.PhysicalResourceId ??
    cfnRequest.RequestId;
  if (
...
async function submitResponse(status, event) {
  ...
  await withRetries(
    { attempts: 5, sleep: 1e3 },
    exports.external.sendHttpRequest
  )(req, responseBody);
}
