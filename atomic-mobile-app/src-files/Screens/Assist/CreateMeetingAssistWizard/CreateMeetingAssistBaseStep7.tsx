...
function CreateMeetingAssistBaseStep7(props: Props) {
    ...
    const changeMinThresholdCount = (value: string) => {
        setMinThresholdCount(parseInt(value.replace(/[^0-9.]/g, ''), 10) || 2)
        setParentMinThresholdCount(parseInt(value.replace(/[^0-9.]/g, ''), 10) || 2)
    }
    ...
    return (
        <ScrollView >
            <Box flex={1} justifyContent="center" alignItems="center" style={{ width: '100%' }}>
                ...
                <Box m={{ phone: 's', tablet: 'm' }} style={{ width: '90%' }}>
                    <Box flexDirection="row" justifyContent="flex-start" mt={{ phone: 's', tablet: 'm' }} style={{ width: '100%'}}>
                        <Hint visible={isMessage} message={'Users will able to select custom timeslots that can overlap with already registered events. Events will be placed somewhere else if possible. Note: events from non-Atomic users cannot be changed.'} color={Colors.purple} onBackgroundPress={() => setIsMessage(false)}>
