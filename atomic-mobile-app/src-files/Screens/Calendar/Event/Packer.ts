...
import { Event, PackedEvent } from '@screens/Calendar/Event/EventBlock';
...
function buildEvent(event: Event & { index: number }, left: number, width: number, { dayStart = 0, hourBlockHeight = HOUR_BLOCK_HEIGHT }: PopulateOptions): PackedEvent {
    ...
    let endTimePlus = endTime
    let isAllDay = dayjs.duration(dayjs(event.end).diff(dayjs(event.start))).asHours() > 23;
    ...
    if (isAllDay) {
        endTimePlus = new XDate(dayjs(event.start).add(30, 'minutes').format());
    }
    ...
    return {
        ...event,
        title: `${isAllDay ? `${event?.title} - All day` : event?.title}`,
        top: (dayStartTime.diffHours(startTime) - dayStart) * hourBlockHeight,
        height: startTime.diffHours(endTimePlus) * hourBlockHeight,
        width,
        left,
    };
}
