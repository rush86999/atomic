...
import { TouchableOpacity, StyleSheet, useColorScheme, Appearance, FlatList, Pressable } from 'react-native'
...
type RootNavigationStackParamList = {
  UserActivateSchedule: undefined,
}

type UserActivateScheduleNavigationProp = StackNavigationProp<
...
type UserActivateScheduleRouteProp = RouteProp<
...
type Props = {
  userId: string,
  route: UserActivateScheduleRouteProp,
  client: ApolloClient<NormalizedCacheObject>,
}

function UserActivateSchedule(props: Props) {
  ...
  const navigation = useNavigation<UserActivateScheduleNavigationProp>()
  ...
  }, [primaryGoalType, secondaryGoalType, userId])
      ...
      if (eventId) {
        const newSchedule = await DataStore.save(
          new Schedule({
            ...
            userIdGoal: secondaryGoalType?.length > 0 ? `${userId}#${primaryGoalType}#${secondaryGoalType}` : `${userId}#${primaryGoalType}`,
            eventId,
...
export default UserActivateSchedule
