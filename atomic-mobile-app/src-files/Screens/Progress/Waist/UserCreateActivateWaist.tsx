...
type RootStackParamList = {
  UserCreateActivateWaist: {},
}

type UserCreateActivateWaistNavigationProp = StackNavigationProp<
  RootStackParamList,
  'UserCreateActivateWaist'
>
...
function UserCreateActivateWaist(props: Props) {
  ...
  const navigation = useNavigation<UserCreateActivateWaistNavigationProp>()
      ...
      if (scheduleDaysEl?.current?.length > 0) {
        ...
        ||  (isReminderEl?.current && !reminderFrequencyEl?.current)) {
          Toast.show({
            ...
            text2: 'You have an invalid selection for schedule. Please correct it before activating step',
          })
          return null
        }
        ...
        const schedule = new Schedule({
          ...
          date: dayjs().format(),
          userIdGoal: `${userIdEl?.current}#${PrimaryGoalType.WAIST}#null`,
          unit: 'inches',
  ...
  const createNewActiveComponent = async (): Promise<undefined | null> => {
    try {
      ...
      Toast.show({
            ...
            text2: `You have activated Waist Component 🙌`
         });
  ...
  return (
    <Box flex={1} justifyContent="center" alignItems="center">
      <Wizard testID={'uilib.wizard'} activeIndex={activeIndex} onActiveIndexChanged={onActiveIndexChanged}>
        ...
        <Wizard.Step state={getStepState(2)} label={'Activate Waist'}/>
      </Wizard>
      {renderCurrentStep()}
    </Box>
  )
...
export default UserCreateActivateWaist
