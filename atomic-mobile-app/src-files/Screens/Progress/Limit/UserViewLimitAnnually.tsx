...
function UserViewLimitAnnually(props: Props) {
  const [datasetG, setDataSetG] = useState<LimitTypeData[] | null>(null)
  const [userId, setUserId] = useState<string | null>(null)
  // const [labels, setLabels] = useState<string[] | null>(null)
  const [chartData, setChartData] = useState<data | null>(null)
  const [goal, setGoal] = useState<Goal | null>(null)
  const [limitTypeProgressBar, setLimitTypeProgressBar] = useState<number>(0)
  ...
  // get Goal
  useEffect(() => {
    const getLimitTypeGoal = async (userId1: string) => {
      try {
    ...
    if (userIdEl?.current) {
      getLimitTypeGoal(userIdEl?.current);
    }
  }, [userId])
  ...
  /** get data */
  useEffect(() => {
    const updateLimitProgressBar = (dataset: LimitTypeData[]) => {
      const currentGoal = goal?.goal as string
      const newLimitTypeProgressBar: number = (((dataset as LimitTypeData[])?.[(dataset as LimitTypeData[])?.length - 1]?.['value'] as number) > parseFloat(currentGoal))
        ? 1
        : typeof ((dataset as LimitTypeData[])?.[(dataset as LimitTypeData[])?.length - 1]?.['value']) === 'number'
          ? math.chain(((dataset as LimitTypeData[])?.[(dataset as LimitTypeData[])?.length - 1]?.['value'] as number)).divide(parseFloat(currentGoal) || 0).done()
          : 0

      setLimitTypeProgressBar(newLimitTypeProgressBar)
    }
    const conformData = (dataset: LimitTypeData[], labels: string[]) => {
      if (labels?.[0] && dataset?.[0]?.id) {
        const valueDataset = dataset.map((i: LimitTypeData) => {
          ...
          if (dataset?.[0]?.id && goal?.goal) {
            updateLimitProgressBar(dataset)
          }
    ...
    const getData = async (userId1: string) => {
      try {
        ...
        for (let i = 0; i < 31; i++) {
          newLabels.push(dayjs().subtract(i + 12, 'd').format('dd'))
        }
            ...
            ? (
              <Box style={{ width: '100%'}} flex={1} justifyContent="flex-start" alignItems="center">
                ...
                <Box style={{ width: '100%'}} flex={1} justifyContent="flex-start" alignItems="center" m={{ phone: 's', tablet: 'm' }}>
                  <Box style={{ width: '100%'}} flex={1} justifyContent="flex-start" alignItems="center" m={{ phone: 's', tablet: 'm' }}>
                    {(typeof ((datasetG as LimitTypeData[])?.[(datasetG as LimitTypeData[])?.length - 1]
                      ?.['value'] as number) === 'number')
                      && parseFloat(goal?.goal) > 0 ? (
                        <Text variant="optionHeader">
                          {`${math.round((datasetG as LimitTypeData[])
                              ?.[(datasetG as LimitTypeData[])?.length - 1]
                  ...
                  <Box style={{ width: '100%'}} flex={1} m={{ phone: 's', tablet: 'm' }} justifyContent="flex-start" alignItems="center">
                    <Bar progress={limitTypeProgressBar} width={200} />
                  </Box>
                </Box>
              </Box>
            ) : null
...
export default UserViewLimitAnnually
