...
UserCreateActivateGenericExercise: {},
};

type UserCreateActivateGenericExerciseNavigationProp = StackNavigationProp<
  RootStackParamList,
  'UserCreateActivateGenericExercise'
>
...
function UserCreateActivateGenericExercise(props: Props) {
  ...
  const navigation = useNavigation<UserCreateActivateGenericExerciseNavigationProp>()
    ...
    try {
      ...
      if (dayReminderTimes?.length > 0) {
        ...
        if (calendarEventIdArray && calendarEventIdArray.length > 0) {
          const realmEventPromises = calendarEventIdArray.map(
            async (eId, index) => createRealmEvent(
              eId,
              dayReminderTimes[index],
              dayReminderTimes[index],
              '',
              calendarEventIdEl.current,
              PrimaryGoalType.GENERICEXERCISE,
              true,
              false,
              false,
              false,
    ...
    try {
      ...
      if (!isAvailable) {
        Toast.show({
          type: 'error',
          text1: 'Not available',
          text2: `You have already activated exercise component`
        })
      ...
      Toast.show({
            ...
            text2: `You have activated Exercise Component 🙌`
         });
  ...
  return (
    <Box flex={1} justifyContent="center" alignItems="center">
      <Wizard testID={'uilib.wizard'} activeIndex={activeIndex} onActiveIndexChanged={onActiveIndexChanged}>
        ...
        <Wizard.Step state={getStepState(2)} label={'Activate Exercise'}/>
      </Wizard>
      {renderCurrentStep()}
    </Box>
  )
...
export default UserCreateActivateGenericExercise
