...
import UserExerciseTimer from '@screens/Playback/UserExerciseTimer'
...
type RootNavigationStackParamList = {
  ...
  UserAddEndurance: undefined,
}

type UserAddEnduranceNavigationProp = StackNavigationProp<
...
function UserAddEndurance(props: Props) {
  ...
  const enduranceRef: React.MutableRefObject<LottieView | null> = useRef(null)

  const navigation = useNavigation<UserAddEnduranceNavigationProp>()



  // play endurance animation
  useEffect(() => {
    enduranceRef?.current?.play()
  }, [])
  ...
  // get pointreward && userId after getting scheduleId
  useEffect(() => {
  ...
  const createEnduranceData = async (): Promise<null | undefined> => {
    ...
    try {
      if (enduranceDataIdEl?.current) {
        ...
        if (original) {
          ...
          const updatedEnduranceData = await DataStore.save(
            EnduranceData.copyOf(original, (updated) => {
              ...
              if (parseFloat(enduranceMinutes) > -1) {
          ...
          if (parseFloat(reps) > -1) {
            Toast.show({
                  ...
                  text2: `You have recorded ${enduranceMinutes} minutes and ${reps} reps.`
               });
          } else if (parseFloat(distance) > -1) {
            Toast.show({
                  type: 'success',
                  text1: `Endurance: ${rescapeUnsafe(capitalizeFirstLetter(type))} recorded`,
                  text2: `You have recorded ${enduranceMinutes} minutes and ${distance} miles.`
               });
          }
        ...
        const newEnduranceData = await DataStore.save(
          enduranceData
        )
  ...
  const onAccumulatedDuration = (value: string) => setEnduranceMinutes(value)
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      ...
      <Box style={{ width: '100%'}} flex={1} justifyContent="center" alignItems="center">
        <Box flex={1} style={{ width: '100%'}} justifyContent="center" alignItems="center" my={{ phone: 's', tablet: 'm' }}>
          <Box flex={0.5} m={{ phone: 's', tablet: 'm' }} style={{ width: '100%'}} flexDirection="row" justifyContent="center" alignItems="center">
            <Box m={{ phone: 's', tablet: 'm'}} style={{ width: '15%'}}>
              <TextField
                type="numeric"
                title="minutes"
                placeholder="0"
          ...
          <Box flex={1} style={{ width: '100%'}} justifyContent="center" alignItems="center">
            <Box style={{ width: '60%'}}>
              <Picker
                ...
                >
                  <Picker.Item color={dark ? palette.white : palette.textBlack} value="reps" label="reps" />
                  <Picker.Item color={dark ? palette.white : palette.textBlack} value="miles" label="miles" />
              </Picker>
            </Box>
          </Box>
          <Box flex={1.25} style={{ width: '100%'}} justifyContent="center" alignItems="center">
            <UserExerciseTimer
              sub={sub}
              numberOfBreakEpisodes={1}
...
export default UserAddEndurance
