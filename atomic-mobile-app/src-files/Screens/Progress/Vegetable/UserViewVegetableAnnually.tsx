...
type UserViewVegetableAnnuallyNavigationProp = StackNavigationProp<
  RootStackParamList,
  'UserViewVegetableAnnually'
>
...
function UserViewVegetableAnnually(props: Props) {
  const [dataset, setDataSet] = useState<VegetableData[] | null>(null)
  const [labels, setLabels] = useState<string[] | null>(null)
  const [chartData, setChartData] = useState<data | null>(null)
  const [goal, setGoal] = useState<Goal | null>(null)
  const [stepProgressBar, setVegetableProgressBar] = useState<number>(0)
  ...
  const navigation = useNavigation<UserViewVegetableAnnuallyNavigationProp>()
  ...
  useEffect(() => {
    const getVegetableGoal = async (userId1: string) => {
      try {
    ...
    if (userIdEl?.current) {
      getVegetableGoal(userIdEl?.current);
    }
  }, [userIdEl?.current])

  useEffect(() => {
    const updateVegetableProgressBar = () => {
      const currentGoal = goal?.goal as string
      const newVegetableProgressBar: number = (((dataset as VegetableData[])[(dataset as VegetableData[]).length - 1]['servings'] as number) > parseFloat(currentGoal))
        ? 1
        : (math.chain(((dataset as VegetableData[])[(dataset as VegetableData[]).length - 1]['servings'] as number)).divide(parseFloat(currentGoal)).done())

      setVegetableProgressBar(newVegetableProgressBar)
    }
    if (dataset && dataset.length > 0 && goal && goal.goal) {
      updateVegetableProgressBar()
    }
  }, [((dataset && dataset[(dataset as VegetableData[]).length - 1] && dataset[(dataset as VegetableData[]).length - 1].servings) || 0), ((goal && goal?.goal))])
  ...
  useEffect(() => {
    const conformData = () => {
      ...
      if (labels && labels.length > 0 && dataset && dataset.length > 0) {
        const repsDataset = dataset.map((i: VegetableData) => i.servings)
          const data: data = {
            labels,
            datasets: [
              {
            ...
            ? (
              <Box m={{ phone: 's', tablet: 'm' }} >
                <Box style={{ width: '100%' }} flexDirection="row" justifyContent="space-between">
                  ...
                  {(((dataset as VegetableData[])[(dataset as VegetableData[]).length - 1]
                    ['servings'] as number) > 0)
                    && goal?.goal ? (
                      <Text variant="caption">
                        {`${((dataset as VegetableData[])
                          [(dataset as VegetableData[]).length - 1]
...
export default UserViewVegetableAnnually
