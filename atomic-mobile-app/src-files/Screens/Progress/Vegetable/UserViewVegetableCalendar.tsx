...
type UserViewVegetableCalendarNavigationProp = StackNavigationProp<
  RootStackParamList,
  'UserViewVegetableCalendar'
>
...
function UserViewVegetableCalendar(props: Props) {
  ...
  const [vegetableProgressBar, setVegetableProgressBar] = useState<number>(0)
  ...
  const navigation = useNavigation<UserViewVegetableCalendarNavigationProp>()
      ...
      try {
        ...
        const streakDataFetched = await Promise.all(dates.map(async (date) => {
          try {
            return DataStore.query(
              Streak,
              c => c.userIdGoal('eq', `${userId1}#${PrimaryGoalType.VEGETABLE}#null`)
        ...
        <Box>
          {
            ...
            ? (
              <Box m={{ phone: 's', tablet: 'm' }} >
                ...
                <Bar progress={vegetableProgressBar} width={200} />
              </Box>
            ) : null
          }
        </Box>
...
export default UserViewVegetableCalendar
