...
function UserViewStrengthCalendar(props: Props) {
  const [userId, setUserId] = useState<string | null>(null)
  const [currentDataView, setCurrentDataView] = useState()
  const [month, setMonth] = useState<number>(0)
  const [selectedDataType, setSelectedDataType] = useState<measureUnit>('Weight')
      ...
      try {
        ...
        const streakDataFetched = await Promise.all(dates.map(async (date) => {
          try {
            return DataStore.query(
              Streak,
              c => c.userIdGoal('eq', `${userId1}#${PrimaryGoalType.STRENGTH}#${type}`)
    ...
    try {
      ...
      const streakDataFetched = await Promise.all(newDatas.map(async (data) => {
        try {
          return DataStore.query(
            Streak,
            c => c.userIdGoal('eq', `${userId}#${PrimaryGoalType.STRENGTH}#${type}`)
...
export default UserViewStrengthCalendar
