...
type UserAddWeightNavigationProp = StackNavigationProp<
...
function UserAddWeight(props: Props) {
  const [goal, setGoal] = useState<Goal | null>(null);
  const [weightPounds, setWeightPounds] = useState<string>('30')
  ...
  const weightRef: React.MutableRefObject<LottieView | null> = useRef(null)
  ...
  const weightDataIdEl = useRef<string>(null)
  const userStatEl = useRef<UserStat>()

  const navigation = useNavigation<UserAddWeightNavigationProp>()
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const dataPoints = await DataStore.query(WeightData,
  ...
  useEffect(() => {
    const getWeightData = async (userId1: string) => {
      try {
        ...
        if (weightDatas && weightDatas[0] && weightDatas[0].id) {
          const { date } = weightDatas[0]

          if (dayjs(date).isSameOrAfter(dayjs().hour(0))) {
            
            const { id } = weightDatas[0]
            weightDataIdEl.current = id
    ...
    if (userIdEl?.current) {
      getWeightData(userIdEl?.current)
    }
  }, [userId])
 ...
 const updateUserStats = async () => {
   try {
     ...
     if (userStatEl?.current?.currentDate && userIdEl?.current) {
         if (dayjs(userStatEl?.current?.currentDate).format('YYYY-MM-DD') !== dayjs().format('YYYY-MM-DD')) {
           const newUserStat = await DataStore.save(
             UserStat.copyOf(userStatEl?.current, updated => {
               ...
               if (parseFloat(weightPounds) > 0) {
                 updated.currentValue = parseFloat(weightPounds)
           ...
           userStatEl.current = newUserStat
             ...
             const original: WeightData | undefined = await DataStore.query(WeightData, weightDataIdEl?.current)

             if (original?.id) {

               const originalCopy = original as WeightData
               const difference = math.chain(originalCopy.weight).subtract(parseFloat(weightPounds)).done()
       ...
       const newUserStat = new UserStat({
         primaryGoalType: PrimaryGoalType.MEDITATION,
         userId: userIdEl?.current,
         value: parseFloat(weightPounds),
         max: parseFloat(weightPounds),
         min: parseFloat(weightPounds),
  ...
  const createNewWeightData = async () => {
    try {
      if (!weightPounds || !(userIdEl?.current)) {
        Toast.show({
              type: 'error',
              text1: `Missing info`,
      ...
      if (weightDataIdEl?.current) {
        ...
        if (original?.id) {
          ...
          const updatedWeightData = await DataStore.save(
            WeightData.copyOf(originalCopy, (updated) => {
              if (parseFloat(weightPounds) > -1) {
                updated.weight = parseFloat(weightPounds)
              }
            })
          )
          ...
          Toast.show({
                ...
                text2: `You have recorded ${weightPounds} weight 🙌`
             })

             navigation.navigate('UserProgressActiveComponents', { isUpdate: uuid() })
          }
      } else if (userIdEl?.current && !(weightDataIdEl?.current)) {
        ...
        const newWeightData = await DataStore.save(
          weightData
        )
        ...
        Toast.show({
              type: 'success',
              text1: 'Weight recorded',
              text2: `You have recorded ${weightPounds} weight. You are awarded ${pointRewardEl?.current} points! 🙌`
           });
        weightDataIdEl.current = weightData.id
        ...
        <Box flex={1} justifyContent="center" alignItems="center">
          <Box flex={2} justifyContent="center" alignItems="center" my={{ phone: 's', tablet: 'm' }}>
            ...
            <Box flex={2}>
              <LottieView
                ref={weightRef}
                source={require('@assets/icons/weightLift.json')}
                loop
                style={styles.svg}
              />
            </Box>
            ...
            <RegularCard>
              <Box>
                <TextField
                  type="numeric"
                  title="weight"
                  onChangeText={(text: string) => setWeightPounds(text.replace(/[^0-9.]/g, ''))}
                  value={weightPounds}
                />
              </Box>
              <Button onPress={createNewWeightData}>
                Save
              </Button>
            </RegularCard>
          </Box>
