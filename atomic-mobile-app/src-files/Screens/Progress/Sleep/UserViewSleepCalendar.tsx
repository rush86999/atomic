...
type UserViewSleepCalendarNavigationProp = StackNavigationProp<
  RootStackParamList,
  'UserViewSleepCalendar'
>
...
function UserViewSleepCalendar(props: Props) {
  ...
  const navigation = useNavigation<UserViewSleepCalendarNavigationProp>()
      ...
      try {
        ...
        const streakDataFetched = await Promise.all(dates.map(async (date) => {
          try {
            return DataStore.query(
              Streak,
              c => c.userIdGoal('eq', `${userId1}#${PrimaryGoalType.SLEEP}#null`)
...
export default UserViewSleepCalendar
