...
function UserViewStepCalendar(props: Props) {
    ...
    for (const date of dates) {
        ...
        if (streakDates?.length > 0) {
          streakDates.forEach(streakDate => {
            ...
            if (dayjs(date).format('YYYY-MM-DD') === streakDate.startDate) {
      ...
      try {
        ...
        const streakDataFetched = await Promise.all(dates.map(async (date) => {
          try {
            return DataStore.query(
              Streak,
              c => c.userIdGoal('eq', `${userId1}#${PrimaryGoalType.STEP}#null`)
...
export default UserViewStepCalendar
