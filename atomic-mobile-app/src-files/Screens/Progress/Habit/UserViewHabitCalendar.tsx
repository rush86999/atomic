...
function UserViewHabitCalendar(props: Props) {
  // const [dataset, setDataSet] = useState<HabitTypeData[] | null>(null)
      ...
      try {
        ...
        const streakDataFetched = await Promise.all(dates.map(async (date) => {
          try {
            return DataStore.query(
              Streak,
              c => c.userIdGoal('eq', `${userId1}#${PrimaryGoalType.HABITTYPE}#${type}`)
...
export default UserViewHabitCalendar
