      ...
      <Swipeable
        ...
        >
          ...
          <Stepper
            minValue={1}
            maxValue={30}
            onValueChange={(count: number) => updateRepInArray(index, count)}
...
function UserAddStrengthMini(props: Props) {
  const [strengthDataId, setStrengthDataId] = useState<string>('')
    ...
    (async () => {
      try {
        ...
        if (dataPoints?.length > 2) {
          const firstDay = dayjs.utc(dataPoints[0].date).format('YYYY-MM-DD')
          const secondDay = dayjs.utc(firstDay).subtract(1, 'd').format('YYYY-MM-DD')
          const thirdDay = dayjs.utc(secondDay).subtract(1, 'd').format('YYYY-MM-DD')

          if (
            (dayjs.utc(dataPoints[1].date).format('YYYY-MM-DD') === secondDay)
            && (dayjs.utc(dataPoints[2].date).format('YYYY-MM-DD') === thirdDay)
          ) {
            await createStreak()
          }
        }
      ...
      try {
        ...
        if (userData?.[0]?.id) {
          ...
          if (
            scheduleDatas?.[0]?.id
            && (event?.endDate
              && dayjs.utc(event?.endDate).isAfter(dayjs())
              || !(event?.endDate)
            )) {
      ...
      try {
        ...
        const goals1 = await DataStore.query(
          ...
          if (goals && goals.length > 0) {
            setGoal(goals[0])
            const [oldGoal] = goals
            if (dayjs().isAfter(dayjs.utc(oldGoal.endDate))) {
    ...
    const getStrengthData = async () => {
      try {
        ...
        if (strengthDatas && strengthDatas[0] && strengthDatas[0].id) {
          ...
          if (dayjs.utc(date).isSameOrAfter(dayjs().hour(0))) {
            const { id } = strengthDatas[0]
            setStrengthDataId(id)
          }
        }
      } catch (e) {
    ...
    (async () => {
      ...
      const schedules = await DataStore.query(Schedule,
        ...
        if (schedules?.length > 0) {
          ...
          if (event?.endDate
            && dayjs().isAfter(dayjs.utc(event?.endDate))) {
        ...
        if (levelEl.current?.level > 1) {
          const newLevel = await DataStore.save(
            new Level({
              ...
              primaryGoalType: PrimaryGoalType.STRENGTH,
              secondaryGoalType: type,
              date: dayjs.utc(comparisonDate).format()
            })
          )
  ...
  const metLevel = async (comparisonDate: string) => {
    try {
      const weeks = dayjs.utc(comparisonDate).diff(dayjs.utc(levelEl.current?.date).format(), 'w', true)
  ...
  const updateUserExerciseStats = async () => {
    try {
      if (userExerciseStatEl.current?.currentDate && userIdEl.current) {
          ...
          if (dayjs.utc(userExerciseStatEl.current.currentDate).format('YYYY-MM-DD') !== dayjs().format('YYYY-MM-DD')) {
          ...
          userExerciseStatEl.current = newUserStat
              ...
              const original: StrengthData | undefined = await DataStore.query(StrengthData, strengthDataId)
  ...
  const updateUserPoint = async () => {
    try {
      if (pointEl?.current?.currentDate) {
          if (dayjs.utc(pointEl.current.currentDate).format('YYYY-MM-DD') !== dayjs().format('YYYY-MM-DD')) {
  ...
  const createManyStrengthData = async () => {
    try {
      ...
      if (strengthArray[1]?.id) {
        setStrengthDataId(strengthArray[1]?.id)
      }
    } catch (e) {
  ...
  const renderRepRightActions = (progress: Animated.AnimatedInterpolation, index: number) => {
    ...
    return (
      <RectButton style={styles.leftAction} onPress={() => removeRepFromArray(index)}>
        <Animated.Text
          style={[
            styles.actionText,
            {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      ...
      <Box style={{ width: '100%', height: '90%' }} justifyContent="space-between">
        <Text  m={{ phone: 's', tablet: 'm' }} textAlign="center" variant="subheader">
...
export default UserAddStrengthMini
