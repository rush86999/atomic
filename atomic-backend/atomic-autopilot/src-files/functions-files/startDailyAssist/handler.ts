...
import { createDailyScheduleAssist7DayWindowEventTrigger, createInitialFeaturesApplyToEventTrigger } from '@libs/api-helper';
import { StartDailyAssistEventTriggerType } from '@libs/types';
...
const publisher = async (event) => {
  try {
    ...
    const eventItem: StartDailyAssistEventTriggerType = JSON.parse(event.body)
    ...
    await createDailyScheduleAssist7DayWindowEventTrigger(
      eventItem?.autopilot,
      eventItem?.body,
    )
