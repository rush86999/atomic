import { authApiToken, bucketName, bucketRegion, calendarFreeDelay, calendarProDelay, calendarShortDelay, classificationUrl, dayOfWeekIntToString, eventVectorName, externalMeetingLabel, hasuraAdminSecret, hasuraGraphUrl, meetingLabel, minThresholdScore, onOptaPlanCalendarAdminCallBackUrl, openSearchEndPoint, optaPlannerPassword, optaPlannerUrl, optaPlannerUsername, searchIndex, text2VectorUrl, vectorDimensions } from "@libs/constants"
import { BufferTimeNumberType, EventPlusType, EventType, MeetingAssistAttendeeType, MeetingAssistEventType, MeetingAssistPreferredTimeRangeType, MeetingAssistType, EventMeetingPlusType, PreferredTimeRangeType, RemindersForEventType, ValuesToReturnForBufferEventsType, UserPreferenceType, CalendarType, WorkTimeType, TimeSlotType, MonthType, DayType, MM, DD, MonthDayType, ActiveSubscriptionType, SubscriptionType, AdminBetaTestingType, Time, EventPartPlannerRequestBodyType, InitialEventPartType, InitialEventPartTypePlus, UserPlannerRequestBodyType, ReturnBodyForHostForOptaplannerPrepType, ReturnBodyForAttendeeForOptaplannerPrepType, ReturnBodyForExternalAttendeeForOptaplannerPrepType, PlannerRequestBodyType, FreemiumType, esResponseBody, CategoryType, ClassificationResponseBodyType, BufferTimeObjectType, ReminderType, CategoryEventType } from "@libs/types"
import got from "got"
...
export const generateNewMeetingEventForHost = (
    ...
    let startDate = dayjs(windowStartDate.slice(0, 19)).tz(hostTimezone, true).format()
    if (preferredStartTimeRange?.dayOfWeek > 0) {
        startDate = dayjs(startDate).isoWeek(preferredStartTimeRange?.dayOfWeek).format()
    }
...
export const generateEventPartsLite = (event: EventPlusType, hostId: string): InitialEventPartType[] => {
    ...
    const minutes = dayjs(event.endDate.slice(0, 19)).tz(event.timezone).diff(dayjs(event.startDate.slice(0, 19)).tz(event.timezone, true), 'm')
...
export const processEventsForOptaPlanner = async (
    ...
    try {
        ...
        (returnValuesFromInternalAttendees as ReturnBodyForAttendeeForOptaplannerPrepType)?.eventParts?.forEach(e => 
        ...
        (returnValuesFromInternalAttendees as ReturnBodyForAttendeeForOptaplannerPrepType)?.breaks?.forEach(e => 
