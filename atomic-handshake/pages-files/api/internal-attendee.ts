...
import { cancelMeetingAssist, deleteMeetingAssistAttendee, getMeetingAssist, getMeetingAssistAttendee, getUserContactInfo, updateMeetingAssistAttendanceCount, updateMeetingAssistInviteResponse, upsertOneMeetingAssistAttendee } from '@lib/api-helper'
...
export default async function handler(
    ...
    try {
        const body: RequestDataForCancelMeetingAssistType
        ...
        if (body?.method === 'deleteMeetingAssistAttendee') {
            const id = (body as RequestDataForDeleteMeetingAssistAttendeeType)?.variables?.id
        ...
        if (body?.method === 'getMeetingAssistAttendee') {
            const id = (body as RequestDataForGetMeetingAssistAttendeeType)?.variables?.id
        ...
        if (body?.method === 'upsertOneMeetingAssistAttendee') {
            const attendee = (body as RequestDataForUpsertMeetingAssistAttendeeType)?.variables?.attendee
        ...
        if (body?.method === 'updateMeetingAssistAttendanceCount') {
            const { id, attendeeCount, attendeeRespondedCount } = (body as RequestDataForUpdateMeetingAssistAttendanceCountType)?.variables
        ...
        if (body?.method === 'updateMeetingAssistInviteResponse') {
            const { id, response } = (body as RequestDataForUpdateMeetingAssistInviteResponseType)?.variables
