...
import { createRecurringMeetingAssists, deleteMeetingAssistPreferredTimesByIds, findEventsForUserGivenMeetingId, generateAvailableSlotsforTimeWindow, getMeetingAssist, getUserPreferences, listEventsForUserGivenDates, listMeetingAssistAttendeesGivenMeetingId, listMeetingAssistEventsForAttendeeGivenDates, listMeetingAssistPreferredTimeRangesGivenMeetingId, startMeetingAssist, upsertMeetingAssistPreferredTimes } from '@lib/api-helper'
import { AvailableSlotsByDate, EventType, MeetingAssistAttendeeType, MeetingAssistEventType, MeetingAssistPreferredTimeRangeType, ScheduleAssistWithMeetingQueueBodyType, MeetingAssistType, NotAvailableSlot, UserPreferenceType } from '@lib/types'
...
export default async function handler(
    ...
    try {
        const body: RequestDataForDeleteMeetingAssistPreferredTimesType
        ...
        if (body?.method === 'generateAvailableSlotsforTimeWindow') {
            ...
            } = (body as RequestDataForGenerateAvailableSlotsforTimeWindowType)?.variables
            const { availableSlotsByDate } = await generateAvailableSlotsforTimeWindow(
                windowStartDate,
                windowEndDate,
                slotDuration,
                hostPreferences,
        ...
        if (body?.method === 'getMeetingAssist') {
            const {
                id,
            } = (body as RequestDataForGetMeetingAssistType)?.variables
        ...
        if (body?.method === 'getUserPreferences') {
            const {
                userId,
            } = (body as RequestDataForGetUserPreferencesType)?.variables
        ...
        if (body?.method === 'listMeetingAssistAttendeesGivenMeetingId') {
            const {
                meetingId,
            } = (body as RequestDataForListMeetingAssistAttendeesType)?.variables
        ...
        if (body?.method === 'listMeetingAssistEventsForAttendeeGivenDates') {
            const {
                ...
                userTimezone,
                hostTimezone,
            } = (body as RequestDataForListMeetingAssistEventsForAttendeeType)?.variables
            const meetingAssistEvents = await listMeetingAssistEventsForAttendeeGivenDates(
                attendeeId,
                hostStartDate,
                hostEndDate,
                userTimezone,
        ...
        if (body?.method === 'listMeetingAssistPreferredTimeRangesGivenMeetingId') {
            const {
                meetingId,
            } = (body as RequestDataForListMeetingAssistPreferredTimeRangesType)?.variables
            const meetingAssistPreferredTimeRanges = await listMeetingAssistPreferredTimeRangesGivenMeetingId(
                meetingId,
            )
        ...
        if (body?.method === 'startMeetingAssist') {
            const {
                body: subBody,
            } = (body as RequestDataForStartMeetingAssistType)?.variables
            await startMeetingAssist(
                subBody,
            )
        ...
        if (body?.method === 'upsertMeetingAssistPreferredTimes') {
            const {
                preferredTimes,
            } = (body as RequestDataUpsertMeetingAsistPreferredTimesType)?.variables
            const meetingAssistPreferredTimesAffectedRows = await upsertMeetingAssistPreferredTimes(
                preferredTimes,
            )

            if ((typeof meetingAssistPreferredTimesAffectedRows === 'number') && (meetingAssistPreferredTimesAffectedRows > -1)) {
        ...
        if (body?.method === 'deleteMeetingAssistPreferredTimesByIds') {
            const {
                ids,
            } = (body as RequestDataForDeleteMeetingAssistPreferredTimesByIdsType)?.variables
            await deleteMeetingAssistPreferredTimesByIds(
                ids,
            )
        ...
        if (body?.method === 'createRecurringMeetingAssists') {
            ...
            } = (body as RequestDataForCreateRecurringMeetingAssistsType)?.variables
